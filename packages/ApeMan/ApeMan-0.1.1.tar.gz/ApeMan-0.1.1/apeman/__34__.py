...
The current implementation hooks into `import`.
It avoids the `ModuleSpec` and `finder`/`loader` mechanisms as Python 3.4 does not include a proper implementation of `FUNCTION`. 

.. note :: 

 I can't recall what `FUNCTION` was called but it had to do with useing a loader in a ModuleSpec, the lack of this function prevented one from developing a properly working `fidner`/`loader` and from developing a proper `ModuleSpec`.
 I know this was introduced in Python 3.5 however.
...
class OverlayImporter(abc.MetaPathFinder, utilities.Indentation): # , abc.SourceLoader
 """
...
#    return import_module(self.name + modsep + self.mapToTarget(name)) # This is a little black magic as we ignore the args
  self.log.debug("import : {}".format(name))
  return self.imp(name, *args, **kvps)
