...
%
% C:\cygwin64\bin\convert.exe -density 600 file.pdf file.png
...
% Trinity.pdf : Creates the Trinity
%
...
% \end{scope}
% Importer
% \node[above = of imp] (Imp) at ($(ctr) + (90:6em)$) {\lstinline|Importer|};
% \begin{scope}[on background layer]
% \draw[fill=gray, even odd rule] let \n{radius}={1.75em} in 
%    (imp) circle (\n{radius});
% \end{scope}
% \path[use as bounding box] circle (8.5em);
\end{tikzpicture}

% OverlayImporter.X.X.pdf : The OverlayImporter that wraps __import__ in Python 3.4
%
% One calls importlib.import_module if there isn't a from list
% and  the builtins.__import__ for all other cases
% 
% \begin{tikzpicture}
% \coordinate (ctr) {}; 
% \node (soc) at ($(ctr) + ( 90:6em)$) [text width = 16em, align=center]  {\lstinline|import package.module|\\ or \\\lstinline|from package import module|}; % Simple implies just name is given, complex implies globals/locals/fromlist and level are known
% \node (lib) at ($(ctr) + (180:6em)$) %[label=above:overlay.MODULE]       
%                                                                         {\lstinline|importlib.import_module|};
% \node (imp) at ($(ctr) + (  0:6em)$) %[label=above:overlay.MODULE]       
%                                                                         {\lstinline|self.import|};
% \node (gcd) at ($(ctr) + (270:6em)$)                                    {\lstinline|_gcd_import|};
% \graph {(soc) -> {(imp), (lib)} -> (gcd)};
% \end{tikzpicture}
...
% The following is a flow chart of the importlib (and I believe the builtins) __import__ function it maps the internal procedures used to perform an import
% \begin{tikzpicture}
% \coordinate (imp) {};
% \node (soc) [above       = of imp]  {simple or complex ?}; % Simple implies just name is given, complex implies globals/locals/fromlist and level are known
% \node (gcd) [right       = of imp]  {\lstinline|_gcd_import|};
% \node (clc) [left        = of imp]  {\lstinline|_calc__package__|};
% % \node (mod) [below       = of imp]  {\lstinline||};
% % In a way this occurs first : For now this is simply commented out
% % \node (mul) [below       = of imp]  {From list ?};
% % \node (aaa) [below right = of mul]  {\lstinline||};
% % \node (bbb) [below left  = of mul]  {\lstinline||};
% \graph {(soc) -> {(clc), (gcd)};
%         (clc) -> (gcd);
% %         {(gcd)} -> (mul);
%        };
% % \node [draw, rounded corners, fit = (gcd) (clc), label=\lstinline|builtins.__import__|/\lstinline|importlib.__import__|] {};
% % \node [draw, rounded corners, fit = (gcd), label=\lstinline|importlib.import_module|] {};
% \end{tikzpicture}
