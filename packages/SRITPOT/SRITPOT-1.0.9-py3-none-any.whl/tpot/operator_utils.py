...
#    config['__set_state__'] = __set_state__
    ...
    def transform(self, X):
        _logger.info("%s asked to transform data with %d rows" % (type(self), len(X)))
        result = self._prim.produce(inputs=DataFrame(X, generate_metadata=False)).value
        _logger.info("%s transformed to %d rows" % (type(self), len(result)))
        return result
    ...
    def predict(self, X):
        ...
        _logger.info("%s asked to predict on data with %d rows" % (type(self), len(X)))
        result = self._prim.produce(inputs=DataFrame(X, generate_metadata=False)).value.values
        _logger.info("%s produced %d predictions" % (type(self), len(result)))
        return result
