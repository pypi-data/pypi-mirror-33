...
from tcell_agent.policies.httptx_policy import HttpTxPolicy
from tcell_agent.policies.http_redirect_policy import HttpRedirectPolicy
from tcell_agent.policies.secure_headers_policy import SecureHeadersPolicy
from tcell_agent.policies.clickjacking_policy import ClickjackingPolicy
...
class TCellAgent(object):
    ...
    def process_policies(self, policies_json, cache=True):
        ...
        LOGGER = get_module_logger(__name__)
        policy_types = list(TCellAgent.policy_classes)
        for policy_type in policy_types + [PolicyTypes.APPSENSOR,
                                           PolicyTypes.PATCHES,
