...
cat("### Samples Dendogram  

**Figure 9:** 

Clustering dendogram of the samples according to the gene expression.
...
Further analysis is done for genes having more than 5 read over all samples.
")
if (exists("condition") && length(summary(condition))>1){ #there is no "condition" if counts_eval == F
    cat("

Normalization of the counts and differential expression analysis was performed using DESeq2 (DOI: [10.1186/s13059-014-0550-8](http://dx.doi.org/10.1186/s13059-014-0550-8)) with the betaPrior set to False. Raw P values were adjusted for multiple testing using the procedure of Benjamini and Hochberg.
    ")
} else if(count_eval) {
    cat("

Normalization of the counts was done using DESeq2 (DOI: [10.1186/s13059-014-0550-8](http://dx.doi.org/10.1186/s13059-014-0550-8)) with the betaPrior set to False. Raw P values were adjusted for multiple testing using the procedure of Benjamini and Hochberg.
    ")
}
