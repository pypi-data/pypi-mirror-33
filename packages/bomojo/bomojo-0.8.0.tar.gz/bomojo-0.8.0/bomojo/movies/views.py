...
from bomojo.utils import get_setting, format_money, split_on_whitespace
...
def box_office(request, movie_id):
    try:
        ...
        if request.GET.get('adjusted', '0') != '0':
            ...
            today = datetime.now()
            current_cpi = inflation_map.get((today.year, today.month))
            if current_cpi is None:
                current_cpi = inflation_map[sorted(inflation_map.keys())[-1]]
                inflation_map[(today.year, today.month)] = current_cpi

            def adjust_result(result, index):
                date = datetime.strptime(result['date'], '%b. %d, %Y')
                cpi = inflation_map[(date.year, date.month)]
