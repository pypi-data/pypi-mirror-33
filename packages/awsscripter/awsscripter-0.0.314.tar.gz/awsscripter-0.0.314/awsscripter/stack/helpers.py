...
def catch_exceptions(func):
    """
    Catches and simplifies expected errors thrown by awsscripter.
    ...
    def decorated(*args, **kwargs):
        ...
        exits awsscripter with a non-zero exit code.
        """
        try:
            return func(*args, **kwargs)
        except (awsscripterException, BotoCoreError, ClientError, Boto3Error,
...
def get_stack_or_env(ctx, path):
    ...
    config_reader = ConfigReader(
        ctx.obj["awsscripter_dir"], ctx.obj["user_variables"]
    )
...
def setup_logging(debug, no_colour):
    ...
    """
    if debug:
        awsscripter_logging_level = logging.DEBUG
        logging.getLogger("botocore").setLevel(logging.INFO)
    else:
        awsscripter_logging_level = logging.INFO
    ...
    logger = logging.getLogger("awsscripter")
    logger.addHandler(log_handler)
    logger.setLevel(awsscripter_logging_level)
    return logger
