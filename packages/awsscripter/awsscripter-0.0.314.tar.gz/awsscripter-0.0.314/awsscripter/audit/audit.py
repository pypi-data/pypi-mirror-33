...
"""
awsscripter.audit.audit

This module implements a Audit class, which stores data and logic associated
with audit.
...
class Audit(object):
    """
    Audit implements methods for carrying out audit operations, such as
    auditing based on standadrd.
    ...
    Attributes:
        ...
        Audit.CONTROL_1_1_DAYS (int): Description
        Audit.IAM_CLIENT (TYPE): Description
    ...
    def audit(self):
        ...
        :returns: The audit result json.
        :rtype: json
        """
        self._protect_execution()
        self.logger.info("%s - Starting Audit in region : ", self.connection_manager.region)
        audit_kwargs = {
        }
        if self.on_failure:
            audit_kwargs.update({"OnFailure": self.on_failure})
        audit_kwargs.update(self.template.get_boto_call_parameter())
        audit_kwargs.update(self._get_role_arn())
    ...
    def get_cred_report(self):
        ...
        x = 0
        status = ""
        while Audit.IAM_CLIENT.generate_credential_report()['State'] != "COMPLETE":
            time.sleep(2)
            x += 1
        ...
        if "Fail" in status:
            return status
        response = Audit.IAM_CLIENT.get_credential_report()
        report = []
    ...
    def get_account_number(self):
        ...
        Returns:
            TYPE: Description
        """
        if Audit.S3_WEB_REPORT_OBFUSCATE_ACCOUNT is False:
    ...
    def s3report(htmlReport, account):
        ...
        Returns:
            TYPE: Description
        """
        if Audit.S3_WEB_REPORT_NAME_DETAILS is True:
        ...
        with tempfile.NamedTemporaryFile(mode='w',delete=False) as f:
            ...
            try:
                f.close()
                Audit.S3_CLIENT.upload_file(f.name, AUdit.S3_WEB_REPORT_BUCKET, reportName)
                os.unlink(f.name)
            except Exception as e:
                return "Failed to upload report to S3 because: " + str(e)
        ttl = int(Audit.S3_WEB_REPORT_EXPIRE) * 60
        signedURL = Audit.S3_CLIENT.generate_presigned_url(
            'get_object',
            Params={
                'Bucket': Audit.S3_WEB_REPORT_BUCKET,
                'Key': reportName
            },
            ExpiresIn=ttl)
        return signedURL
    ...
    def lambda_handler(self, event, context):
        ...
        cloud_trails = self.get_cloudtrails(region_list)
    ...
    if __name__ == '__main__':
        ...
        if not profile_name == "":
            try:
                ...
                Audit.IAM_CLIENT = boto3.client('iam')
                Audit.S3_CLIENT = boto3.client('s3')
            except Exception as e:
