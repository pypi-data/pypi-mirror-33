...
class ConfigReader(object):
    """
    Respresents a awsscripter project folder. Reads in yaml configuration files and
    ...
    :param awsscripter_dir: The absolute path to the awsscripter directory.
    :type awsscripter_dir: str
    """

    def __init__(self, awsscripter_dir, variables=None):
        self.logger = logging.getLogger(__name__)

        self.awsscripter_dir = awsscripter_dir

        # Check is valid awsscripter project folder
        self.config_folder = path.join(self.awsscripter_dir, "config")
        self._check_valid_awsscripter_dir(self.config_folder)

        # Add Resolver and Hook classes to PyYAML loader
        self._add_yaml_constructors(
            ["awsscripter.hooks", "awsscripter.resolvers"]
        )
    ...
    def read(self, rel_path, base_config=None):
        """
        Reads in configuration from yaml files within the awsscripter project
        folder.
        ...
        config = {
            "awsscripter_dir": self.awsscripter_dir,
    ...
    @staticmethod
    def _check_valid_awsscripter_dir(config_path):
        """
        Raises an InvalidawsscripterDirectoryError if ``path`` is not a directory.
    ...
    def _check_version(self, config):
        ...
        if 'require_version' in config:
            ...
            if Version(awsscripter_version) not in SpecifierSet(require_version):
                raise VersionIncompatibleError(
                    "Current awsscripter version ({0}) does not meet version "
                    "requirements: {1}".format(
                        awsscripter_version, require_version
                    )
                )
    ...
    def _construct_stack(self, rel_path, environment_config=None):
        ...
        if filename != "config.yaml":
            ...
            abs_template_path = path.join(
                self.awsscripter_dir, config["template_path"]
            )
            ...
            stack = Stack(
                ...
                awsscripter_user_data=config.get("awsscripter_user_data", {}),
    ...
    def construct_environment(self, rel_path):
        ...
        items = glob(
            path.join(self.awsscripter_dir, "config", rel_path, "*")
        )

        paths = {
            item: path.relpath(
                item, path.join(self.awsscripter_dir, "config")
            )
