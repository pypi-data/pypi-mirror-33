...
from awsscripter.common.exceptions import TemplateawsscripterHandlerError
from awsscripter.common.exceptions import UnsupportedTemplateFileTypeError
...
class Template(object):
    ...
    :type path: str
    :param awsscripter_user_data: A dictionary of arbitrary data to be passed to \
        a handler function in an external Python script.
    :type awsscripter_user_data: dict
    """

    _boto_s3_lock = threading.Lock()

    def __init__(
        self, path, awsscripter_user_data, connection_manager=None, s3_details=None
    ):
        self.logger = logging.getLogger(__name__)

        self.path = path
        self.awsscripter_user_data = awsscripter_user_data
    ...
    def __repr__(self):
        return (
            "awsscripter.template.Template(name='{0}', path='{1}', "
            "awsscripter_user_data={2}, s3_details={3})".format(
                self.name, self.path, self.awsscripter_user_data, self.s3_details
            )
        )
    ...
    def body(self):
        ...
        if self._body is None:
            ...
            if file_extension in {".json", ".yaml"}:
                ...
                self._body = self._call_awsscripter_handler()
    ...
    def _call_awsscripter_handler(self):
        """
        Calls the function `awsscripter_handler` within templates that are python
        scripts.

        :returns: The string returned from awsscripter_handler in the template.
        :rtype: str
        :raises: IOError
        :raises: TemplateawsscripterHandlerError
        """
        ...
        try:
            body = module.awsscripter_handler(self.awsscripter_user_data)
        except AttributeError as e:
            if 'awsscripter_handler' in str(e):
                raise TemplateawsscripterHandlerError(
                    "The template does not have the required "
                    "'awsscripter_handler(awsscripter_user_data)' function."
                )
            else:
                raise e
        for directory in relpaths_to_add:
    ...
    def _render_jinja_template(template_dir, filename, jinja_vars):
        ...
        awsscripter supports passing awsscripter_user_data to JSON and YAML
