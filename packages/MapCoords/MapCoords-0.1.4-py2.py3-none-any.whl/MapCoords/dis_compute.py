...
【距离计算】
计算经纬度坐标间的球面大弧距离
1. 地球的模型有球体和椭球体两种，后者相对前者要更精确一些
2. 常见的三种坐标系下（GPS、GCJ、BD），计算的都应当是球体或椭球体模型下的球面大弧距离；
   但是墨卡托投影坐标系下，计算的应当是两点间的欧式距离。
3. geopy模块中已有两点间球面大弧距离的计算函数，但是对两列点计算距离矩阵似乎并未提供较直接高效的方式，
   所以我写了这个模块，并以geopy中函数来检验结果，在此致谢。
4. 本模块中球面大弧距离的计算，等同于geopy模块中使用同样的地球半径的球体模型，通过几组对照试验发现，其与椭球体模型相比，千米大概误差一米
5. 关于效率，由于距离矩阵的计算是可以并行的CPU密集型任务，所以采用多核的计算机多进程计算可大幅度缩减时间（尚未编写）
6. scipy.spatial.distance模块中cdist函数实现了对多种类型距离的距离矩阵的计算，其中对欧氏距离的计算效率要高于我自己用numpy的实现；
...
def compute_arc_dis(lgt1, lat1, lgt2, lat2, need_convert=True, earth_radius=DIS_COMPUTE_EARTH_RADIUS):
    """
    由两（列）点的经纬度计算其球面大弧距离
    在计算距离时由于使用的球体模型，本身估算就带有一定误差，因此对于坐标系并不敏感，只要不是投影坐标系，都可以用该公式来估算距离
    ...
    e.g.
        1. 传入：第一个点：x1           第二个点：y1           得到：dis(x1, y1)
        2. 传入：第一批点：x1, x2, x3   第二个点：y1           得到：dis(x1, y1), dis(x2, y1), dis(x3, y1)
        3. 传入：第一批点：x1, x2, x3   第二个点：y1, y2, y3   得到：dis(x1, y1), dis(x2, y2), dis(x3, y3)

    :param lgt1: 点1的经度，数据类型为float或者np.ndarray
    :param lat1: 点1的纬度
    :param lgt2: 点2的经度
    :param lat2: 点2的纬度
    :param need_convert: 是否需要从经纬度转换至弧度制（默认为True）
    :param earth_radius: 地球半径（米）
    :return: distance: 距离（米），数据类型为float或np.ndarray
    """
    ...
    # 计算距离
    h = haver_sin(lat2 - lat1) + np.cos(lat1) * np.cos(lat2) * haver_sin(lgt2 - lgt1)
    distance = 2 * earth_radius * np.arcsin(np.sqrt(h))

    return distance


def compute_arc_dis_matrix(lgts1, lats1, lgts2, lats2, need_convert=True):
    """
    计算两批点之间两两的球面大弧距离，返回距离矩阵

    e.g. 传入：第一批点：x1, x2, x3   第二批点：y1, y2
         得到：dis(x1, y1), dis(x1, y2)
              dis(x2, y1), dis(x2, y2)
              dis(x3, y1), dis(x3, y2)
    ...
    for i in range(len(lgts2)):
        distance_matrix[:, i] = compute_arc_dis(lgts1, lats1, lgts2[i], lats2[i], False)

    return distance_matrix


def compute_arc_dis_matrix_symmetric(lgts, lats, need_convert=True):
    """
    同一列点计算彼此间距离矩阵
    由于得到的是对称阵，而赋值的耗时远小于运算的耗时，所以可以只计算对角线以上部分，同时赋值给对称的区域，缩减了一半时间左右
    ...
    for i in range(1, l):
        distance_matrix[i, : i] = distance_matrix[: i, i] = compute_arc_dis(lgts[: i], lats[: i], lgts[i], lats[i], False)
    # distance_matrix += distance_matrix.T  # 矩阵运算耗时超乎想象
...
def compute_line_dis_matrix(points1, points2):
    """
    计算两组点间的平面欧氏距离，返回其距离矩阵
    直接调用了scipy.spatial.distance中的函数cdist
...
def compute_line_dis_matrix_not_recommended(lgts1, lats1, lgts2, lats2):
    """
...
if __name__ == "__main__":
    ...
    gc.collect()
    time1 = time.time()
    results2 = compute_line_dis_matrix_not_recommended(lgts, lats, lgts, lats)
    print("{0}*{1} style2:".format(len(lgts), len(lgts)), time.time() - time1)
    results2_sample = results2[: 100, : 100].copy()
    del results2
    gc.collect()
