...
from impetuous.data import soften_datetime
...
def main():
    ...
    parser.add_argument('--since', help='Operate on entries starting after ...',
                        type=friendly_datetime, default=since)
    parser.add_argument('--until', help='Operate on entries ending before ...',
                        type=friendly_datetime, default=until)
    ...
    utcnow_ish = utcnow().astimezone(localtz) - timedelta(days=args.yesterday)
    datetime_arg = partial(friendly_datetime, default=utcnow_ish)
    datetime_arg.__name__ = 'super cool datetime'
