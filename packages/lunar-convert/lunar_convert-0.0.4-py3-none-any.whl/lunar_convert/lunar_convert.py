...
Usage
-----
        >>> LunarConvert.fromSolarDate(1976, 10, 1)
        LunarConvert(1976, 8, 8, 1)
        >>> LunarConvert(1976, 8, 8, 1).toSolarDate()
        datetime.date(1976, 10, 1)
        >>> LunarConvert(1976, 8, 8, 1).year
        1976
        >>> LunarConvert(1976, 8, 8, 1).month
        8
        >>> LunarConvert(1976, 8, 8, 1).day
        8
        >>> LunarConvert(1976, 8, 8, 1).isLeapMonth
        True

        >>> today = LunarConvert.today()
        >>> type(today).__name__
        'LunarConvert'

        >>> # support '+' and '-' between datetime.date and datetime.timedelta
        >>> ld = LunarConvert(1976,8,8)
        ...
        >>> ld-td
        LunarConvert(1976, 7, 27, 0)
        >>> sd-ld
        datetime.timedelta(11444)
        >>> ld+td
        LunarConvert(1976, 8, 18, 0)
        >>> td+ld
        LunarConvert(1976, 8, 18, 0)
        >>> ld2 = LunarConvert.today()
        >>> ld < ld2
        True
        >>> ld <= ld2
        True
        ...
        >>> ld == ld
        True
        >>> LunarConvert.today() == LunarConvert.today()
        True
...
__version__ = "0.1.5"
__all__ = ['LunarConvert']

class LunarConvert(object):
    ...
    def __str__(self):
        return 'LunarConvert(%d, %d, %d, %d)' % (self.year, self.month, self.day, self.isLeapMonth)
    ...
    def fromSolarDate(year, month, day):
        '''
        >>> LunarConvert.fromSolarDate(1900, 1, 31)
        LunarConvert(1900, 1, 1, 0)
        >>> LunarConvert.fromSolarDate(2008, 10, 2)
        LunarConvert(2008, 9, 4, 0)
        >>> LunarConvert.fromSolarDate(1976, 10, 1)
        LunarConvert(1976, 8, 8, 1)
        >>> LunarConvert.fromSolarDate(2033, 10, 23)
        LunarConvert(2033, 10, 1, 0)
        '''
        solarDate = datetime.date(year, month, day)
        offset = (solarDate - LunarConvert._startDate).days
        return LunarConvert._fromOffset(offset)

    def toSolarDate(self):
        '''
        >>> LunarConvert(1900, 1, 1).toSolarDate()
        datetime.date(1900, 1, 31)
        >>> LunarConvert(2008, 9, 4).toSolarDate()
        datetime.date(2008, 10, 2)
        >>> LunarConvert(1976, 8, 8, 1).toSolarDate()
        datetime.date(1976, 10, 1)
        >>> LunarConvert(2004, 1, 30).toSolarDate()
        Traceback (most recent call last):
        ...
        ValueError: day out of range
        >>> LunarConvert(2050, 1, 1).toSolarDate()
        Traceback (most recent call last):
        ...
    ...
    def __eq__(self, other):
        '''
        >>> LunarConvert.today() == 5
        False
        '''
        if not isinstance(other, LunarConvert):
            return False
    ...
    def __lt__(self, other):
        ...
        Traceback (most recent call last):
        ...
        TypeError: can't compare LunarConvert to int
        '''
        try:
            return self - other < datetime.timedelta(0)
        except TypeError:
            raise TypeError("can't compare LunarConvert to %s" % (type(other).__name__,))
    ...
    @classmethod
    def nextday(cls, offset):
        res = datetime.date.today() + datetime.timedelta(days=offset)
        return cls.fromSolarDate(res.year, res.month, res.day)
    ...
    def _fromOffset(cls, offset):
        ...
        return LunarConvert(year, month, day, isLeapMonth)
