...
def cloudScattering(q, cloud, relError=50, V=0, formfactor=None, rms=0, ffpolydispersity=0,ncpu=0):
    ...
    The scattering intensity of a single object represented by the cloud is :math:`I(Q)=P(Q) \cdot (\int_V b(r) \mathrm{d}r)^2`.
    ...
    Therefore avoid to use the same gridpoint distance with changing number of points e.g. inside of your object.
...
def ellipsoidFilledCylinder(q=1, R=10, L=0, Ra=1, Rb=2, eta=0.1, SLDcylinder=0.1, SLDellipsoid=1, SLDmatrix=0, alpha=90,
    ...
    if 0:
        # above some first minimum the ellipsoid beta is artificial high compared to proteins (potato like),
...
def superball(q,R,p,SLD=1,solventSLD=0,nGrid=15,returngrid=False):
    ...
    Use the time to think about the advantages of an analytical solution if it is available and atomic resolution.
    ...
    # surrounding cubic grid as start
    D=R*3**0.5
    grid0=np.mgrid[-D:D:1j*nGrid,-D:D:1j*nGrid,-D:D:1j*nGrid].reshape(3,-1).T         # 89 µs
    # rotate to remove alignment of borders to grid
