...
async def handshake():
    ...
    async with websockets.connect(sys.argv[1]) as websocket:
        ...
        try:
            ...
            print("Unkown JSON")
        sys.stdout.write(colored(returned["stdout"], "green"))
        sys.stdout.write(colored(returned["stderr"], "red"))
        if returned["stderr"] == "Bad password !":
            return 0
        return 1
...
def main():
    global slave
    while True:
        if asyncio.get_event_loop().run_until_complete(handshake()):
            try:
                asyncio.get_event_loop().run_until_complete(send_commands())
            except EOFError:
                print("Exiting {0}".format(slave))
