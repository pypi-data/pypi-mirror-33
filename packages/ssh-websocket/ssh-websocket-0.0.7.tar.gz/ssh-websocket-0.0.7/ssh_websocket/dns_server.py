...
class WebSSHFactory(websocket.WebSocketServerFactory):
    ...
    def register(self, client):
        ...
        self.clients[client.peer] = {"object": client, "name": None, "pswd": ""}
        ...
        for i, v in self.clients.items():
            if v["name"] == infos["slave"]:
                if infos["pswd"] == v["pswd"]:
                    ...
                    client.sendMessage(json.dumps({"stderr": "Bad password !\n", "stdout": ""}).encode())
                    print("Bad password")
...
def main():
    ...
    with open(sys.argv[1], "r") as cert:
        pems = cert.read()
    contextFactory = pem.twisted.certificateOptionsFromPEMs(pems)
    factory = WebSSHFactory("wss://0.0.0.0:8080")
