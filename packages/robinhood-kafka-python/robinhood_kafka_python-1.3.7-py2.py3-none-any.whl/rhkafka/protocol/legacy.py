...
class KafkaProtocol(object):
    ...
    def encode_produce_request(cls, payloads=(), acks=1, timeout=1000):
        ...
        for topic, topic_payloads in group_by_topic_and_partition(payloads).items():
            ...
            for partition, payload in topic_payloads.items():
                ...
                for msg in payload.messages:
                    m = rhkafka.protocol.message.Message(
        ...
        return rhkafka.protocol.produce.ProduceRequest[0](
            required_acks=acks,
            timeout=timeout,
            topics=topics
        )
    ...
    def decode_produce_response(cls, response):
        ...
        """
        return [
            rhkafka.structs.ProduceResponsePayload(topic, partition, error, offset)
    ...
    def encode_fetch_request(cls, payloads=(), max_wait_time=100, min_bytes=4096):
        ...
        Return: FetchRequest
        """
        return rhkafka.protocol.fetch.FetchRequest[0](
            replica_id=-1,
    ...
    def decode_fetch_response(cls, response):
        ...
        """
        return [
            rhkafka.structs.FetchResponsePayload(
    ...
    def decode_message_set(cls, messages):
        for offset, _, message in messages:
            if isinstance(message, rhkafka.protocol.message.Message) and message.is_compressed():
                inner_messages = message.decompress()
                for (inner_offset, _msg_size, inner_msg) in inner_messages:
                    yield rhkafka.structs.OffsetAndMessage(inner_offset, inner_msg)
            else:
                yield rhkafka.structs.OffsetAndMessage(offset, message)

    @classmethod
    def encode_offset_request(cls, payloads=()):
        return rhkafka.protocol.offset.OffsetRequest[0](
            replica_id=-1,
            topics=[(
                topic,
                [(
    ...
    def decode_offset_response(cls, response):
        ...
        """
        return [
            rhkafka.structs.OffsetResponsePayload(topic, partition, error, tuple(offsets))
            for topic, partitions in response.topics
    ...
    def encode_list_offset_request(cls, payloads=()):
        return rhkafka.protocol.offset.OffsetRequest[1](
            replica_id=-1,
            topics=[(
                topic,
                [(
    ...
    def decode_list_offset_response(cls, response):
        ...
        """
        return [
            rhkafka.structs.ListOffsetResponsePayload(topic, partition, error, timestamp, offset)
            for topic, partitions in response.topics
    ...
    def encode_metadata_request(cls, topics=(), payloads=None):
        ...
        return rhkafka.protocol.metadata.MetadataRequest[0](topics)
    ...
    def encode_offset_commit_request(cls, group, payloads):
        ...
        """
        return rhkafka.protocol.commit.OffsetCommitRequest[0](
    ...
    def decode_offset_commit_response(cls, response):
        ...
        """
        return [
            rhkafka.structs.OffsetCommitResponsePayload(topic, partition, error)
            for topic, partitions in response.topics
    ...
    def encode_offset_fetch_request(cls, group, payloads, from_kafka=False):
        ...
        return rhkafka.protocol.commit.OffsetFetchRequest[version](
            consumer_group=group,
            topics=[(
                topic,
    ...
    def decode_offset_fetch_response(cls, response):
        ...
        """
        return [
            rhkafka.structs.OffsetFetchResponsePayload(
...
def create_message(payload, key=None):
    ...
    """
    return rhkafka.structs.Message(0, 0, key, payload)
...
def create_gzip_message(payloads, key=None, compresslevel=None):
    ...
    return rhkafka.structs.Message(0, 0x00 | codec, key, gzipped)
...
def create_snappy_message(payloads, key=None):
    ...
    return rhkafka.structs.Message(0, 0x00 | codec, key, snapped)
