...
def _send_upstream(queue, client, codec, batch_time, batch_size,
    ...
    Arguments:
        ...
        client (rhkafka.SimpleClient): instance to use for communicating
            with brokers
        codec (rhkafka.protocol.ALL_CODECS): compression codec to use
...
class Producer(object):
    ...
    Arguments:
        client (rhkafka.SimpleClient): instance to use for broker
            communications. If is_async=True, the background thread will use
    ...
    def __del__(self):
        if self.is_async and not self.stopped:
            self.stop()
