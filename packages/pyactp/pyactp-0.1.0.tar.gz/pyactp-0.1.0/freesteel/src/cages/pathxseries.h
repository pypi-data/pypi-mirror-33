...
class PathXSeries {
 public:
  ...
  void Add(const P2& pt) { pths.push_back(pt); }
  ...
  int GetNbrks() { return brks.size(); }

  int GetBrkIndex(int i) { return brks[i]; }

  int GetNlnks(int i) { return linkpths[i].size(); }

  double GetLinkX(int j, int i) { return linkpths[j][i].x; }

  double GetLinkY(int j, int i) { return linkpths[j][i].y; }

  double GetLinkZ(int j, int i) { return linkpths[j][i].z; }

  int GetNpts() { return pths.size(); }

  double GetX(int i) { return pths[i].u; }
  double GetY(int i) { return pths[i].v; }
};
...
class PathXboxed {
 public:
  ...
  PathXboxed() {}
  PathXboxed(PathXSeries* lppathx) : ppathx(lppathx) { ; };
