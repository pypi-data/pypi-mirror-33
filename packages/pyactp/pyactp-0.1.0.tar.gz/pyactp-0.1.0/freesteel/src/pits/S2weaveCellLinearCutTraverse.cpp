...
void S2weaveCellLinearCutTraverse::Findibbfore(int libb) {
  for (ibb = libb; ibb < (int)bolistcrossings.size(); ibb++) {
    if (!bolistcrossings[ibb].second) {
      ...
      // facing a down crossing; we are inside the area, and shouldn't be since
      // we can't model being here
      lamcpbb = Getbolistcrossing(lambb, ptcpbb, ibb);
      ASSERT(lamcpbb <= 0.0);
    }
#endif
...
//////////////////////////////////////////////////////////////////////
void S2weaveCellLinearCutTraverse::SetCellCutBegin(const P2& lptcst,
                                                   const P2& lvbearing) {
...
// this handles leaving a contour, and continuing (with a change of direction)
// while not on a contour.
...
bool S2weaveCellLinearCutTraverse::SetCellCutContinue(const P2& lvbearing) {
  ...
  if (bOnContour && EqualOr(lambb, 0.0, 1.0)) {
    ...
    int sic = (lambb == 0.0 ? boundlist[bolistpairs[ib].first].first
                            : boundlist[bolistpairs[ib].second].first);
  ...
  // ASSERT(!bOnBoundB || boundlist[bolistpairs[pib]].first :
  // boundlist[bolistpairs[ib]].second);
  ...
  if (bOnContour) {
    ...
    // if this is so and the iblb is right, then it won't be represented in the
    // list of crossings
    if (lambb == 0.0) {
#ifdef MDEBUG
      if (!bOnBoundF) {
    ...
    // we should see it in the list of crossings and move on.
      ...
      TOL_ZERO(
          (Along(lambb, GetBoundPoint(ccrD.first), GetBoundPoint(ccrD.second)) -
           ptcpbb)
              .Len());
#endif
...
//////////////////////////////////////////////////////////////////////
void S2weaveCellLinearCutTraverse::AdvanceThroughForeCutTraverse(
    bool bOnBoundF) {
...
void S2weaveCellLinearCutTraverse::AdvanceToContourCut() {
  ...
  TOL_ZERO(
      (Along(lambb, GetBoundPoint(ccr.first), GetBoundPoint(ccr.second)) - ptcp)
          .Len());
...
void S2weaveCellLinearCutTraverse::AdvanceAlongContourToLamPos(double llambn) {
  ...
  pair<int, int> ccr = bolistpairs[ib];
  TOL_ZERO((Along(lambb, GetBoundPoint(ccr.first), GetBoundPoint(ccr.second)) -
            ptcpbb)
               .Len());
...
void S2weaveCellLinearCutTraverse::AdvanceAlongContourAcrossCell() {
  ...
  // find the point in the boundlist which matches the one we are crossing over
  // on.
...
//////////////////////////////////////////////////////////////////////
bool S2weaveCellLinearCutTraverse::OnContourFollowBearing(double dch,
                                                          double folldist) {
  ...
  if (llambn <= 1.0) {
    ...
    // we will cross this cell boundary.  Mark it if it's entirely cleared from
    // the start.
...
//////////////////////////////////////////////////////////////////////
double S2weaveCellLinearCutTraverse::FollowBearing(const P2& lvbearing,
                                                   double folldist) {
