...
class NormRay_gen {
 public:
  ...
  double reslo;
  bool binterncellboundlo;  // tells it's the flat end of the cylinder, which
                            // must be covered by a ball.
  double reshi;
...
class Ray_gen : public NormRay_gen {
 public:
  ...
  Ray_gen(double lradball, const I1& lwrg) : NormRay_gen(lradball, lwrg) { ; };

  void HoldFibre(S1* lpfib, double lz);

  P3 Transform(const P3& p) {
    return (pfib->ftype == 1 ? P3(p.x - pfib->wp, p.z - radball - z, p.y)
                             : P3(p.z - radball - z, p.y - pfib->wp, p.x));
  }
...
class Ray_gen2 {
 public:
  ...
  Ray_gen2(double lraddisc) : raddisc(lraddisc), raddiscsq(Square(raddisc)) {
    ;
  };
  ...
  P2 Transform(const P2& p) {
    return (pfib->ftype == 1 ? P2(p.u - pfib->wp, p.v)
                             : P2(p.v - pfib->wp, p.u));
  }
};
