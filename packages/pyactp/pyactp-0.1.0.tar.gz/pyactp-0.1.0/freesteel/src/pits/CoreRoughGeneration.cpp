...
// Copyright (C) 2004  Julian Todd and Martin Dunschen, with modifcations by Dan
// Heeks 2009
//
...
void MakeCorerough(vector<PathXSeries>& vpathseries,
  ...
  Area2_gen a2g;
  a2g.SetShape(sx.gxrg.Inflate(areaoversize), sx.gyrg.Inflate(areaoversize),
               params.triangleweaveres);
  a2g.SetSurfaceTop(&sxb, params.toolcornerrad);

  Area2_gen a2gfl;
  a2gfl.SetShape(sx.gxrg.Inflate(areaoversize), sx.gyrg.Inflate(areaoversize),
                 params.flatradweaveres);
  ...
  while (hz > htopz) {
    ...
    CoreRoughGeneration crg(&vpathseries.back(), sx.gxrg.Inflate(10),
                            sx.gyrg.Inflate(10));
    // the stock boundary
    //	crg.tsbound.paths.insert(crg.tsbound.paths.end(),
    // bound.ftpaths.paths.begin(), bound.ftpaths.paths.end());
    ...
    crg.trad =
        params.toolcornerrad * 0.9 + params.toolflatrad;  // the clearing radius
...
///////////////////////////////////////////////////////////
void BuildRetract(vector<P3>& lnkpth,
                  const P3& pts,
                  const P3& pte,
                  const MachineParams& params) {
...
///////////////////////////////////////////////////////////
void BuildCurl(vector<P2>& lnkpth,
               const P2& pts,
               const P2& dirs,
               const MachineParams& params,
               bool bCurlIn) {
...
void BuildLink(vector<P2>& lnkpth,
               const P2& pts,
               const P2& dirs,
               const P2& pte,
               const P2& dire,
...
void BuildLinkZ(vector<P3>& lnkpth,
                const vector<P2>& lnk2D,
                double z,
                const MachineParams& params) {
  // total length
  double totallen = 0;
  ...
  ASSERT((lnkStart.back().z > z) &&
         (lnkStart.back().z <= params.leadoffdz + z));
...
int CoreRoughGeneration::TrackLink(const vector<P2>& lnk2D,
...
void CoreRoughGeneration::GrabberAlg(const MachineParams& params) {
  ...
  while (true) {
    ...
    if (itracked < (int)lnk2D.size()) {
      ...
      P3 ptStartRetract =
          bUseOut
              ? ConvertGZ(curlout.back(), pathxb.ppathx->z + params.leadoffdz)
              : ConvertGZ(wclink.ptcp, pathxb.ppathx->z);
      P3 ptEndRetract = bUseIn ? ConvertGZ(curlin.front(),
                                           pathxb.ppathx->z + params.leadoffdz)
                               : ConvertGZ(wc.ptcp, pathxb.ppathx->z);

      if (bUseOut) {
        double z = pathxb.ppathx->z;
        double zstep =
            params.leadoffsamplestep * params.leadoffdz / params.leadofflen;
...
///////////////////////////////////////////////////////////
bool CoreRoughGeneration::RestartAtBCI(BCellIndex& bci,
                                       const MachineParams& params,
  ...
  if (bci.ib != -1) {
    ...
    P2 Nvbearing =
        wc.GetBoundPoint(wc.bolistpairs[wc.ib].second) - wc.GetBoundPoint(ibl);
...
///////////////////////////////////////////////////////////
CoreRoughGeneration::CoreRoughGeneration(PathXSeries* px,
                                         const I1& lxrg,
                                         const I1& lyrg)
...
///////////////////////////////////////////////////////////
double CoreRoughGeneration::ChangeBearing(const P2& pt,
                                          const P2& ltvec,
                                          const MachineParams& params) {
  ...
  ASSERT(lccpath.empty() ||
         ((lccpath.front().lo == 0.0) == (lccpath.back().hi == 4.0)));

  // control for the change of direction depending on how much stock we are
  // touching.
  double dch;
  if (!lccpath.empty()) {
    countfreespacesteps = 0;
    ...
    double ldch =
        (dchden != 0.0 ? dchnum / dchden : (ghi >= datvec ? -99.0 : 99.0));
    ...
    // turn right to go more into it.
      ...
      ASSERT(ldch >= -MDTOL);
      dch = min(ldch, (wc.bOnContour ? params.dchangrightoncontour
                                     : params.dchangright));
    }
    ...
    if ((int)(lccpath.size()) >
        (((lccpath.front().lo == 0.0) && (lccpath.back().hi == 4.0)) ? 2 : 1)) {
