...
struct P2  // 2D point
{
  ...
  P2 operator-(const P2& a) const { return P2(u - a.u, v - a.v); }
  P2 operator+(const P2& a) const { return P2(u + a.u, v + a.v); }
  P2 operator*(double f) const { return P2(u * f, v * f); }
  P2 operator/(double f) const { return P2(u / f, v / f); }
  ...
  double Lensq() const { return u * u + v * v; }
  ...
  bool operator==(const P2& b) const { return ((u == b.u) && (v == b.v)); }
  bool operator!=(const P2& b) const { return ((u != b.u) || (v != b.v)); }
};
...
inline P2 P2::InvDarg(double a) {
  if (a == 4.0)
    a = 0.0;
  ASSERT((0.0 <= a) && (a < 4.0));
  P2 res((a < 2.0 ? (1.0 - a) : (a - 3.0)),
         (a < 3.0 ? ((a > 1.0) ? (2.0 - a) : a) : (a - 4.0)));
...
inline double P2::Arg() const {
  // Use atan2 instead of old Arg code - check that they give the same result
  // for now
