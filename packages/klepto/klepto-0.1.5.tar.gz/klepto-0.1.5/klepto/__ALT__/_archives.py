...
class dir_archive(dict):
    ...
    def _lookup(self, key, input=False):
        ...
        _dir = self._getdir(key)
        if input:
            try: # we don't impose output encoding on input, always dill.load
                _args = os.path.join(_dir, self._args)
                f = open(_args, 'rb')
                memo = dill.load(f)
                f.close()
    ...
    def _store(self, key, value, input=False):
        ...
        try:
            ...
            if input: # don't impose output encoding on input, always dill.dump
                _args = os.path.join(self._getdir(_key), self._args)
                f = open(_args, 'wb')
                dill.dump(key, f)  #XXX: byref=True ?
                f.close()
        except OSError:
