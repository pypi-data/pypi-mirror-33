...
class X500DistinguishedNameFields(X500DistinguishedName):
    ...
    *(extends* :py:class:`delphixpy.web.vo.X500DistinguishedName` *)* The
    ...
    def to_dict(self, dirty=False):
        dct = super(X500DistinguishedNameFields, self).to_dict(dirty)
        ...
        def dictify(obj):
            if isinstance(obj, list):
                ...
                return obj
        if "city" == "type" or (self.city is not self.__undef__ and not (dirty and not self._city[1])):
            dct["city"] = dictify(self.city)
        if "country" == "type" or (self.country is not self.__undef__ and not (dirty and not self._country[1])):
            dct["country"] = dictify(self.country)
        if "common_name" == "type" or (self.common_name is not self.__undef__ and not (dirty and not self._common_name[1])):
            dct["commonName"] = dictify(self.common_name)
        if "organization_unit" == "type" or (self.organization_unit is not self.__undef__ and not (dirty and not self._organization_unit[1])):
            dct["organizationUnit"] = dictify(self.organization_unit)
        if "state_region" == "type" or (self.state_region is not self.__undef__ and not (dirty and not self._state_region[1])):
            dct["stateRegion"] = dictify(self.state_region)
        if "organization" == "type" or (self.organization is not self.__undef__ and not (dirty and not self._organization[1])):
