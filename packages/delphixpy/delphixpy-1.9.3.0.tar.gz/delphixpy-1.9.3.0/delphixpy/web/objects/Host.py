...
class Host(ReadonlyNamedUserObject):
    ...
    *(extends* :py:class:`delphixpy.web.vo.ReadonlyNamedUserObject` *)* The
    ...
    def to_dict(self, dirty=False):
        ...
        def dictify(obj):
            if isinstance(obj, list):
                ...
                return obj
        if "dsp_keystore_password" == "type" or (self.dsp_keystore_password is not self.__undef__ and not (dirty and not self._dsp_keystore_password[1])):
        ...
        if "dsp_keystore_path" == "type" or (self.dsp_keystore_path is not self.__undef__ and not (dirty and not self._dsp_keystore_path[1])):
            dct["dspKeystorePath"] = dictify(self.dsp_keystore_path)
        if "nfs_address_list" == "type" or (self.nfs_address_list is not self.__undef__ and not (dirty and not self._nfs_address_list[1])):
            dct["nfsAddressList"] = dictify(self.nfs_address_list)
        if "dsp_keystore_alias" == "type" or (self.dsp_keystore_alias is not self.__undef__ and not (dirty and not self._dsp_keystore_alias[1])):
    ...
    def is_dirty(self):
        return any([self._dsp_keystore_password[1], self._date_added[1], self._privilege_elevation_profile[1], self._host_configuration[1], self._dsp_keystore_path[1], self._nfs_address_list[1], self._dsp_keystore_alias[1], self._host_runtime[1], self._address[1], self._ssh_port[1]])
