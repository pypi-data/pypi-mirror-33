...
class NetbackupConnectivityParameters(TypedObject):
    ...
    *(extends* :py:class:`delphixpy.web.vo.TypedObject` *)* Parameters needed
    ...
    def to_dict(self, dirty=False):
        dct = super(NetbackupConnectivityParameters, self).to_dict(dirty)
        ...
        if "master_server_name" == "type" or (self.master_server_name is not self.__undef__ and not (dirty and not self._master_server_name[1])):
            dct["masterServerName"] = dictify(self.master_server_name)
        if "source_client_name" == "type" or (self.source_client_name is not self.__undef__ and not (dirty and not self._source_client_name[1])):
