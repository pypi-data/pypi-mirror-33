...
def ipmi_sensor_netxms_format(
        ...
        filter_thresholds=False, sensor_name=None,
        list_sensor_types=False, filter_by_headers=None):
    """
    ...
    if filter_by_headers:
        header_filter_list = filter(
            lambda header: header, filter_by_headers.split(","))
        header_key_list = []
        for header in header_filter_list:
            if header not in HDRMAP:
                raise Exception, "{} is a wrong header!".format(header)
            else:
                header_key_list.append(HDRMAP.get(header))
    ...
    for row in result_dict:
        ...
        elif filter_by_headers:
            body_line = ";".join([
                row[header] for header in header_key_list
            ])
        else:
            body_line = ";".join([
                row["id"], row["name"], row["type"], row["state"],
    ...
    if show_header:
        if filter_thresholds:
            ...
            header = filter_by_headers.replace(",", ";")
        else:
            header = ";".join([
                "ID", "Name", "Type", "Status", "Event", "Reading", "Unit",
