...
ROSIE_URL = "https://gitlab.com/rosie-pattern-language/rosie"
...
def git_clone_rosie():
    ...
    if os.path.isdir(ROSIE_DIR):
        ...
        #     print("Unable to obtain code updates from gitlab.  Using existing local code repository.")
    else:
        print("Rosie source code directory not found.  Attempting to clone it now...")
        subprocess.check_call(
            ('git clone --depth 1 -b {} --recurse-submodules {}.git {}'.format(TAG, ROSIE_URL, ROSIE_DIR)),
            shell=True)
        os.chdir(ROSIE_DIR)
...
class custom_install(install):
    def run(self):
        ...
        (1) obtain the full rosie source code from gitlab, if needed;
