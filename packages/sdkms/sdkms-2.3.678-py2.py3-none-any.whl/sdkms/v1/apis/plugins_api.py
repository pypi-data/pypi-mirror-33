...
class PluginsApi(object):
    ...
    def sys_v1_plugins_invoke_plugin_id_get(self, plugin_id, body, async=False, **kwargs):
        """
        Invokes a plugin execution with the provided request body as input to the plugin.
        @type async: bool
        @param async: Whether the call should be performed asynchronously. (Default is False).
        ...
        kwargs['_return_http_data_only'] = True
        if async:
            return self.sys_v1_plugins_invoke_plugin_id_get_with_http_info(plugin_id, body, async=async, **kwargs)
        else:
            (data) = self.sys_v1_plugins_invoke_plugin_id_get_with_http_info(plugin_id, body, async=async, **kwargs)
            return data

    def sys_v1_plugins_invoke_plugin_id_get_with_http_info(self, plugin_id, body, async=False, **kwargs):
        """
        Invokes a plugin execution with the provided request body as input to the plugin.
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async=True::
            >>> thread = api.sys_v1_plugins_invoke_plugin_id_get_with_http_info(plugin_id, body, async=True)
            >>> result = thread.get()
        ...
        # verify the required parameter 'body' is set
        if ('body' not in params) or (params['body'] is None):
            raise ValueError("Missing the required parameter `body` when calling `sys_v1_plugins_invoke_plugin_id_get`")
