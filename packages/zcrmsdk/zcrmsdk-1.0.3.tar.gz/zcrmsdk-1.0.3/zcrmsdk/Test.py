...
class MyClass(object):
    ...
    def api_call(self):
        authtoken = 'c809a0cbb0f6e4acb410c86e3328'
        module_name = "Cases"
        #case_id = request.POST.get("CASEID")
        case_id="1386586000001866216"
        file_handle=open("/Users/sumanth-3058/Downloads/test1.html",'rb')
        file_content=file_handle.read()
        final_URL = "https://crm.zoho.com/crm/private/xml/"+module_name + "/" + "uploadFile?authtoken=" + authtoken + "&scope=crmapi&id=" + case_id
        '''print final_URL
        response = requests.post(final_URL)
        xml_response = response.content
        print xml_response
        '''
        file_content = {'content': open("/Users/sumanth-3058/Downloads/test1.html",'rb')}
        response=requests.post(final_URL, files=file_content,headers={},allow_redirects=False,data={})
    ...
    def create_record(self):
        try:
            records_instances=list()
            record=ZCRMRecord.get_instance('Leads') #module API Name
            record.set_field_value('Last_Name', 'Robert')#field_api_name , field value
            record.set_field_value('Company', 'Zoho')#field_api_name , field value
            user=ZCRMUser.get_instance(1386586000000105001,'User1')
            record.set_field_value('Owner',user)
            records_instances.append(record)
            
            record1=ZCRMRecord.get_instance('Leads') #module API Name
            record1.set_field_value('Last_Name', 'Roy')
            record1.set_field_value('Company', 'Zoho')
            records_instances.append(record1)
            resp=ZCRMModule.get_instance('Leads').create_records(records_instances)
            print resp.code
            print resp.details
            print resp.message
            print resp.status
    ...
    def create_record(self):
        try:
            ...
            record.set_field_value('Last_Name', 'Robert')#field_api_name , field value
            record.set_field_value('Company', 'Zoho')#field_api_name , field value
            #Set the required field values to the record instance
            user=ZCRMUser.get_instance(1386586000000105001,'User1')
            record.set_field_value('Owner',user)
            records_instances.append(record)
    ...
    def create_single_record(self):
        try:
            ...
            if response.status=='success':
                created_record=response.data
                print created_record.entity_id
                print created_record.created_time
                print created_record.created_by.id
        except ZCRMException as ex:
            print ex.status_code
            print ex.error_message
            print ex.error_code
    ...
    def mass_update_records(self):
        try:
            ...
            bulk_resp=module_ins.mass_update_records(entityid_list, 'Status', 'Created')
    ...
    def update_records(self):
        try:
            record_ins_list=list()
            idsArr=['1386586000000569001','1386586000000720883','1386586000000720923'];
            for i in range(0,2):
                record=ZCRMRecord.get_instance('Accounts',idsArr[i])
                record.set_field_value('Employees', 10)
                record.set_field_value('Account_Name', 'IIIT')
                record_ins_list.append(record)
            resp=ZCRMModule.get_instance('Accounts').update_records(record_ins_list)
            print resp.status_code
            entity_responses=resp.bulk_entity_response
            for entity_response in entity_responses:
                print entity_response.details
    ...
    def get_custom_functions(self):
        res=ZCRMCustomFunction.get_instance().get_all_custom_functions('restapi');
        cfInstances=res.data
        for cfIns in cfInstances:
            print cfIns.associated_places
            print cfIns.api_name
            print cfIns.description
            print cfIns.id
            print cfIns.name
            print cfIns.source
            print cfIns.category
            print cfIns.display_name
            print cfIns.config
            print cfIns.restapi_details_for_oauth
            print cfIns.restapi_details_for_zapikey
            print cfIns.return_type
            print cfIns.work_flow
            print cfIns.parameters
            print cfIns.script
            print cfIns.modified_time
            print cfIns.modified_by
    
    def get_custom_function(self):
        res=ZCRMCustomFunction.get_instance().get_custom_function_details('','restapi');
        cfInstances=array.push(res.data)
        for cfIns in cfInstances:
            print cfIns.associated_places
            print cfIns.api_name
            print cfIns.description
...
threading.current_thread().__setattr__('current_user_email','sumanth.chilka@zohocorp.com')
ZCRMRestClient.initialize()

#obj.api_call()
#obj.get_custom_function()
#obj.get_custom_functions()

#obj.test()

#obj.create_record()

obj.create_single_record()
