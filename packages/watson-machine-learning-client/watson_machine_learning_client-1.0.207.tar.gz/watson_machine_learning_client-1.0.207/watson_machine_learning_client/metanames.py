...
class PayloadLoggingMetaNames:
    """
    Set of Meta Names for payload logging system.
    ...
    | MetaName                                         | Type | Required | Example value                                                                            |
    +==================================================+======+==========+==========================================================================================+
    | PayloadLoggingMetaNames.PAYLOAD_DATA_REFERENCE   | json | Y        | ``"{}"``                                                                                 |
    +--------------------------------------------------+------+----------+------------------------------------------------------------------------------------------+
    | PayloadLoggingMetaNames.LABELS                   | list | N        | ``"['a','b','c']"``                                                                      |
    +--------------------------------------------------+------+----------+------------------------------------------------------------------------------------------+
    | LearningSystemMetaNames.OUTPUT_DATA_SCHEMA       | json | N        | ``{"fields": [{"metadata": {}, "type": "string", "name": "GENDER", "nullable": True}]}`` |
    +--------------------------------------------------+------+----------+------------------------------------------------------------------------------------------+
    """

    def __init__(self):
        self.PAYLOAD_DATA_REFERENCE = "payload_store"
        self.LABELS = "labels"
        self.OUTPUT_DATA_SCHEMA = "output_data_schema"

        self._PAYLOAD_DATA_REFERENCE_REQUIRED = True
        self._LABELS_REQUIRED = False
        self._OUTPUT_DATA_SCHEMA_REQUIRED = False

    def _validate(self, meta_props):
        WMLResource._validate_meta_prop(meta_props, self.PAYLOAD_DATA_REFERENCE, dict, self._PAYLOAD_DATA_REFERENCE_REQUIRED)
        WMLResource._validate_meta_prop(meta_props, self.LABELS, list, self._LABELS_REQUIRED)
        WMLResource._validate_meta_prop(meta_props, self.OUTPUT_DATA_SCHEMA, dict, self._OUTPUT_DATA_SCHEMA_REQUIRED)

    def get(self):
        """
        Get available payload logging metanames.

        :return: available payload logging metanames
        :rtype: array
        """
        return sorted(list(filter(lambda x: not x.startswith('_'), self.__dict__.keys())))
    ...
    def show(self):
        ...
        table = tabulate(
            [
                ["META_PROP NAME",                  "TYPE", "REQUIRED"],
                ["PAYLOAD_DATA_REFERENCE",          "json", "Y" if self._PAYLOAD_DATA_REFERENCE_REQUIRED else "N"],
                ["LABELS",                          "list", "Y" if self._LABELS_REQUIRED else "N"],
                ["OUTPUT_DATA_SCHEMA",              "json",  "Y" if self._OUTPUT_DATA_SCHEMA_REQUIRED else "N"]
            ]
        )
        print(table)
    ...
    def get_example_values(self):
        ...
        """
        return {
            self.PAYLOAD_DATA_REFERENCE_DATA_REFERENCE: {},
            self.LABELS: ['a', 'b', 'c'],
            self.OUTPUT_DATA_SCHEMA: "conditionally"
        }
