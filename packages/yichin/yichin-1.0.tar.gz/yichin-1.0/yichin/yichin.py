...
def encode(yichins, days=0, hours=0, minutes=0, seconds=0):
    ...
    else: return yichin_join([yichin_chr(YICHIN_NUMBER-1)] + [YICHIN_STRING[:3]] + [yichin_chr(yichin_int(yichin, 16) + YICHIN_NUMBER) for yichin in yichin_str(yichin_hex(yichin_bytes(yichins, YICHIN_ENCODE)), YICHIN_ENCODE)] + [yichin_chr(YICHIN_NUMBER-1)] + [yichin_chr(yichin_int(yichin, 16) + YICHIN_NUMBER) for yichin in yichin_str(yichin_hex(yichin_bytes((yichin_now(YICHIN_TIMEZONE) + yichin_timedelta(days=days, hours=hours, minutes=minutes)).strftime(YICHIN_FORMAT), YICHIN_ENCODE)), YICHIN_ENCODE)] + [YICHIN_STRING[3:]] + [yichin_chr(YICHIN_NUMBER-1)])

def decode(yichins):
    if yichins[0] == yichin_chr(YICHIN_NUMBER-1) and yichins[-1] == yichin_chr(YICHIN_NUMBER-1):
        if yichin_now(YICHIN_TIMEZONE) <= yichin_time.strptime(yichin_str(yichin_unhex(yichin_join([hex(ord(yichin) - YICHIN_NUMBER)[2:] for yichin in yichin_split(yichin_strip(yichins[1:-1], YICHIN_STRING), yichin_chr(YICHIN_NUMBER-1), -1)])), YICHIN_ENCODE), YICHIN_FORMAT):
            return yichin_str(yichin_unhex(yichin_join([hex(ord(yichin) - YICHIN_NUMBER)[2:] for yichin in yichin_split(yichin_strip(yichins[1:-1], YICHIN_STRING), yichin_chr(YICHIN_NUMBER-1), 0)])), YICHIN_ENCODE)
