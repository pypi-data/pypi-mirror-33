...
class Config(NamedTuple):
    """API configuration (authentication and root URL setup).

    The authentication defaults to variables ALWAYSDATA_API_KEY and
    ALWAYSDATA_ACCOUNT read from the environment.

    Attributes:
        auth: Authentication tuple `(username, password)` although, for
            the API, password should be an empty string and username is
            formed from the following pattern: `API_KEY account=ACCOUNT`.
        root: The API root path. This defaults to
            `https://api.alwaysdata.com/` which should be good for all
            purposes.
    """
    auth: Tuple[str, str]
    root: str = 'https://api.alwaysdata.com/'


try:
    DEFAULT_CONFIG = Config(
        ('{ALWAYSDATA_API_KEY} account={ALWAYSDATA_ACCOUNT}'.format(**environ),
         '')
    )
except KeyError:
    DEFAULT_CONFIG = Config(('', ''))
...
class Resource(SimpleNamespace):
    ...
    """
    def delete(self, config: Config = DEFAULT_CONFIG) -> None:
        """Deletes the resource from the server.

        Args:
            config: Optional API configuration. With environment
                variables the default should be good (see `Config`).
        ...
        Raises:
            ...
            successful.
        """
        self.request('DELETE', self.href, config=config)
    ...
    @classmethod
    def get(cls, id: int, config: Config = DEFAULT_CONFIG) -> 'Resource':
        ...
        response = cls.request('GET', address, config=config)
    ...
    @classmethod
    def list(cls, config: Config = DEFAULT_CONFIG,
        ...
        """
        response = cls.request('GET', cls.href, params=kwargs, config=config)
        result: List[cls] = list(map(cls.from_json, response.json()))
        return result

    def patch(self, config: Config = DEFAULT_CONFIG) -> None:
        ...
        Raises:
            ...
            successful.
        """
        self.request('PATCH', self.href, json=self.__dict__, config=config)

    def post(self, config: Config = DEFAULT_CONFIG) -> None:
    ...
    def put(self, config: Config = DEFAULT_CONFIG) -> None:
        ...
        Raises:
            ...
            successful.
        """
        self.request('PUT', self.href, json=self.__dict__, config=config)

    @classmethod
    def request(cls, method: str, endpoint: str, config: Config,
        ...
        Args:
            ...
            endpoint: The resource path of the request (e.g.
                `/account/123`).
            config: API configuration (see `Config`).
        ...
        url = urljoin(config.root, endpoint)
        response = requests.request(method, url, **kwargs, auth=config.auth)
...
class Domain(Resource):
    ...
    def whois(self, config: Config = DEFAULT_CONFIG) -> str:
        ...
        return self.request('POST', endpoint, config=config).text
...
class Site(Resource):
    ...
    def __init__(self, *args, language_version: str,
                 elixir_version: str, nodejs_version: str,
                 php_version: str, ruby_version: str,
                 python_version: str, redirect_type: str,
    ...
    def restart(self, config: Config = DEFAULT_CONFIG) -> None:
        ...
        self.request('POST', endpoint, config=config)
