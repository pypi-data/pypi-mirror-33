    ...
    def auth_publickey(self, packet):
        ...
        if hasSig:
            ...
            b = NS(self.transport.sessionID) + \
                ...
                NS(self.nextService) + NS(b'publickey') + chr(hasSig) + \
                NS(pubKey.sshType()) + NS(blob)
...
class PublicKeyFromLaunchpadChecker(SSHPublicKeyDatabase):
    ...
    def _checkForAuthorizedKey(self, user_dict, credentials):
        ...
        if credentials.algName == 'ssh-dss':
            ...
            wantKeyType = 'RSA'
        elif credentials.algName.startswith('ecdsa-sha2-'):
            wantKeyType = 'ECDSA'
        elif credentials.algName == 'ssh-ed25519':
            wantKeyType = 'ED25519'
        else:
            # unknown key type
            return False
