...
def upgrade():
    ...
    op.create_table('longitude_permission_roles',
    ...
    op.add_column('longitude_auth_users',
      sa.Column('role_id', sa.Integer(), sa.ForeignKey('longitude_permission_roles.id'), nullable=True, server_default=None)
    )


def downgrade():
    op.drop_column('longitude_auth_users', 'role_id')
    op.drop_table('longitude_permission_roles')
