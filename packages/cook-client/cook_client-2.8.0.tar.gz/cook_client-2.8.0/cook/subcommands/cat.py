...
def cat_for_real(clusters, ref_string, path):
    """TODO(DPO)"""
    guard_no_cluster(clusters)
    entity_refs, clusters_of_interest = parse_entity_refs(clusters, [ref_string])

    assert len(entity_refs) == 1, 'Only a single UUID or URL is supported.'

    command_fn = partial(cat_for_instance, path=path)
...
def cat(clusters, args, _):
    ...
    ref_strings = args.get('target-entity')
    paths = args.get('path')

    # argparse should prevent these, but we'll be defensive anyway
    assert len(ref_strings) == 1, 'Only a single UUID or URL is supported.'
    assert len(paths) == 1, 'Only a single path is supported.'

    cat_for_real(clusters, ref_strings[0], paths[0])
