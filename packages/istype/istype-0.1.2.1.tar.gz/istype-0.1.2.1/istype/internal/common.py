...
def isinstanceof(x, types: tuple, *, typevar_table: typing.Dict[str, type]=None):
    '''
...
def issubclassof(x, types, *, typevar_table: typing.Dict[str, type]=None):
    '''
...
def declare_interface_impl(cls: type, interface: type):
    '''
    declare a type impl a interface.
    '''
    if not isinstance(cls, type):
        raise TypeError
    if not isinstance(interface, type):
        raise TypeError
    table = _SUBCLASSTABLE.get(cls)
    if table is None:
        _SUBCLASSTABLE[cls] = table = set()
    table.add(interface)
