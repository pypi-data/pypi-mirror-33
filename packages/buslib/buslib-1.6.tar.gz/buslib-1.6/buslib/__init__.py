...
class AccountsEndpoint(Endpoint):
    ...
    def global_sign_out(self, id):
        response = self._s.get(url = self._host + '/' + self._endpoint + '/' + id + '/global_sign_out', proxies=self._proxies)
        if response.status_code // 200 != 1:
            _handle_error(response)
...
class Client:
    ...
    def global_sign_out(self):
        response = self._s.get(url = self._host + '/auth/global_sign_out', proxies=self._proxies)
        if response.status_code // 200 != 1:
            _handle_error(response)
