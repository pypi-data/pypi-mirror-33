...
`}static get properties(){return{hass:{type:Object,observer:"_hassChanged"},_config:Object}}constructor(){super(),this._elements=[]}ready(){super.ready(),this._config&&this._buildConfig()}getCardSize(){return 1+(this._config?this._config.entities.length:0)}_showHeaderToggle(e){return!1!==e}_showHeader(e){return e.title||e.show_header_toggle}setConfig(e){this._config=e,this._configEntities=h(e.entities),this.$&&this._buildConfig()}_buildConfig(){const e=this.$.states,t=this._configEntities;for(;e.lastChild;)e.removeChild(e.lastChild);this._elements=[];for(const i of t){const t=i.entity,s=v(i,this.hass);t&&!p.c.includes(Object(u.a)(t))&&(s.classList.add("state-card-dialog"),s.addEventListener("click",()=>this.fire("hass-more-info",{entityId:t}))),this._elements.push({entityId:t,element:s});const a=document.createElement("div");a.appendChild(s),e.appendChild(a)}}_hassChanged(e){for(let t=0;t<this._elements.length;t++){const{entityId:i,element:s}=this._elements[t],a=e.states[i];s.stateObj=a,s.hass=e}}}),customElements.define("hui-entity-filter-card",class extends a.a{static get properties(){return{hass:{type:Object,observer:"_hassChanged"}}}getCardSize(){return this.lastChild?this.lastChild.getCardSize():1}setConfig(e){if(!e.state_filter||!Array.isArray(e.state_filter))throw new Error("Incorrect filter config.");this._config=e,this._configEntities=h(e.entities),this.lastChild&&(this.removeChild(this.lastChild),this._element=null);const t="card"in e?Object.assign({},e.card):{};t.type||(t.type="entities"),t.entities=[];const i=N(t);i._filterRawConfig=t,this._updateCardConfig(i),this._element=i}_hassChanged(){this._updateCardConfig(this._element)}_updateCardConfig(e){if(!e||"HUI-ERROR-CARD"===e.tagName||!this.hass)return;const t=(i=this.hass,s=this._config.state_filter,this._configEntities.filter(e=>{const t=i.states[e.entity];return t&&s.includes(t.state)}));var i,s;0!==t.length||!1!==this._config.show_empty?(this.style.display="block",e.setConfig(Object.assign({},e._filterRawConfig,{entities:t})),e.isPanel=this.isPanel,e.hass=this.hass,this.lastChild||this.appendChild(e)):this.style.display="none"}}),customElements.define("hui-error-card",class extends a.a{static get template(){return s["a"]`
    ...
    `}static get properties(){return{hass:Object,_config:Object,stateHistory:Object,stateHistoryLoading:Boolean}}getCardSize(){return 4}setConfig(e){if(!e.entities||!Array.isArray(e.entities))throw new Error("Error in card configuration.");this._config=e}_computeCacheConfig(e){return{cacheKey:e.entities,hoursToShow:e.hours_to_show||24,refresh:e.refresh_interval||0}}}),customElements.define("hui-horizontal-stack-card",class extends a.a{static get template(){return s["a"]`
      ...
      <div id="root"></div>
    `}static get properties(){return{hass:{type:Object,observer:"_hassChanged"}}}constructor(){super(),this._elements=[]}ready(){super.ready(),this._config&&this._buildConfig()}getCardSize(){let e=1;return this._elements.forEach(t=>{const i=x(t);i>e&&(e=i)}),e}setConfig(e){if(!e||!e.cards||!Array.isArray(e.cards))throw new Error("Card config incorrect.");this._config=e,this.$&&this._buildConfig()}_buildConfig(){const e=this._config;this._elements=[];const t=this.$.root;for(;t.lastChild;)t.removeChild(t.lastChild);const i=[];e.cards.forEach(e=>{const s=N(e);s.hass=this.hass,i.push(s),t.appendChild(s)}),this._elements=i}_hassChanged(e){this._elements.forEach(t=>{t.hass=e})}}),customElements.define("hui-iframe-card",class extends a.a{static get template(){return s["a"]`
      <style>
        ha-card {
          overflow: hidden;
        }
...
`}static get properties(){return{hass:{type:Object,observer:"_hassChanged"},entity:String,image:String,stateImage:Object,cameraImage:String,_error:{type:Boolean,value:!1},_imageClass:String,_imageSrc:String}}static get observers(){return["_configChanged(image, stateImage, cameraImage)"]}connectedCallback(){super.connectedCallback(),this.cameraImage&&(this.timer=setInterval(()=>this._updateCameraImageSrc(),1e4))}disconnectedCallback(){super.disconnectedCallback(),clearInterval(this.timer)}_configChanged(e,t,i){i?this._updateCameraImageSrc():e&&!t&&(this._imageSrc=e)}_onImageError(){this.setProperties({_imageSrc:null,_error:!0})}_onImageLoad(){this._error=!1}_hassChanged(e){if(this.cameraImage||!this.entity)return;const t=e.states[this.entity],i=!function(e,t=[]){return e&&"object"==typeof e&&!Array.isArray(e)&&t.every(t=>t in e)}(t,["state"]);if(!this.stateImage)return void(this._imageClass=i||p.g.includes(t.state)?"state-off":"");const s=i?this.stateImage.unavailable:this.stateImage[t.state];this.setProperties({_imageClass:s||!i&&!p.g.includes(t.state)?"":"state-off",_imageSrc:s||this.image})}_updateCameraImageSrc(){this.hass.connection.sendMessagePromise({type:"camera_thumbnail",entity_id:this.cameraImage}).then(e=>{e.success?this.setProperties({_imageSrc:`data:${e.result.content_type};base64, ${e.result.content}`,_error:!1}):this.setProperties({_imageSrc:null,_error:!0})})}});customElements.define("hui-picture-entity-card",class extends(Object(n.a)(Object(E.a)(a.a))){static get template(){return s["a"]`
    ...
    `}static get properties(){return{hass:Object,_config:Object,_entitiesDialog:{type:Array,computed:"_computeEntitiesDialog(hass, _config, _entitiesService)"},_entitiesService:{type:Array,value:[],computed:"_computeEntitiesService(hass, _config)"}}}getCardSize(){return 3}setConfig(e){if(!e||!e.entities||!Array.isArray(e.entities)||!(e.image||e.camera_image||e.state_image)||e.state_image&&!e.entity)throw new Error("Invalid card configuration");this._config=e}_computeEntitiesDialog(e,t,i){return t.force_dialog?t.entities:t.entities.filter(t=>!i.includes(t)&&t in e.states)}_computeEntitiesService(e,t){return t.force_dialog?[]:t.entities.filter(t=>t in e.states&&!A.includes(Object(u.a)(t))&&Object(d.a)(e,e.states[t]))}_showEntity(e,t){return e in t}_computeIcon(e,t){return Object(L.a)(t[e])}_computeButtonClass(e,t){return p.g.includes(t[e].state)?"":"state-on"}_computeTooltip(e,t){return`${Object(C.a)(t[e])}: ${Object(w.a)(this.localize,t[e])}`}_computeImageClass(e){return e.navigation_path||e.camera_image?"clickable":""}_openDialog(e){this.fire("hass-more-info",{entityId:e.model.item})}_callService(e){const t=e.model.item;z(this.hass,t)}_handleImageClick(){this._config.navigation_path?this.navigate(this._config.navigation_path):this._config.camera_image&&this.fire("hass-more-info",{entityId:this._config.camera_image})}}),i(165),customElements.define("hui-plant-status-card",class extends ${constructor(){super("ha-plant-card","plant")}}),customElements.define("hui-vertical-stack-card",class extends a.a{static get template(){return s["a"]`
      ...
      <div id="root"></div>
    `}static get properties(){return{hass:{type:Object,observer:"_hassChanged"}}}constructor(){super(),this._elements=[]}ready(){super.ready(),this._config&&this._buildConfig()}getCardSize(){let e=0;return this._elements.forEach(t=>{e+=x(t)}),e}setConfig(e){if(!e||!e.cards||!Array.isArray(e.cards))throw new Error("Card config incorrect");this._config=e,this.$&&this._buildConfig()}_buildConfig(){const e=this._config;this._elements=[];const t=this.$.root;for(;t.lastChild;)t.removeChild(t.lastChild);const i=[];e.cards.forEach(e=>{const s=N(e);s.hass=this.hass,i.push(s),t.appendChild(s)}),this._elements=i}_hassChanged(e){this._elements.forEach(t=>{t.hass=e})}}),i(169),customElements.define("hui-weather-forecast-card",class extends ${constructor(){super("ha-weather-card","weather")}getCardSize(){return 4}});const D=["entities","entity-filter","error","glance","history-graph","horizontal-stack","iframe","map","markdown","media-control","picture","picture-elements","picture-entity","picture-glance","plant-status","vertical-stack","weather-forecast"],M="custom:";function H(e,t){const i=document.createElement(e);try{i.setConfig(t)}catch(i){return console.error(e,i),R(i.message,t)}return i}function R(e,t){return H("hui-error-card",f(e,t))}function N(e){let t;if(!e||"object"!=typeof e||!e.type)return R("No card type configured.",e);if(e.type.startsWith(M)){if(t=e.type.substr(M.length),customElements.get(t))return H(t,e);const i=R(`Custom element doesn't exist: ${t}.`,e);return customElements.whenDefined(t).then(()=>Object(l.a)(i,"rebuild-view")),i}return D.includes(e.type)?H(`hui-${e.type}-card`,e):R(`Unknown card type encountered: ${e.type}.`,e)}customElements.define("hui-unused-entities",class extends a.a{static get template(){return s["a"]`
      <style>
        #root {
          max-width: 600px;
          margin: 0 auto;
    ...
    `}static get properties(){return{hass:{type:Object,observer:"_hassChanged"},columns:{type:Number},config:{type:Object}}}static get observers(){return["_configChanged(columns, config)"]}constructor(){super(),this._elements=[],this._debouncedConfigChanged=I(this._configChanged,100)}_configChanged(){const e=this.$.columns,t=this.config;for(;e.lastChild;)e.removeChild(e.lastChild);if(!t)return void(this._elements=[]);const i=t.cards.map(e=>{const t=N(e);return t.hass=this.hass,t});let s=[];const a=[];for(let e=0;e<this.columns;e++)s.push([]),a.push(0);i.forEach(e=>{this.appendChild(e);const t="function"==typeof e.getCardSize?e.getCardSize():1;s[function(e){let t=0;for(let e=0;e<a.length;e++){if(a[e]<5){t=e;break}a[e]<a[t]&&(t=e)}return a[t]+=e,t}(t)].push(e)}),(s=s.filter(e=>e.length>0)).forEach(t=>{const i=document.createElement("div");i.classList.add("column"),t.forEach(e=>i.appendChild(e)),e.appendChild(i)}),this._elements=i,"theme"in t&&Object(P.a)(e,this.hass.themes,t.theme)}_hassChanged(e){for(let t=0;t<this._elements.length;t++)this._elements[t].hass=e}});const B={};customElements.define("hui-root",class extends(Object(r.a)(Object(n.a)(a.a))){static get template(){return s["a"]`
...
//# sourceMappingURL=fc8d0853ad2510f46588.chunk.js.map