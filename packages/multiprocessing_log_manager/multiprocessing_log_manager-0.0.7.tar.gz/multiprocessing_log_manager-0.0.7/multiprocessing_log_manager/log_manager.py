# coding=utf8
"""
日志管理，支持日志打印彩色日志到控制台或写入文件或mongodb
...
或者 logger = LogManager('logger_name').get_without_handlers(),此种不立即记录日志，之后可以在单独统一的总闸处对所有日志根据loggerame进行get_and_add_handlers按需添加handlers就能记录所有日志并记录了
"""
import os
import unittest
import time
...
class LogManager(object):
    ...
    def __add_file_handler(self):
        ...
        if os_name == 'nt':
            # windows下用这个，非进程安全
            rotate_file_handler = ConcurrentRotatingFileHandler(log_file, mode="a", maxBytes=self._log_file_size * 1024 * 1024, backupCount=10,
                                                                encoding="utf-8")
        if os_name == 'posix':
