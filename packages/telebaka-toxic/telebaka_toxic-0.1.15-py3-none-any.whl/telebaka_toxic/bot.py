...
def get_string(rating):
    max_rating = String.objects.filter(max_rating__lte=rating).aggregate(mmr=models.Max('max_rating'))['mmr']
    result = String.objects.filter(max_rating=max_rating).order_by('?').first()
    if result:
        return result.text
    else:
        if rating == -1:
