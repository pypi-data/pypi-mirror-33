...
class Apitax:
    ...
    def apitax(self, args: list):
        ...
        config = ConfigConsumer(getPathToTopLevel('config.txt'))
        config.path = str(Path(os.path.dirname(os.path.abspath(inspect.stack()[0][1]))).resolve())
        #print(config.path)
        #config.path = getPath('config.txt')
        #config.path = str(Path(os.path.dirname(os.path.abspath(inspect.stack()[0][1])) + '/config.txt').resolve())
        #config.path = getPath()
        #print(config.path)
