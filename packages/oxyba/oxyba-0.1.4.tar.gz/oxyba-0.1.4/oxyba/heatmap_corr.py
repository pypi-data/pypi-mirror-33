...
def heatmap_corr(r, pval, varnames=None, dpi=96, cmap='RdBu'):
    ...
    fig, ax = plt.subplots(dpi=dpi, figsize=(num*0.75,num*0.75))
    #fig.tight_layout()
    ...
    plt.xticks(range(num), varnames, rotation=45, ha="left", rotation_mode="anchor");
    ...
    for i in range(num):
        for j in range(num):
            if i is not j:
                #concate the text
                strtxt = ('+' if r[i,j]>0 else ('-' if r[i,j]<0 else '')) \
                ...
                text = ax.text(j, i, strtxt, ha="center", va="center", color=strcol)
