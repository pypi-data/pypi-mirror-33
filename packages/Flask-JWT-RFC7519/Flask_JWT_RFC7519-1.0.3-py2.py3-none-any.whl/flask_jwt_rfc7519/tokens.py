...
from flask_jwt_rfc7519.exceptions import JWTDecodeError, JWTEncodeError, CSRFError
...
def encode_access_token(identity, secret, algorithm, expires_delta, fresh,
    ...
    if additional_claims and not isinstance(additional_claims, dict):
        raise TypeError
    ...
    if additional_claims and isinstance(additional_claims, dict):
        ...
        keys.append('aud')
        keys.append('iss')
        _check_claims(keys, additional_claims)
...
def decode_jwt(encoded_token, secret, algorithm, sub_claim_key,
               additional_claim_keys, csrf_value=None):
    """
