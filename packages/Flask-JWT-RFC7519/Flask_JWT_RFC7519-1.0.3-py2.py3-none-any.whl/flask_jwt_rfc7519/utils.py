...
from flask_jwt_rfc7519.exceptions import (
    ...
    AdditionalClaimsVerificationError, AudClaimVerificationError,
    IssClaimVerificationError
)
from flask_jwt_rfc7519.tokens import decode_jwt
...
def get_jwt_identity():
    ...
    """
    return get_raw_jwt().get(config.sub_claim_key, None)
...
def get_jwt_claims():
    ...
    res = result.copy()
    res.update(dict((k, claims.get(k)) for k in config.additional_claim_keys
                    if k in claims))
    return res

    #return dict((k, claims.get(k)) for k in config.additional_claim_keys
    #                          if k in claims)
...
def _get_jwt_manager():
    try:
        return current_app.extensions['flask-jwt-RFC7519']
    except KeyError:  # pragma: no cover
        raise RuntimeError("You must initialize a JWTManager with this flask "
                           "application before using this method")


def create_access_token(identity, data=None, fresh=False, expires_delta=None):
    """
    ...
    :param data:  The data that will be added to the token, permissions, roles,
                  etc.
    ...
    return jwt_manager._create_access_token(identity, data, fresh, expires_delta)


def create_refresh_token(identity, data=None, expires_delta=None):
    """
    ...
    """
    jwt_manager = _get_jwt_manager()
    return jwt_manager._create_refresh_token(identity, data, expires_delta)
...
def verify_token_claims(jwt_data):
    ...
    aud_claim = jwt_data.get('aud', None)
    if not jwt_manager._aud_claim_verification_callback(aud_claim):
        raise AudClaimVerificationError("Wrong audience")

    iss_claim = jwt_data.get('iss', None)
    if not jwt_manager._iss_claim_verification_callback(iss_claim):
        raise IssClaimVerificationError("Wrong issuer")

    additional_claims = dict((k, jwt_data[k]) for k in config.additional_claim_keys if k in jwt_data)
    if not jwt_manager._claims_verification_callback(additional_claims):
