...
from flask_jwt_rfc7519.exceptions import (
    ...
    AdditionalClaimsVerificationError, IssClaimVerificationError, AudClaimVerificationError
)

from flask_jwt_rfc7519.default_callbacks import (
    ...
    default_claims_callback,
    default_claims_verification_callback,
    default_claims_verification_failed_callback,
    default_aud_verification_callback,
    default_aud_verification_failed_callback,
    default_iss_verification_callback,
    default_iss_verification_failed_callback
)

from flask_jwt_rfc7519.tokens import (
    encode_refresh_token, encode_access_token
)
from flask_jwt_rfc7519.utils import get_jwt_identity
...
class JWTManager(object):
    ...
    def __init__(self, app=None):
        ...
        self._claims_callback = default_claims_callback
        ...
        self._aud_claim_verification_callback = default_aud_verification_callback
        self._aud_claim_verification_failed_callback = default_aud_verification_failed_callback
        self._iss_claim_verification_callback = default_iss_verification_callback
        self._iss_claim_verification_failed_callback = default_iss_verification_failed_callback
    ...
    def init_app(self, app):
        ...
        if not hasattr(app, 'extensions'):   # pragma: no cover
            app.extensions = {}
        app.extensions['flask-jwt-RFC7519'] = self
    ...
    def _set_error_handler_callbacks(self, app):
        ...
        def handle_failed_claims_verification(e):
            return self._claims_verification_failed_callback()

        @app.errorhandler(IssClaimVerificationError)
        def handle_wrong_iss_error(e):
            return self._iss_claim_verification_failed_callback()

        @app.errorhandler(AudClaimVerificationError)
        def handle_wrong_aud_error(e):
            return self._aud_claim_verification_failed_callback()
    ...
    def _set_default_configuration_options(app):
        ...
        app.config.setdefault('JWT_AUD_CLAIM', [])
        app.config.setdefault('JWT_ISS_CLAIM', '')
        app.config.setdefault('JWT_SUB_CLAIM', 'sub')
    ...
    def additional_claims_loader(self, callback):
        ...
        """
        self._claims_callback = callback
        return callback
    ...
    def aud_claim_verification_loader(self, callback):
        self._aud_claim_verification_callback = callback
        return callback

    def aud_claim_verification_failed_loader(self, callback):
        self._aud_claim_verification_failed_callback = callback
        return callback

    def iss_claim_verification_loader(self, callback):
        self._iss_claim_verification_callback = callback
        return callback

    def iss_claim_verification_failed_loader(self, callback):
        self._iss_claim_verification_failed_callback = callback
        return callback


    def _create_refresh_token(self, identity, data, expires_delta=None):
        if expires_delta is None:
            expires_delta = config.refresh_expires

        if config.claims_in_refresh_token:
            additional_claims = self._claims_callback(data)
        else:
            additional_claims = {}
        ...
        refresh_token = encode_refresh_token(
            ...
            sub_claim_key=config.sub_claim_key,
            json_encoder=config.json_encoder
        )
        return refresh_token

    def _create_access_token(self, identity, data, fresh=False, expires_delta=None):
        if expires_delta is None:
