...
from sidecar.azure.azure_clients import AzureClientsManager
from sidecar.azure.azure_status_maintainer import AzureStatusMaintainer
from sidecar.azure.azure_tag_helper import AzureTagHelper
from sidecar.azure.retrying_helpers import retry_if_connection_error
...
class AzureAppInstanceService(IAppInstanceService):
    ...
    def get_all_app_instances(self) -> List[AppInstanceIdentifier]:
        ...
        app_instance_identifiers = [identifier
                                    for vm_resource in vm_resources
                                    for identifier in self._create_app_instance_identifiers_from_vm(vm_resource)]
    ...
    @staticmethod
    def _filter_app_vm_resources(vm_resources: List[Resource]) -> List[Resource]:
        return [vm for vm in vm_resources
                if AzureTagHelper.safely_get_tag(vm, Const.INSTANCELOGICALID) is not None]
