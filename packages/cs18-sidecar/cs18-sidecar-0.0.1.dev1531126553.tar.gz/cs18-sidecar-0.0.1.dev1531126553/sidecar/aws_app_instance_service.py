...
class AwsAppInstanceService(IAppInstanceService):
    ...
    def _get_existing_instance(self, instance_id: str, app_name: str):
        ...
        if instance is None:
            raise Exception(
                "instance for app '{APP_NAME}' not found. instance_id={INSTANCE_ID}".format(APP_NAME=app_name,
    ...
    def _query_instances_by_ids(self, instance_ids):
        return list(self.aws_session.get_ec2_resource().instances.filter(InstanceIds=instance_ids))
