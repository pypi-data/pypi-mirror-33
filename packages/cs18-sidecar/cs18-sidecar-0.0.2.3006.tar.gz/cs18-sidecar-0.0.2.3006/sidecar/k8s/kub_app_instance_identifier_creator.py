...
from sidecar.k8s.kub_api_pod_reader import KubApiPodReader
from sidecar.k8s.kub_api_service import IKubApiService
...
class KubAppInstanceIdentifierCreator(IAppInstanceIdentifierCreator):
    ...
    def create(self, app_name: str, ip_address: str, additional_data: {}) -> AppInstanceIdentifier:
        ...
        pod_json = self.get_pod_json_by_ip_with_retries(ip_address=ip_address)
        return self._create_identifier_for_app_on_pod(app_name, pod_json)

    def get_pod_json_by_ip_with_retries(self, ip_address: str):
        remains = 10
        while True:
            try:
                ...
                if pod_json:
                    ...
                    if remains == 0:
                        ...
                        self._logger.info("pod with address '{ADDRESS}' not found ({REMAINS} remaining attempts)"
                                          .format(ADDRESS=ip_address, REMAINS=remains))
