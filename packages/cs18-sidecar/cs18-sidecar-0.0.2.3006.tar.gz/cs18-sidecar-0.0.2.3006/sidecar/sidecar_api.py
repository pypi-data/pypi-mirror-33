...
from sidecar.aws.aws_app_instance_identifier_creator import AwsAppInstanceIdentifierCreator
from sidecar.aws.aws_app_instance_service import AwsAppInstanceService
from sidecar.aws.aws_sandbox_start_time_updater import AwsSandboxStartTimeUpdater
from sidecar.aws.aws_sandbox_deployment_end_updater import AwsSandboxDeploymentEndUpdater
from sidecar.aws.aws_session import AwsSession
...
from sidecar.k8s.kub_api_service import KubApiService
from sidecar.k8s.kub_app_instance_identifier_creator import KubAppInstanceIdentifierCreator
from sidecar.k8s.kub_app_instance_service import KubAppInstanceService
from sidecar.k8s.kub_sandbox_start_time_updater import KubSandboxStartTimeUpdater
from sidecar.k8s.kub_sandbox_deployment_end_updater import KubSandboxDeploymentEndUpdater
from sidecar.k8s.kub_token_provider import KubTokenProvider
...
from sidecar.aws.aws_status_maintainer import AWSStatusMaintainer
...
def get_status_updater(config_json, app_instance_event_handler: AppInstanceEventHandler, aws_session: AwsSession):
    ...
    if provider == "kubernetes":
        ...
        kas = KubApiService(hostname=hostname, namespace=sandbox_id, kub_token_provider=kub_token_provider, logger=logger)
        ...
        app_instance_identifier_creator = KubAppInstanceIdentifierCreator(kub_api_service=kas, logger=logger)
    ...
    if provider == 'azure':
        return AzureSidecarApiInitializer(sandbox_id=sandbox_id,
                                          space_id=space_id,
                                          app_requests=app_requests,
                                          messaging_service=messaging_service,
                                          app_instance_status_event_reporter=app_instance_status_event_reporter,
...
def run():
    ...
    try:
        ...
        default_health_check_ports_to_test = _get_apps_ports_to_test(config_json)
        ...
        _health_check_monitor = HealthCheckMonitor(logger=logger,
                                                   ...
                                                   default_health_check_ports_to_test=default_health_check_ports_to_test,
                                                   apps_timeout=apps_timeouts,
                                                   apps_env=apps_env)
    except Exception as exc:
...
def configure_debug():
    ...
    config_file_content = {
        ...
        "provider": "aws",
        "sandbox_id": "nefmwtza4r121",  # can set it to an existing sandbox
