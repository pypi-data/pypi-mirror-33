...
from sidecar.aws.aws_tag_helper import AwsTagHelper
...
class AwsAppInstanceService(IAppInstanceService):
    ...
    def _get_existing_instance(self, instance_id: str, app_name: str):
        ...
        if instance is None:
            raise Exception("instance for app '{APP_NAME}' not found. instance_id={INSTANCE_ID}".format(APP_NAME=app_name, INSTANCE_ID=instance_id))
        return instance
    ...
    def get_all_app_instances_statuses(self) -> Dict[AppInstanceIdentifier, str]:
        ...
        instances = self._get_sandbox_instances()
        for instance in instances:
            app_to_status_map = AwsTagHelper.get_apps_status_map(instance=instance, logger=self._logger)
            for app_instance_identifier in self._create_app_instance_identifiers(instance):
        ...
        while instance is None:
            ...
            if instance is None:
                if time.time() - start_time >= 60:
                    message = 'Timeout: Waiting for instance with ip {}'.format(private_ip_address)
    ...
    def _get_sandbox_instances(self) -> list():
        instances = self._query_instances_in_sandbox()
        self._logger.info("[{TIMESTAMP} utc] all sandbox instances are: {ids}"
        ...
        self._logger.info("app instances are: {}".format(", ".join(inst.instance_id for inst in instances)))
        self._filter_out_autoscaling_instances_not_in_asg(instances)
        self._logger.info("filtered app instances are: {}".format(", ".join(inst.instance_id for inst in instances)))
        return instances

    def _query_instances_in_sandbox(self):
