...
from kooki.config import read_config_file, parse_document_rules
...
class BakeCommand(Command):
    ...
    def __init__(self):
        ...
        self.add_argument('-v', '--verbose', help='Verbose', action='store_true')

    def callback(self, args):
        try:
            pretty_output.set_output_policy(args.verbose)
...
def collect_jars(jars):
    all_jars = set(jars)
    for jar in jars:
        jars = jar_dependencies(jar)
        all_jars.update(collect_jars(jars))
    return all_jars


def execute_update(document):
    pretty_output.title_2('unfreeze')
    jars = collect_jars(document.jars)
