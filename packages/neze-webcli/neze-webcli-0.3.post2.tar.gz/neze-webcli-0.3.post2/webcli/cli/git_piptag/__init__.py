...
group.add_argument('-s','--set',action='store_const',dest='run',const=_piptag_set)

def _piptag_dev(argv):
...
group.add_argument('-d','--dev',action='store_const',dest='run',const=_piptag_dev)

def _piptag_get(argv):
...
group.add_argument('-g','--get',action='store_const',dest='run',const=_piptag_get)

@cli.mainrun
def _piptag_root(argv):
    if argv.force is not None:
        _piptag_set(argv)
        cli.exit(0)
    head,tag = _get_current_version()
...
group.add_argument('-r','--root',action='store_const',dest='run',const=_piptag_root)
