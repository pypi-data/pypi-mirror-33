...
class JsonFile(File):
    def _dump(self,obj,stream=None,separators=(',',': '),indent=2,newline=True):
        with stream or self.open('w') as cfgfile:
            _json().dump(obj,cfgfile,separators=separators,indent=indent)
            if newline:
                cfgfile.write('\n')
    def _load(self,obj=None,stream=None):
