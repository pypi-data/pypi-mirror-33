...
#This script is where the majority of the batch processing code resides. It's meant to loop through a batch.txt file and perform
#the necessary processing steps based on what's in the batch file.
def run(batch_file):
    #First, the batch.txt file is parsed and input into a batch_list variable
    batch_list = list()
    ...
    #Once we have a batch_list variable fully populated, we loop through it and detect any 'False' booleans and perform necessary processing steps
    for i in range(len(batch_list)):
        if 'False' in batch_list[i]:
            ...
            if copy == 'False':
                ...
                #Copy rawdata into output and rename files 
            ...
            #Normalization/QC module, if false perform normalization and QC depending on array type
            if norm_qc == 'False':
...
#This function updates the input batch file based on the batch_list, its meant to be run during batch processing
