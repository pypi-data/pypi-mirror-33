...
class WorkflowInstance(TimeStampedModel):
    ...
    def current_status(self):
        ...
        # sometimes there is no running task due to other related rokflows's status
        # we need to find the last completed task instead
        if not r:
            sorted_ids = reversed(self.topological_sort())
            for i in sorted_ids:
                x = self.task_instances.filter(status=complete, pk=i)
                if x:
                    r = x
                    break
        return r
