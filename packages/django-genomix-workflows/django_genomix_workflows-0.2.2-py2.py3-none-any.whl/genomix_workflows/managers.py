...
class TaskManager(Manager):
    ...
    def combined_next_task_instances(self, obj):
        ...
        for next in obj.task.next_tasks.all():
            # check if a task instance exists in the wf instance
            if combined_task_instances.filter(task__id=next.id):
                pi = combined_task_instances.filter(task__id=next.id).first()
    ...
    def combined_previous_is_complete(self, obj):
        """
        Check combined previous task instances to see if they are all completed
        """
        result = True
