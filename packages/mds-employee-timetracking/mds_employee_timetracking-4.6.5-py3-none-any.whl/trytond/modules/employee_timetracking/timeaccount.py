...
class TimeAccountItem(Workflow, ModelSQL, ModelView):
    ...
    def __setup__(cls):
        ...
        cls._transitions |= set((
            ...
            (WF_ACCOUNT_LOCK, WF_ACCOUNT_EXAMINE),
            ))
        cls._error_messages.update({
            'delete_accitem': (u"The time account item is in the 'locked' state and can not be deleted."),
        ...
        for items, values in zip(actions, actions):
            for i in items:
                if ('state' in values.keys()) and (len(values.keys()) == 1):
                    # edit of 'state' is allowed, even if evaluatin is locked
                    pass
