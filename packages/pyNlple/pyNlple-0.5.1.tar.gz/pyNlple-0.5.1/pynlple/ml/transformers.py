...
class ApplyToPandas(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        kwds = self.kwds if self.kwds else None
        if self.axis:
            return X.apply(func=self.func, axis=self.axis, args=self.args, **self.kwds if self.kwds else None)
        else:
            return X.apply(func=self.func, args=self.args, **self.kwds if self.kwds else None)
