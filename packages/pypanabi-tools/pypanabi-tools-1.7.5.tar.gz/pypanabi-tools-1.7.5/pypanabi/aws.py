...
class Redshift(object):
    ...
    def add_partition(self, schema, table, partitions, with_partition_name=True):
        ...
        if len(result) == 1:
            ...
            partition = '/'.join([result[0]["path"]] + l_values + [''])
            query = """
                        ALTER TABLE {}.{} 
    ...
    def execute(self, query):
        ...
        try:
            ...
            if token in ['UPDATE', 'DELETE', 'INSERT', 'SELECT', 'WITH']:
                ...
                if token in ['SELECT', 'WITH', 'ALTER']:
                    try:
                        print "ADD PARTITION" in query
                        print "ADD IF NOT EXISTS PARTITION" in query
                        ...
                        if "ADD PARTITION" in query or "ADD IF NOT EXISTS PARTITION" in query:
                            ...
                            print table
                            print partition
                            print [t["partition"] for t in self.get_partitions(schema, table)]
