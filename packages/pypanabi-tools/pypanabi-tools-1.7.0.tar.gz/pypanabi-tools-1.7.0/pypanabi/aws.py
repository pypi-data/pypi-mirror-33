...
class Redshift(object):
    ...
    def get_external_tables(self, schema=None, table=None):
        ...
        l_tables = list()
        for row in result['data']:
            l_tables.append({"table": row.table, "path": row.path})
        return l_tables

    def is_external(self, schema, table):
        return (schema + '.' + table) in [t["table"] for t in self.get_external_tables()]

    def add_partition(self, schema, table, with_partition_name=True, **kwargs):
        rc = 0
        ...
        if len(result) == 1:
            ...
            for p in kwargs:
                if with_partition_name:
                    l_partitions.append("{}={}".format(p, kwargs[p]))
                else:
                    l_partitions.append(kwargs[p])

                print result[0]
                print l_partitions

                partition = '/'.join(result[0]["path"] + l_partitions + [''])
                query = """
                            ALTER TABLE {}.{} 
    ...
    def execute(self, query):
        ...
        try:
            ...
            if token in ['UPDATE', 'DELETE', 'INSERT', 'SELECT', 'WITH']:
                ...
                if token in ['SELECT', 'WITH', 'ALTER']:
                    try:
                        if "ADD PARTITION" in query or "ADD IF NOT EXISTS PARTITION" in query:
