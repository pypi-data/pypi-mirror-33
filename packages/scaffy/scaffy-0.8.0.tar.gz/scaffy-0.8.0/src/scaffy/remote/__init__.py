...
class Client(object):
    ...
    def __init__(self, backend_url=None):
        backend_url = backend_url or config.DEFAULT_BACKEND
    ...
    def token(self):
        ...
        if self._token is None:
            if exists(config.CREDENTIALS_PATH):
                with open(config.CREDENTIALS_PATH) as fp:
                    self._token = fp.read()
    ...
    def token(self, token):
        ...
        with open(config.CREDENTIALS_PATH, 'w') as fp:
            fp.write(token)
