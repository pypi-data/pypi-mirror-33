...
from RouteInfoGenerator import RouteInfoGenerator
...
class Router:
    ...
    @classmethod
    def rule(cls, url, public=True):
        # decorator
        def wrapper(handler):
            route_info = RouteInfoGenerator.generate_route_info(url, handler)
            debug("add rule url:{url},route_info{r}".format(url=url, r=route_info))
            cls.add_route(url, handler, public, route_info)
            return handler

        return wrapper

    @classmethod
    def add_route(cls, rule, handler, public, route_info):
        '''加入路由表
    ...
    def run_daemon(self, argv, try_time=2):
        ...
        try:
            ...
            if try_time > 0:
                ClientDaemon.daemon_stop('sft_daemon')
                ClientDaemon.daemon_run('sft_daemon', RpycServer.run_service)
                time.sleep(1)
    ...
    def real_call(self, is_exit=True):
        ...
        if route is None and self.url != '/':
            ...
            # 入口打印日志
            debug(
                "URL:%s,Handler:%s.%s,Args:%s" %
                (self.url if not re.match(
                    r'^json.*',
                    self.url) else "json",
                 handler.__module__ or None,
                 handler.__name__ or None,
                 self.params))
