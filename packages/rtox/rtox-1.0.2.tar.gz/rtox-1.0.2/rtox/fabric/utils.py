...
def error(message, func=None, exception=None, stdout=None, stderr=None):
    ...
    if rtox.fabric.state.output.exceptions or rtox.fabric.state.output.debug:
    ...
    if func is abort:
        if stdout and not rtox.fabric.state.output.stdout:
            message += _format_error_output("Standard output", stdout)
        if stderr and not rtox.fabric.state.output.stderr:
