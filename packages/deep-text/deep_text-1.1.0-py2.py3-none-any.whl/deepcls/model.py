...
from deepcls.default_custom import DefaultCNNTransform
from deepcls.default_custom import DefaultRNNTransform
...
class GraphTextCNNModel(DeepCLSModel):
    def __init__(self, model_path, config, TransformClass=None):
        if TransformClass is None:
            super(GraphTextCNNModel, self).__init__(tf.Session(graph=utils.load_graph_file(model_path)), DefaultCNNTransform(config), config)
        else:
