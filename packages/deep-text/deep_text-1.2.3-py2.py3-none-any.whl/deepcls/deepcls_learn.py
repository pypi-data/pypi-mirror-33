...
from .default_custom import DefaultCNNTransform
from .default_custom import DefaultRNNTransform
import sys
import getopt
...
def main():
    ...
    if model_type == "cnn":
        config = utils.load_deep_cls_cnn_config(config_file_path, train_input_path, test_input_path)
        trainer = TextCNNTrainer(config, DefaultCNNTransform, need_transform, need_build_word2vec)
        model = trainer.train()
    elif model_type == "rnn":
        config = utils.load_deep_cls_rnn_config(config_file_path, train_input_path, test_input_path)
        trainer = TextRNNTrainer(config, DefaultRNNTransform, need_transform, need_build_word2vec)
        model = trainer.train()
    else:
