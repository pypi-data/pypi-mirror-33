...
class Command(object):
    ...
    def _parse_yaml_filehandle(self, fh):
        ...
        for section in sections:
            ...
            for ro in rlist:
                if 'name' not in ro:
                    raise ValueError('"name" is not defined for resource '
                        'entry #{}'.format(len(resources)))
                if 'type' not in ro:
                    raise ValueError('"type" is not defined for resource '
    ...
    def main_go(self, *args, **kwargs):
        try:
            ...
            if type(r) == int:
                ...
                raise RuntimeError('unexpected command return type {}'.format(
                    type(r)))

        except AuthError as e:
            self.logger.error('Error: {}'.format(str(e)))
            self.logger.exception(e)
            self.eprint(0, 'Hint: Use "rock configure" to update your '
                'credentials.')
            ret = 126
            if self.verbose:
                raise
        except (HTTPError, InputError, LimitReached,
            NotImplementedError, NotYetImplemented,
            RequestTimeout, ResourceSuspendedError, ServerError,
            TransientServerError) as e:
    ...
    def print_list_json(self, level, objs):
        for o in objs:
            self.lprint(level, json.dumps(o, indent=1, separators=(',', ':'),
                sort_keys=True))
        return len(objs)
    ...
    def print_list_text(self, level, objs, field_order, header):
        ...
        if header == True:
            table.add_row([f.upper() for f in fields])
