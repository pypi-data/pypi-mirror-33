...
_linode_cli()
{
    ...
    case "${prev}" in
        linode-cli)
            COMPREPLY=( $(compgen -W "account events users domains default images linodes kernels stackscripts longview managed networking nodebalancers profile regions tickets volumes --help" -- ${cur}) )
            return 0
            ;;
        account)
        ...
        nodebalancers)
            ...
            ;;
        profile)
            COMPREPLY=( $(compgen -W "view update apps-list app-view app-delete tfa-disable tfa-enable tfa-confirm trusted-computers-list tfa-trusted-computers-view untrust-tfa-trusted-computer tokens-list token-create token-view token-update token-delete --help" -- ${cur}) )
            return 0
            ;;
        regions)
            COMPREPLY=( $(compgen -W "list view --help" -- ${cur}) )
