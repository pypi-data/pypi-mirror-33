...
class GenDataForm(QtWidgets.QMainWindow):
    def __init__(self
        ...
        self.neglected_args    = ['limit','constraints','check_func'] #Args that user can specify
    ...
    def _connect_signals(self)->None:
        self.gendata_combobox.currentIndexChanged.connect(self._update_input_table)
        self.push_submit.clicked.connect(self._submit)

    def _update_input_table(self)->None: # type: ignore
        current_function_name = self.gendata_combobox.currentText()
        gendata_function = self.gendata_functions[current_function_name]
        arg_dict = self.get_arg_dict(gendata_function)
    ...
    def _submit(self)-> None:
        ...
        gendata_kwargs.update({'constraints':constraints,'limit':limit})
        print(gendata_kwargs)
        # gendata_obj = self.gendata_functions[self.gendata_combobox.currentText()](**gendata_kwargs)
        # submit_kwargs = self._get_cluster_info()
        # gendata_obj.submit(**submit_kwargs)
    ...
    def setupUi(self) -> None:
        ...
        self.gendata_combobox = QtWidgets.QComboBox(self.push_box)
        self.gendata_combobox.addItems(list(self.gendata_functions.keys()))
        self.gendata_combobox.setObjectName("gendata_combobox")

        self.push_box_layout.setWidget(0, QtWidgets.QFormLayout.FieldRole, self.gendata_combobox)
