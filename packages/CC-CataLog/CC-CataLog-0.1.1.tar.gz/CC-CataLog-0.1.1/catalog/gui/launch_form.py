...
class LaunchForm(QtWidgets.QDialog):
    ...
    def launch(self) -> None:
        ...
        if clust_str == 'Fizzle':
            [inc.fizzle(fwid,force_cancel = True) for fwid in self.fwid_list] # type: ignore
            return self.accept()
        ...
        for fwid in self.fwid_list:
            ...
            if not fworker == to_cluster.name:
                inc.switchCluster(fwid, from_cluster_str = fworker, to_cluster_str = clust_str)
    ...
    def launch_rockets(self, clusters_to_launch : typ.List[str])-> None:
        if 'sherlock2' in clusters_to_launch:
            if not get_cluster() == 'sherlock':
                ssh_command = 'ssh {}@login.sherlock.stanford.edu'.format(SHERLOCK2_USERNAME)
                command ='bash /scratch/users/{}/CataLog/catalog/fw/sherlock_launcher.sh'.format(SHERLOCK2_USERNAME)
                p = subprocess.Popen(ssh_command.split()+command.split(),stdout=subprocess.PIPE)
                out, _ = p.communicate()
        ...
        if 'suncat' in clusters_to_launch:
            ssh_command = 'ssh {}@suncatls1.slac.stanford.edu'.format(SHERLOCK2_USERNAME)
            command ='bash /u/if/{}/CataLog/catalog/fw/suncat_launcher.sh'.format(SHERLOCK2_USERNAME)
            p = subprocess.Popen(ssh_command.split()+command.split(),stdout=subprocess.PIPE)
            out, _ = p.communicate()
            logging.info(out.decode())
