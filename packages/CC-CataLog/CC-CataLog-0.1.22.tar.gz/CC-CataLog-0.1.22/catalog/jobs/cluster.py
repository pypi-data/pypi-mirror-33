...
#Internal Imports
from catalog.catalog_config import USER, SUNCAT_USERNAME
...
def sherlock2Q(timeInHours  : int = 1
    ...
    return {'_fw_name'       : 'CommonAdapter'
           ,'_fw_q_type'     : 'SLURM'
           ,'rocket_launch'  : 'rlaunch -c $CATALOG_LOC/FW_CONFIG_DIR singleshot'
           ,'nodes'          : nodes
           ,'ntasks_per_core': 1
           ,'mem'            : 64000
...
def suncat2Q(timeInHours : int
    ...
    return {'_fw_name':       'CommonAdapter'
            ...
            ,'walltime':      printTime(timeInHours)
            ,'queue':         'suncat2'
            ,'pre_rocket':    'source /nfs/slac/g/suncatfs/sw/py2.7.13/env.bash'
            ,'logdir':        '$CATALOG_LOC/logs'
            }
...
sherlock2_CATALOG_LOC = subprocess.check_output(['ssh', '-XY','{}@login.sherlock.stanford.edu'.format(USER),'echo',"$CATALOG_LOC"]).decode().strip('\n')
sherlock2_launchdir   = os.path.join(sherlock2_CATALOG_LOC,'fireworks','jobs')

suncat_CATALOG_LOC    = subprocess.check_output(['ssh', '-XY','{}@suncatls1.slac.stanford.edu'.format(USER),'echo',"$CATALOG_LOC"]).decode().strip('\n')
suncat_launchdir      = os.path.join(suncat_CATALOG_LOC,'fireworks','jobs')
###########################################################################
###########################################################################


sherlock2        = Cluster('sherlock2',  'sherlock2',   sherlock2_launchdir,       sherlock2Q,'SLURM')
sherlock2gpaw    = Cluster('sherlock2',  'sherlock2',   sherlock2_launchdir,       sherlock2QGPAW,'SLURM')


suncat          = Cluster('suncat',     'suncat',      suncat_launchdir, suncatQ, 'LoadSharingFacility')
suncat2         = Cluster('suncat2',    'suncat',      suncat_launchdir, suncat2Q,'LoadSharingFacility')
