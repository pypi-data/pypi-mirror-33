...
class SeriesSet(object):
    ...
    def __init_frame_matrix(self, series, miss_symbol):
        ...
        if isinstance(series, Frame):
            ...
            if not self._columns:
                self._columns = ['Col_%d'%i for i in range(self._dim.Col)]
            ...
            for j, sequence in enumerate(zip(*series)):
                for i, v in enumerate(sequence):
                    if v == miss_symbol or v == self._miss_value:
                        ...
                        self._m_value[j] += 1
                self._set[self._columns[j]] = sequence      
            return
    ...
    def extend(self, item):
        ...
        elif isinstance(item, Frame) or\
             all(filter(is_iter, item)):
                ...
                if i >= first_line:
                    for j, item in enumerate(lines):
                        if j in _col_types or item == miss_symbol or item == self._miss_value:
                            continue
                        _col_types[j] = _trans_fun_set[type(str2value(item))]
    ...
    def sort(self, *orders): # OK #
        ...
        for k, title in enumerate(self._columns):
            sequence = self._set[title]
            self._set[title] = [sequence[i] for i in new_index]
            self._type_column[k] = list
...
class Frame(object):
    ...
    @property
    def info(self):
        new_m_v = [str(t) for t in self._m_value]
    ...
    def __getslice__(self, i, j):
        if i in self._columns or j in self._columns:
            ...
            return Frame(new_frame, self._columns[i:j + 1],
                         self._type_value[i:j + 1],
                         self._miss_value)
    ...
    def pop_miss_value(self, axis='LINE'):
        ...
        if axis.upper() == 'COL':
            ...
            new._columns = [self._columns.pop(i) for i in reversed(save_col)]
            if len(self._columns) == 0:
                self._dim = Frame.dims(0, 0)
                self._Frame = list()
                return
            
            save_col = reversed(save_col)
            new_frame = [0] * self._dim.Ln
            for i, record in enumerate(self._Frame):
                new_frame[i] = [record.pop(i) for i in save_col]
            for record in new_frame:
               record.reverse() 
            self._dim = Frame.dims(self._dim.Ln, len(self._columns))
            return Frame(new_frame, new._columns)
    ...
    def read_text(self, addr, first_line=1, miss_symbol='NA',
        ...
        t5 = time.clock()
        if not self._columns:
            self._columns = ['Col_%d'%i for i in range(col)]
        ...
        print 'scan the data: %.2f'%(t2 - t1)
        print 'transform the types: %.2f'%(t3-t2)
        print 'jump out some lines: %.2f'%(t4-t3)
        print 'get data: %.2f'%(t5-t4)
        print 'check columns name: %.2f'%(time.clock() - t5)
        print 'total:%.2f'%(t5 - t1)
