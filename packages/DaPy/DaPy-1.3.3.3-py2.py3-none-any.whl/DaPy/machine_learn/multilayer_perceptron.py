...
class MLP(object):
    ...
    def train(self, data, target, train_time=5000, info=True, plot=True):
        ...
        if plot:
            try:
                ...
                warnings.warn('DaPy uses `matplotlib` library for draw picture.')
    ...
    def __add_bias(self, data):
        if self._numpy:
            append_list = self._f.array([[1] * len(data)])
            return self._f.c_[data, append_list.T]
        else:
            _data = Frame(deepcopy(data), None)
            _data.append_col('', [1] * len(_data))
            return Matrix(_data)
