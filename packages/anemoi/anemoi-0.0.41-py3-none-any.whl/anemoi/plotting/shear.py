...
def annual_mast_results(mast_shear_results, lower_shear_bound=0.1, upper_shear_bound=0.3):
    ...
    '''
    assert mast_alpha_matrix.index.nlevels == 3, "Expecting three index levels for plotting ['orient','height','sensor']"

    orients = mast_alpha_matrix.index.get_level_values(level='orient').unique().tolist()
    mast_alpha_matrix.columns.name = ''
    stacked_shear_results = mast_alpha_matrix.stack().to_frame('alpha')
