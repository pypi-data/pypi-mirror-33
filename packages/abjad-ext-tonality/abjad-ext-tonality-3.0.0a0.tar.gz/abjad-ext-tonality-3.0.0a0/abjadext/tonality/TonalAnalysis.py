...
class TonalAnalysis(system.AbjadObject):
    ...
    def _analyze_chord(argument):
        ...
        for x in range(len(ordered_npcs)):
            ...
            return None
        root = ordered_npcs[0]
        class_ = tonality.RootlessChordClass
        ...
        return tonality.RootedChordClass(
            root,
    ...
    def _analyze_incomplete_tonal_function(argument, key_signature):
        import abjad
        from abjadext import tonality
        if isinstance(argument, tonality.RootedChordClass):
        ...
        scale = tonality.Scale(key_signature)
