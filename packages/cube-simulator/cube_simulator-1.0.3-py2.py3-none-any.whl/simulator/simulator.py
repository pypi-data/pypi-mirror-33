...
def main(transaction_datetime, basket_items, setup_flag=1):
    result = []

    if setup_flag:
        dateutil = setup_for_date(transaction_datetime)
    else:
...
def get(mode, transaction_date, scheme_id):
    ...
    if mode == 1:
        date_str = (transaction_date + timedelta(hours=5, minutes=30)).strftime("%Y-%m-%d %H:%M:%S").split(" ")
        event_list, order_dates = main(date_str, "1000", scheme_id)[0]
        return order_dates[scheme_id] - timedelta(hours=5, minutes=30)

    if mode == 2:
        date_str = (transaction_date + timedelta(hours=5, minutes=30)).strftime("%Y-%m-%d %H:%M:%S").split(" ")
        event_list, order_dates = main(date_str, "0010", scheme_id)[0]
        return order_dates['neft'] - timedelta(hours=5, minutes=30)

    if mode == 3:
        date_str = (transaction_date + timedelta(hours=5, minutes=30)).strftime("%Y-%m-%d %H:%M:%S").split(" ")
        event_list, order_dates = main(date_str, "0100", scheme_id)[0]
        return order_dates['bill'] - timedelta(hours=5, minutes=30)

    if mode == 4:
        date_str = (transaction_date + timedelta(hours=5, minutes=30)).strftime("%Y-%m-%d %H:%M:%S").split(" ")
        event_list, order_dates = main(date_str, "0001", scheme_id)[0]
        return order_dates['mfr'] - timedelta(hours=5, minutes=30)
