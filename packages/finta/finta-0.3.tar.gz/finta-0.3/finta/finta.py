...
class TA:
    ...
    @classmethod
    def EVWMA(cls, ohlc, period=20):
        '''
        EVWMA is a statistical measure using the volume to define the period of the moving average.
        The eVWMA can be looked at as an approximation to the average price paid per share.
        :source: http://christian-fries.de/evwma/PDF/evwma_intro.pdf
        '''
        '''
        ...
        N = ohlc['volume'].rolling(window=period).sum()
        roll = (N - ohlc['volume']).rolling(window=1).apply(func=lambda x: x * x)

        evwma = pd.Series(roll + (ohlc['volume'] * ohlc['close']) / N)
