...
class TestGeoImageVrtStr(unittest.TestCase):
    """Testing for :
    GeoImage class with a vrt string
    """
    # Run defined method setUp to setup the test environment
    def setUp(self):
        # Create a temporary vrt file on disk
        with tempfile.NamedTemporaryFile(suffix='.vrt') as tmp_file:

            tt.cmd_line.exec_cmd(['gdalbuildvrt',tmp_file.name,
                                        dgsamples.bayou_chip.extract_test])
            # Read VRT string to a python var
            self.vrt_string = tmp_file.read().decode()

        self.img = geoio.GeoImage(self.vrt_string)

        self.bad_vrt_string = self.vrt_string.replace(
                                dgsamples.bayou_chip.extract_test,'dummy.tif')

    def test_VRT_string_meta_exists(self):
        self.assertIsInstance(self.img.meta,tt.bunch.OrderedBunch)

    def test_VRT_string_getdata(self):
        data = self.img.get_data()
        self.assertTrue(np.all(data.shape == np.array([8, 174, 246])))

    def test_VRT_string_raise_on_bad_vrt(self):
        with self.assertRaises(ValueError):
            geoio.GeoImage(self.bad_vrt_string)

class TestGeoImageGdalObj(unittest.TestCase):
    """Testing for :
    GeoImage class instantiated with a gdal obj
    """
    # Run defined method setUp to setup the test environment
    def setUp(self):
        # Create objects from files to pull the underlying gdal objects.
        self._img1 = geoio.GeoImage(dgsamples.bayou_chip.extract_test)
        self._img2 = geoio.GeoImage(dgsamples.wv2_longmont_1k.ms)

        # Create GeoImage object from the hidden gdal objects from above.
        self.img1 = geoio.GeoImage(self._img1.ds)
        self.img2 = geoio.GeoImage(self._img2.ds)

    def test_meta_exists1(self):
        self.assertIsInstance(self.img1.meta,tt.bunch.OrderedBunch)

    def test_meta_exists2(self):
        self.assertIsInstance(self.img1.meta,tt.bunch.OrderedBunch)

    def test_getdata1(self):
        data = self.img1.get_data()
        self.assertTrue(np.all(data.shape == np.array([8, 174, 246])))

    def test_getdata2(self):
        data = self.img2.get_data()
        self.assertTrue(np.all(data.shape == np.array([8, 501, 500])))
...
class TestGeoImage_getdata(unittest.TestCase):
    ...
    def test_get_data_resolution_down(self):
        d = self.img.get_data(resolution=4.0)
        self.assertEqual(d.shape,(8,251,250))

    def test_get_data_resolution_up(self):
        d = self.img.get_data(resolution=1.0)
        self.assertEqual(d.shape,(8,1002,1000))
...
class TestGeoImage_iter_vector(unittest.TestCase):
    ...
    def test_get_data_propListBad(self):
        with self.assertRaises(ValueError):
            list(self.img.iter_vector(vector=self.vec,
...
class TestDGImage(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.img = geoio.DGImage(self.test_img)
    ...
    @unittest.skip("No need for IDL anymore")
    def test_DGImage_idl_available(self):
        self.assertTrue(tt.cmd_line.is_tool('idl'))

    @unittest.skip("No need for ENVI anymore")
