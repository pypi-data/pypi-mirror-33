...
class NoValidBandRequested(ValueError):
    '''Raise when the window does not overlap the image.  This can be
    caught and passed when the window is expected to not overlap in
    some cases.
    '''
...
class NotValidGeom(ValueError):
    '''Raised when instantiate_geom method fails to create a geom.
    '''
    pass
...
class GeoImage(object):
    ...
    def __init__(self, file_in, derived_dir=None):
        ...
        if isinstance(file_in, gdal.Dataset):
            ifile = ''
            fdir = ''
        ...
        if isinstance(file_in, gdal.Dataset):
            # Get files and tiles info from the object
            self.ds = file_in
            if file_in.GetDriver().GetDescription() == 'MEM':
                self.files.dfile = 'Using MEM driver'
                self.files.dfile_tiles = ''
            else:
                self.files.dfile = file_in.GetFileList()
                self.files.dfile_tiles = file_in.GetDescription()
                self.files.dfile_tiles = [x for x in self.files.dfile_tiles
                                    if not tt.files.filter(x,
                                    self.files.dfile, case_sensitive=False)]
        else:
            # If a regular file...
            (tmpfile,tmptiles)=self._get_file_and_tiles(file_in)
            ...
            self.ds = self._get_gdal_obj(self.files.dfile,
                                         self.files.dfile_tiles)

            if self.ds.GetFileList() is None:
                raise ValueError("There are no files associated with the input.  "
                                 "This can catch when an invalid VRT string is "
                                 "passed.")
    ...
    def _get_file_and_tiles(self, ifile):
        ...
        # object vars to pass to gdal.
            ...
            try:
                ...
                file_loc = ifile
                tiles_loc = [x for x in tmp_files if not
                             tt.files.filter(x, ifile, case_sensitive=False)]
            except:
                file_loc = ifile
                tiles_loc = [ifile]
    ...
    def _set_metadata(self):
        """ Get image metadata."""
        meta_geoimg_dict = read_geo_file_info(self.ds)
    ...
    def __repr__(self):
        ...
        for x in prefixes:
            ...
            if tt.files.filter(message,"<VRTDataset*"):
                sss = sss + prefix + 'VRT text string (see files.dfile)' + '\n'
    ...
    def iter_bands(self, **kwargs):
        '''
        Iterator to iterate through image bands.
        ...
        ------
        ndarray
            Two dimensional numpy array for each band.

        '''

        logger.debug('*** begin iter_bands ***')

        # Iterate through windows generated from input parameters
        if 'bands' in list(kwargs.keys()):
            (_, band_nums) = self.get_data(window=[0,0,0,0],
                                           bands=kwargs['bands'],
                                           return_band_nums=True)
            del kwargs['bands']
        else:
            band_nums = list(range(1,self.shape[0]+1))

        for b in band_nums:
            logger.debug('iterating band number %s', b+1)
            yield self.get_data(bands=[b], **kwargs).squeeze()
        ...
        for feat in lyr:
            ...
            if properties is True:
                ...
                if isinstance(properties, (list, tuple, str)):
                    ...
                    if not all([(x in it.keys()) for x in properties]):
    ...
    def get_coord_tran(self, from_wkt=None, to_wkt=None):

        # If you set both from_sr and to_sr to img, then you get no real
        # coord_transform.  There could be a case where you'd want this
        # to run without explicitly hadling it, so an error is not raised.
        from_sr = osr.SpatialReference()
        if from_wkt is None:
            # Set to img spatial reference
            from_sr.ImportFromWkt(self.meta.projection_string)
        else:
            from_sr.ImportFromWkt(from_wkt)

        to_sr = osr.SpatialReference()
        if to_wkt is None:
            # Set to img spatial reference
            to_sr.ImportFromWkt(self.meta.projection_string)
        else:
            to_sr.ImportFromWkt(to_wkt)

        coord_trans = osr.CoordinateTransformation(from_sr, to_sr)

        return coord_trans

    def apply_coord_trans(self, x, y, coord_trans):

        # Lots of format handling yet to be done

        # [ul_img, lr_img] = coord_trans.TransformPoints([ul_vec, lr_vec])
        xx, yy, zz = list(zip(*coord_trans.TransformPoints(list(zip(x, y)))))
    ...
    def get_data(self, component = None,
        ...
        resolution: int or list of two ints
            The desired resolution for the returned numpy array.  Can be either
            a single number that will be applied to both axes or a length
            two list of numbers for the x and y axes.  The value(s) should be
            specified in the image resolution measurement.
        window : list of ints
        ...
        if isinstance(bands,list) and len(bands)==0:
            raise NoValidBandRequested('What the error says.')
        ...
        # Changing resolution is a difficult proposition because it requires
        # either exposing options for what to do at the edges in the case of
        # fraction operations and a resampling kernel or reasonable defaults
        # need to be sufficient for most users.  In this case, there are
        # more powerful upsample/downsample routines that will operate on the
        # full resolution numpy data array for detailed operations.  However,
        # these routines will run more slowely than the gdal implementation
        # used here.  The defaults selected here should be good for most work
        # and can probably be used unless you know otherwise.
        if resolution is not None:
            if virtual:
                raise ValueError('virtual is not implemented when a '
                                 'specific resolution is also requested.')
            if not isinstance(resolution,list):
                resolution = [resolution, resolution]
            file_res = self.meta.resolution
            buf_xsize = int(round((file_res[0]/float(resolution[0]))*win_xsize))
            buf_ysize = int(round((file_res[1]/float(resolution[1]))*win_ysize))
        else:
            buf_xsize=win_xsize
            buf_ysize=win_ysize
        ...
        if not virtual:
            ...
            data = np.empty([zt, buf_ysize, buf_xsize], dtype=dt)
    ...
    def write_img_replace_this(self,np_array):
        ...
        self.ds = None
        self.ds = gdal.Open(reload_fname, gdalconst.GA_Update)

        # Load new data into gdal object file
        nbands = self.shape[0]
        for i in range(nbands):
            b = self.ds.GetRasterBand(i + 1).WriteArray(np_array[i, :, :])
            b = None

        # Dereference gdal object and reopen as read only
        self.ds = None
        self.ds = gdal.Open(reload_fname, gdalconst.GA_ReadOnly)
    ...
    def get_stretch_values(self,**kwargs):
        ...
        return get_img_stretch_vals(self.ds, **kwargs)
...
def instantiate_geom(g):
    ...
    raise NotValidGeom("A geometry object was not able to be created from " \
                       "the value passed in.")
