...
from datasmash.utils import (DatasetLoader, pprint_dict,
...
class Hyperparams(hyperparams.Hyperparams):
    n_quantizations = hyperparams.Hyperparameter[int](
        default = 1,
        description = 'Number of quantization-parameter sets to create from each time series. ',
        semantic_types=['https://metadata.datadrivendiscovery.org/types/ControlParameter']
    )


class d3m_Quantizer(TransformerPrimitiveBase[Inputs, Outputs, Hyperparams]):
    """
    ...
    def _fit_one_channel(self, directory):
        ...
        X = qtz.fit_transforma(directory)
        return X
    ...
    def produce(self, *, inputs: Inputs, timeout: float = None,
        ...
        for channel_dir in selected_channels:
            X = self._fit_one_channel(channel_dir)
        ...
        distance_matrices = []
        for channel_dir, X in self._channel_partition_map.items():
