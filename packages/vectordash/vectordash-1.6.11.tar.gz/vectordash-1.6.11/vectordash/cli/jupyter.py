        ...
        if os.path.isfile(token):
            ...
            if r.status_code == 200:
                ...
                if data.get(machine):
                    ...
                    # ssh.exec_command('rm .serve_jupyter.sh')
                    # ssh.exec_command('touch .serve_jupyter.sh')
                    # cmd = 'echo "nohup jupyter notebook --no-browser --port=8889 --NotebookApp.token=' + jupyter_token + ' &" >> .serve_jupyter.sh'
                    # ssh_stdin, ssh_stdout, ssh_stderr = ssh.exec_command(cmd)

                    # ssh.exec_command('bash .serve_jupyter.sh')
                    # ssh.exec_command('touch bye')
                    transport = ssh.get_transport()
                    channel = transport.open_session()
                    channel.exec_command('nohup jupyter notebook --no-browser --port=8889 --NotebookApp.token='
                                         + jupyter_token + ' > /dev/null 2>&1 &')
                    # ssh.close()
