        ...
        if os.path.isfile(token):
            ...
            if r.status_code == 200:
                ...
                if data.get(machine):
                    ...
                    cmd = 'nohup jupyter notebook --no-browser --port=8889 --NotebookApp.token={} > /dev/null 2>&1 &'.format(jupyter_token)

                    # cmd = ['nohup', 'jupyter', 'notebook', '--no-browser', '--port=8889',
                    #        '--NotebookApp.token={}'.format(jupyter_token), '>', '/dev/null 2>&1 &']
                    ssh_command = ["ssh", user + "@" + ip, "-p", port, "-i", key_file, cmd, "echo '{}' > boyyy".format(cmd)]
