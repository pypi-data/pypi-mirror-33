...
from eostalkbase.exceptions import AccountDoesNotExistsException
...
class Account(dict):
    ...
    def get_balances(self):
        available = {
            'EOSBIT': Amount(self['balance']).amount,
            'SBD': Amount(self['sbd_balance']).amount,
            'VESTS': Amount(self['vesting_shares']).amount,
        }

        savings = {
            'EOSBIT': Amount(self['savings_balance']).amount,
            'SBD': Amount(self['savings_sbd_balance']).amount,
        }

        rewards = {
            'EOSBIT': Amount(self['reward_steem_balance']).amount,
        ...
        totals = {
            'EOSBIT':
                sum([available['EOSBIT'], savings['EOSBIT'], rewards['EOSBIT']]),
            'SBD':
