...
from eostalkbase import memo
from eostalkbase import operations
from eostalkbase.account import PrivateKey, PublicKey
from eostalkbase.exceptions import AccountExistsException, MissingKeyError
from eostalkbase.storage import configStorage
...
class Commit(object):
    ...
    def create_account(
            ...
            delegation_fee_steem='0 EOSBIT',
            creator=None,
    ):
        """ Create new account in Steem

            The brainkey/password can be used to recover all generated keys
            (see `eostalkbase.account` for more details.
        ...
        from eostalkbase.account import PasswordKey, PublicKey
        if password:
    ...
    def transfer(self, to, amount, asset, memo="", account=None):
        ...
        assert asset in ['EOSBIT', 'SBD']

        if memo and memo[0] == "#":
            from eostalkbase import memo as Memo
    ...
    """
        ...
        op = operations.TransferToVesting(
            **{
                ...
                "amount":
                    '{:.{prec}f} {asset}'.format(
                        float(amount), prec=3, asset='EOSBIT')
            })
