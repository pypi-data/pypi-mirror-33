...
from aioacm import ACMClient, DEFAULT_GROUP_NAME
...
async def list_conf(args):
    ...
    try:
        configs = await c.list_all(args.group, args.prefix)
    except:
        print("List failed.")
        sys.exit(1)
...
await def pull(args):
    ...
    try:
        if "/" in args.data_id:
            ...
            content = await c.get(d, g, no_snapshot=True)
        else:
            content = await c.get(args.data_id, None, no_snapshot=True)
    except:
...
async def export(args):
    ...
    for config in configs:
        ...
        sys.stdout.flush()
        try:
            content = await c.get(config["dataId"], config["group"], no_snapshot=True)
        except:
...
async def import_to_server(args):
    e, ep, n, ns = _process_namespace(args)
    c = _get_client(e, ep, n, ns)
    ...
    if args.delete:
        ...
        if delete_list:
            ...
            if delete:
                for i in delete_list:
                    await c.remove(i["dataId"], i["group"])
    ...
    for data in data_to_import:
        ...
        try:
            await c.publish(data[0], data[1], content)
        except:
