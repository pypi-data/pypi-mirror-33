...
def parse_weibo(weibo, target_uid=None):
    ...
    if media_wrap:
        ...
        if picts:
            ...
            media_urls = '|'.join(get_url(item.attrs.get('src', '')) for item in media_urls)
        elif video:
            video_action = dict(parse_qsl(video.attrs['action-data']))
            media_type = 'video' if 'type' in video_action else 'live'
            media_urls = get_url(video_action.get('short_url', video_action.get('url')), with_query=False)
    ...
    urls = dict((item.get_text(strip=True).encode('utf-8'), get_url(item.attrs.get('href', '')))
