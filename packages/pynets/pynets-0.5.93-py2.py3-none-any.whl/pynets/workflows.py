...
def wb_structural_connectometry(ID, atlas_select, network, node_size, mask, parlistfile, plot_switch, parc, ref_txt,
    ...
    prep_nodes_node = pe.Node(niu.Function(input_names=['dwi_dir', 'node_size', 'parc', 'parcel_list',
                                                        ...
                                                        'atlas_select'],
                                           output_names=['parcel_list', 'volumes_dir', 'node_size'],
                                           function=diffconnectometry.prep_nodes, imports=import_list),
                              name="prep_nodes_node")

    if parc is True:
        reg_parcels2diff_node = pe.Node(niu.Function(input_names=['dwi_dir', 'volumes_dir'],
                                                     output_names=['volumes_list'],
        ...
        reg_coords2diff_node = pe.Node(niu.Function(input_names=['coords', 'dwi_dir', 'node_size', 'volumes_dir'],
                                                    output_names=['out_file_diff'],
                                                    function=diffconnectometry.reg_coords2diff, imports=import_list),
                                       name="reg_coords2diff_node")

        cleanup_tmp_nodes_node = pe.Node(niu.Function(input_names=['out_file_diff', 'coords', 'dir_path', 'volumes_dir'],
                                                      output_names=['spheres_list'],
    ...
    if parc is False:
        ...
        wb_structural_connectometry_wf.connect([(inputnode, build_coord_list_node, [('dwi_dir', 'dwi_dir')]),
                                                ...
                                                (prep_nodes_node, reg_coords2diff_node, [('volumes_dir', 'volumes_dir'),
                                                                                         ('node_size', 'node_size')]),
                                                ...
                                                (reg_coords2diff_node, cleanup_tmp_nodes_node, [('out_file_diff', 'out_file_diff')]),
                                                (build_coord_list_node, cleanup_tmp_nodes_node, [('coords', 'coords')]),
                                                (prep_nodes_node, cleanup_tmp_nodes_node, [('volumes_dir', 'volumes_dir')]),
                                                (cleanup_tmp_nodes_node, create_seed_mask_file_node, [('volumes_list', 'volumes_list')])
                                                ])
        ...
        wb_structural_connectometry_wf.connect([(inputnode, reg_parcels2diff_node, [('dwi_dir', 'dwi_dir')]),
                                                (prep_nodes_node, reg_parcels2diff_node, [('volumes_dir', 'volumes_dir')]),
                                                (reg_parcels2diff_node, create_seed_mask_file_node, [('volumes_list', 'volumes_list')])
                                                ])
...
def rsn_structural_connectometry(ID, atlas_select, network, node_size, mask, parlistfile, plot_switch, parc, ref_txt,
    ...
    nodif_brain_mask_path = dwi_dir + '/nodif_brain_mask.nii.gz'
    ...
    create_mni2diff_transforms_node = pe.Node(niu.Function(input_names=['dwi_dir'],
    ...
    grow_nodes_node = pe.Node(niu.Function(input_names=['dwi_dir', 'coords', 'node_size', 'parc', 'parcel_list',
                                                        'net_parcels_map_nifti', 'network', 'dir_path', 'mask'],
    ...
    rsn_structural_connectometry_wf.connect([
        ...
        (inputnode, grow_nodes_node, [('dwi_dir', 'dwi_dir'),
                                      ('node_size', 'node_size'),
                                      ('parc', 'parc'),
                                      ('mask', 'mask'),
                                      ('network', 'network')]),
