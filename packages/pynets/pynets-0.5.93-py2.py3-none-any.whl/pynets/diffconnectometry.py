...
def reg_coords2diff(coords, dwi_dir, node_size, volumes_dir):
    try:
        FSLDIR = os.environ['FSLDIR']
    except NameError:
        print('FSLDIR environment variable not set!')
    ...
    for coord in coords[0]:
        ...
        out_file1 = "%s%s%s%s%s%s%s%s" % (volumes_dir, '/roi_point_', str(X), '_', str(Y), '_', str(Z), '.nii.gz')
        ...
        out_file2 = "%s%s%s%s%s%s%s%s" % (volumes_dir, '/region_', str(X), '_', str(Y), '_', str(Z), '.nii.gz')
    ...
    return out_file_diff


def cleanup_tmp_nodes(out_file_diff, volumes_dir, coords, dir_path):
    import glob
    import nipype.interfaces.fsl as fsl
    try:
        import cPickle as pickle
    ...
    volumes_list = [i for i in os.listdir(volumes_dir) if 'diff.nii.gz' in i]
...
def reg_parcels2diff(dwi_dir, volumes_dir):
    import re
    import nipype.interfaces.fsl as fsl
    import nipype.pipeline.engine as pe
    merged_f_samples_path = "%s%s" % (dwi_dir, '/merged_f1samples.nii.gz')
...
def create_seed_mask_file(node_size, network, dir_path, parc, volumes_list):
    import glob
    import re
    import shutil
...
def save_parcel_vols(node_size, parcel_list, net_parcels_map_nifti, dir_path, volumes_dir):
    import os
    import time
...
def prep_nodes(dwi_dir, node_size, parc, parcel_list, net_parcels_map_nifti, network, dir_path, mask, atlas_select):
    ...
    if parc is True:
        volumes_dir = "%s%s%s" % (dir_path, '/volumes_', atlas_select)
    else:
        volumes_dir = "%s%s%s%s%s" % (dir_path, '/volumes_', atlas_select, '_', str(node_size))
    ...
    if parc is True:
        ...
        diffconnectometry.save_parcel_vols(node_size, parcel_list, net_parcels_map_nifti, dir_path, volumes_dir)

    return parcel_list, volumes_dir, node_size
...
def dwi_ndmg_run(dwi_dir, node_size, dir_path, conn_model, parc, atlas_select, way_mask=None, num_total_samples=1000000):
    ...
    seed_files = glob.glob("%s%s" % (volumes_dir, '/*diff.nii.gz'))

    label_sum = "%s%s" % (volumes_dir, '/all_rois.nii.gz')
    args = ''
    for i in seed_files:
        args = args + ' -add ' + i
...
def collect_struct_mapping_outputs(parc, dwi_dir, network, ID, probtrackx_output_dir_path, dir_path, procmem, volumes_dir):
    import os
    import time
    import glob
    import nibabel as nib
