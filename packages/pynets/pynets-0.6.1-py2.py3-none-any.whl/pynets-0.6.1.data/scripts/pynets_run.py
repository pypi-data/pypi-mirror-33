...
#    adapt_thresh=args.at
    ...
    def workflow_selector(input_file, ID, atlas_select, network, node_size, mask, thr, parlistfile, multi_nets,
        ...
        meta_wf.get_node("%s%s%s" % ('wb_functional_connectometry_', str(ID), '.WB_fetch_nodes_and_labels_node'))._num_threads = int(procmem[0])-1
        meta_wf.get_node("%s%s%s" % ('wb_functional_connectometry_', str(ID), '.WB_fetch_nodes_and_labels_node'))._mem_gb = int(procmem[1])-1
        meta_wf.get_node("%s%s%s" % ('wb_functional_connectometry_', str(ID), '.extract_ts_wb_coords_node'))._num_threads = int(procmem[0])-1
        meta_wf.get_node("%s%s%s" % ('wb_functional_connectometry_', str(ID), '.extract_ts_wb_coords_node'))._mem_gb = int(procmem[1])-1
        meta_wf.get_node("%s%s%s" % ('wb_functional_connectometry_', str(ID), '.thresh_and_fit_node'))._num_threads = int(procmem[0])-1
        meta_wf.get_node("%s%s%s" % ('wb_functional_connectometry_', str(ID), '.thresh_and_fit_node'))._mem_gb = int(procmem[1])-1
