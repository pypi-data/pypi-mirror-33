...
if __name__ == '__main__':
    ...
    if resources:
        ...
        nthreads = cpu_count()
        procmem = str(cpu_count()) + ',' + str(nthreads*2)
    thr = float(args.thr)
...
#    adapt_thresh=args.at
    ...
    def workflow_selector(input_file, ID, atlas_select, network, node_size, mask, thr, parlistfile, multi_nets,
        ...
        meta_wf = Workflow(name='Meta_wf_imp_est_' + str(ID))
        meta_wf.base_dir = os.path.dirname(input_file)
        ...
        config.update_config({'logging': {'log_directory': dir_path, 'log_to_file': True}})
        ...
        #               'raise_insufficient': False, 'maxtasksperchild': 1}
    ...
    def init_wf_single_subject(ID, input_file, dir_path, atlas_select, network, node_size, mask, thr, parlistfile,
        ...
        wf.base_directory = os.path.dirname(input_file)
