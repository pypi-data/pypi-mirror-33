...
def post_multi_tag_datapoints(timeseries_with_datapoints, **kwargs):
    ...
    for bin in timeseries_to_upload_binned:
        body = {'items': [{'tagId': quote_plus(ts_with_data.name), 'datapoints': [dp.__dict__ for dp in ts_with_data.datapoints]} for ts_with_data in bin]}
...
def get_timeseries(prefix=None, description=None, include_metadata=False, asset_id=None, path=None, **kwargs):
    ...
    headers = {'api-key': api_key, 'accept': 'application/json'}
    params = {'q': quote_plus(prefix), 'description': description, 'includeMetadata': include_metadata, 'assetId': asset_id, 'path': path, 'limit': (kwargs.get('limit', 10000) if (not kwargs.get('autopaging')) else 10000)}
    time_series = []
