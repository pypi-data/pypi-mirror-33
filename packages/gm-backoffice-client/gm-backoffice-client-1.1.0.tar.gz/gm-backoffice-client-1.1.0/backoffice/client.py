...
class BackofficeClient:
    ...
    PRODUCTS_OFFERS_STAT = '{HOST}/products/offers_stat/?site_ids={site_ids}'
    ...
    def get_products_sales_stat(self, site_ids: List[Union[str, int]]) -> dict:
        return self._get_data_by_site_ids(self.PRODUCTS_SALES_STAT, site_ids)

    def get_products_offers_stat(self, site_ids: List[Union[str, int]]) -> dict:
        return self._get_data_by_site_ids(self.PRODUCTS_OFFERS_STAT, site_ids)

    def _get_data_by_site_ids(self, url, site_ids: List[Union[str, int]]) -> dict:
        site_ids_str = ','.join([str(el) for el in site_ids])
        r = requests.get(
            url.format(HOST=self.url, site_ids=site_ids_str),
            headers=self.headers,
            timeout=self.TIMEOUT,
        )
        if r.status_code != 200:
            raise BackofficeHTTPException(
                'Non-200 response when getting stat for site_ids from url{}: {}'.format(url, site_ids))
