...
from scrappyserver.scrapy_utils import (create_crawler_object, start_logger,
...
def get_item_export_extension(bool_json, bool_csv):
    extensions = {}
    if bool_json:
        extensions['scrappyserver.extensions.ExportJSON'] = 100
    if bool_csv:
        extensions['scrappyserver.extensions.ExportCSV'] = 200
    return extensions
...
class TestScrapyUtils(TestCase):
    ...
    def test_get_spider_settings(self):
        ...
        for item in bool_json_csv:
            ...
            spider_settings = {
                'EXTENSIONS': {
                    'scrappyserver.extensions.ExportJSON': 300,
                    'scrappyserver.extensions.ExportCSV': 400,
                }
            }
    ...
    def test_start_crawler(self):
        if SCRAPY_VERSION <= (1, 0, 0):
            with patch('scrappyserver.scrapy_utils.tlog') as tlog:
                with patch('scrappyserver.scrapy_utils.load_object') as load_object:
                    with patch('scrappyserver.scrapy_utils.Crawler') as _:
                        spider_loc = 'ABC.ABC'
