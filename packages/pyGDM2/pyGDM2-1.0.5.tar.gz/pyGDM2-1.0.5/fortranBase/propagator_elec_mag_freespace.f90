      ...
      if ( (cn1.ne.cn2).or.(cn1.ne.cn3) ) then
       write (*,*) "Error. Only defined for homogeneous media! please use according refractive-indices."
        ...
        CALL PROPAGATOR123(ALAMBDA,&
                ...
                & TXX1,TYY1,TZZ1,TXY1,TXZ1,TYZ1,TYX1,TZX1,TZY1,&
                & TXX2,TYY2,TZZ2,TXY2,TXZ2,TYZ2,TYX2,TZX2,TZY2,&
                & cn1,cn2,cn3)
        
        CBBXX=TXX1*CUN+TXX2*CIM
        CBBYY=TYY1*CUN+TYY2*CIM
        CBBZZ=TZZ1*CUN+TZZ2*CIM
        CBBXY=TXY1*CUN+TXY2*CIM
        CBBXZ=TXZ1*CUN+TXZ2*CIM
        CBBYZ=TYZ1*CUN+TYZ2*CIM
        CBBYX=TYX1*CUN+TYX2*CIM
        CBBZX=TZX1*CUN+TZX2*CIM
        CBBZY=TZY1*CUN+TZY2*CIM
      END 
!!    -----------------------------------------------------------------------
