    ...
    def upload_player_positions(self, timestamp, match):
        try:
            ...
            for i in range(players.shape[0]):
                if players[i][0] == -1:
                    continue
                data["playerPositions"]["playerId"] = int(players[i][0])
                data["playerPositions"]["teamId"] = int(players[i][1])
                data["playerPositions"]["positionX"] = int(players[i][2] / 1000)
                data["playerPositions"]["positionY"] = int(players[i][3] / 1000)
                data["playerPositions"]["certainty"] = int(players[i][4] / 1000)

            url = "{}/playerPositions/{}/{}/{}/{}".format(self.api_stream_url, match_id, timestamp, self.user_escaped, self.password)
            self.post(url, **data)
