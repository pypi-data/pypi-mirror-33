...
class E_M:
  ...
  def  __init__(self,conf_sec='section_etl_excel_label',config_file='conf/etl.conf',header=0,encoding='gbk',is_csv=False,csv_sep='\t'):
    ...
    print("开始寻找 E_M 的配置项，请确认 EM 的配置文件路径是否正确")
  ...
  def get_ftps_client(self,config_ftp_sec='sec_ftps_login'):
    """
    获取 ftp 客户端 句柄
    :param config_ftp_sec: ftp 相关配置片段
    :return: self.Ftps
    """
  ...
  def  push_file_ftp(self,upload_etl_file,config_ftp_sec='sec_ftps_login',dir_sep='/'):
    """
    通过ftp 上传文件到ftp 服务器
    :param upload_etl_file:
    :param config_ftp_sec:
    :param dir_sep:
    :return:
    """
  ...
  def  get_ssh_client(self,sec_remote_host = 'sec_remote_host'):
    """
    获取 ssh 客户端 句柄
    :param sec_remote_host: ssh客户端配置片段
  ...
  def  exec_ssh_command(self,hive_host,cmd_str,is_jump=True):
    if self.ssh_session != None:
      ...
      stdin, stdout, stderr = self.ssh_session.exec_command(final_cmd_str,get_pty=True)
  ...
  def  exec_cd_path_ssh_command(self,server_path,hive_host,cmd_str,is_jump=True):
    if self.ssh_session==None:
      self.get_ssh_client()
    if self.ssh_session != None:
      logging.info(msg="server host %s will exec__command  %s" % (hive_host, cmd_str))
      if is_jump==True:
        final_cmd_str = 'ssh  %s -t %s' % (hive_host, cmd_str)
        logger.info(msg="final_cmd_str %s "%final_cmd_str)
      else:
        final_cmd_str =cmd_str
      print("****************入库会消耗一段时间，请耐心等待 程序的完成，最后插入情况会输出在 console中 。。。。。*********")
      stdin, stdout, stderr = self.ssh_session.exec_command(final_cmd_str,get_pty=True)
      channel = stdout.channel
      status = channel.recv_exit_status()
      for line in stdout:
  ...
  def  ftpget_file_server(self,sec_remote_host='sec_remote_host',dir_sep='/'):
    """
    把已经存在ftp服务器上的上传文件  拉取到 hive 服务器上，
    如果要通用的话 self.hive_server_dir_path 不为空  self.ftp_etl_file不为空
    :param sec_remote_host: hive 机器配置片段
    :param dir_sep: 目录分隔符
    :return:
    """
  ...
  def  script_insert_hive(self,script_param_str='',hive_server_dir_path=None,sec_remote_host='sec_remote_host',script_name='load_label.sh',dir_sep='/'):
    hivehost = self.config_parser.get(sec_remote_host, 'hive_host')
    if hive_server_dir_path==None:
    ...
    #ftp_etl_file=str(self.hive_server_dir_path)+"/"+ftp_etl_file
    ...
    try:
     insert_status=self.exec_cd_path_ssh_command(hive_server_dir_path,hivehost, exec_hive_cmd)
