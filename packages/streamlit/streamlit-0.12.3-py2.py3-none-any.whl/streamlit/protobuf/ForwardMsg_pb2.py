# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: ForwardMsg.proto

import sys
_b=sys.version_info[0]<3 and (lambda x:x) or (lambda x:x.encode('latin1'))
from google.protobuf import descriptor as _descriptor
from google.protobuf import message as _message
from google.protobuf import reflection as _reflection
from google.protobuf import symbol_database as _symbol_database
from google.protobuf import descriptor_pb2
# @@protoc_insertion_point(imports)

_sym_db = _symbol_database.Default()


import Delta_pb2 as Delta__pb2


DESCRIPTOR = _descriptor.FileDescriptor(
  name='ForwardMsg.proto',
  package='',
  syntax='proto3',
  serialized_pb=_b('\n\x10\x46orwardMsg.proto\x1a\x0b\x44\x65lta.proto\"\xa4\x01\n\nForwardMsg\x12\x14\n\nnew_report\x18\x01 \x01(\tH\x00\x12 \n\ndelta_list\x18\x02 \x01(\x0b\x32\n.DeltaListH\x00\x12\x19\n\x0freport_finished\x18\x03 \x01(\x08H\x00\x12 \n\x16upload_report_progress\x18\x04 \x01(\rH\x00\x12\x19\n\x0freport_uploaded\x18\x05 \x01(\tH\x00\x42\x06\n\x04typeb\x06proto3')
  ,
  dependencies=[Delta__pb2.DESCRIPTOR,])




_FORWARDMSG = _descriptor.Descriptor(
  name='ForwardMsg',
  full_name='ForwardMsg',
  filename=None,
  file=DESCRIPTOR,
  containing_type=None,
  fields=[
    _descriptor.FieldDescriptor(
      name='new_report', full_name='ForwardMsg.new_report', index=0,
      number=1, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='delta_list', full_name='ForwardMsg.delta_list', index=1,
      number=2, type=11, cpp_type=10, label=1,
      has_default_value=False, default_value=None,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='report_finished', full_name='ForwardMsg.report_finished', index=2,
      number=3, type=8, cpp_type=7, label=1,
      has_default_value=False, default_value=False,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='upload_report_progress', full_name='ForwardMsg.upload_report_progress', index=3,
      number=4, type=13, cpp_type=3, label=1,
      has_default_value=False, default_value=0,
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None, file=DESCRIPTOR),
    _descriptor.FieldDescriptor(
      name='report_uploaded', full_name='ForwardMsg.report_uploaded', index=4,
      number=5, type=9, cpp_type=9, label=1,
      has_default_value=False, default_value=_b("").decode('utf-8'),
      message_type=None, enum_type=None, containing_type=None,
      is_extension=False, extension_scope=None,
      options=None, file=DESCRIPTOR),
  ],
  extensions=[
  ],
  nested_types=[],
  enum_types=[
  ],
  options=None,
  is_extendable=False,
  syntax='proto3',
  extension_ranges=[],
  oneofs=[
    _descriptor.OneofDescriptor(
      name='type', full_name='ForwardMsg.type',
      index=0, containing_type=None, fields=[]),
  ],
  serialized_start=34,
  serialized_end=198,
)

_FORWARDMSG.fields_by_name['delta_list'].message_type = Delta__pb2._DELTALIST
_FORWARDMSG.oneofs_by_name['type'].fields.append(
  _FORWARDMSG.fields_by_name['new_report'])
_FORWARDMSG.fields_by_name['new_report'].containing_oneof = _FORWARDMSG.oneofs_by_name['type']
_FORWARDMSG.oneofs_by_name['type'].fields.append(
  _FORWARDMSG.fields_by_name['delta_list'])
_FORWARDMSG.fields_by_name['delta_list'].containing_oneof = _FORWARDMSG.oneofs_by_name['type']
_FORWARDMSG.oneofs_by_name['type'].fields.append(
  _FORWARDMSG.fields_by_name['report_finished'])
_FORWARDMSG.fields_by_name['report_finished'].containing_oneof = _FORWARDMSG.oneofs_by_name['type']
_FORWARDMSG.oneofs_by_name['type'].fields.append(
  _FORWARDMSG.fields_by_name['upload_report_progress'])
_FORWARDMSG.fields_by_name['upload_report_progress'].containing_oneof = _FORWARDMSG.oneofs_by_name['type']
_FORWARDMSG.oneofs_by_name['type'].fields.append(
  _FORWARDMSG.fields_by_name['report_uploaded'])
_FORWARDMSG.fields_by_name['report_uploaded'].containing_oneof = _FORWARDMSG.oneofs_by_name['type']
DESCRIPTOR.message_types_by_name['ForwardMsg'] = _FORWARDMSG
_sym_db.RegisterFileDescriptor(DESCRIPTOR)

ForwardMsg = _reflection.GeneratedProtocolMessageType('ForwardMsg', (_message.Message,), dict(
  DESCRIPTOR = _FORWARDMSG,
  __module__ = 'ForwardMsg_pb2'
  # @@protoc_insertion_point(class_scope:ForwardMsg)
  ))
_sym_db.RegisterMessage(ForwardMsg)


# @@protoc_insertion_point(module_scope)
