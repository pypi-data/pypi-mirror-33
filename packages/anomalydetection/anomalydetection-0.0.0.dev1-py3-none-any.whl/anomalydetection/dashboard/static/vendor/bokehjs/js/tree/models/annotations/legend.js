    ...
    LegendView.prototype.on_hit = function (sx, sy) {
        ...
        for (var _i = 0, _a = this.model.items; _i < _a.length; _i++) {
            ...
            for (var _b = 0, labels_1 = labels; _b < labels_1.length; _b++) {
                ...
                if (vertical)
                    _c = [legend_bbox.width - 2 * legend_padding, this.max_label_height], w = _c[0], h = _c[1];
                else
                    _d = [this.text_widths[label] + glyph_width + label_standoff, this.max_label_height], w = _d[0], h = _d[1];
        ...
        var _loop_1 = function (item) {
            ...
            for (var _i = 0, labels_2 = labels; _i < labels_2.length; _i++) {
                ...
                if (!active) {
                    var w = void 0, h = void 0;
                    if (vertical)
                        _c = [bbox.width - 2 * legend_padding, this_1.max_label_height], w = _c[0], h = _c[1];
                    else
                        _d = [this_1.text_widths[label] + glyph_width + label_standoff, this_1.max_label_height], w = _d[0], h = _d[1];
                    ctx.beginPath();
                    ctx.rect(x1, y1, w, h);
