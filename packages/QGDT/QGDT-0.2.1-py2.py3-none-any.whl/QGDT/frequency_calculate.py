...
from QGDT.templates.templates_x0 import x0_1,x0_2,x0_3
...
class Search2Frequency(object):
    ...
    def question2probability(self,t):
        ...
        with torch.no_grad():
            ...
            prob = torch.ones(len(self.dict))
            id = 0
            while True:
                try:
                    init_id = self.dict[question[id]]
                    break
                except:
                    id+=1
            prob[init_id] = float('inf')
            input = torch.multinomial(prob, num_samples=1).unsqueeze(1).to(device)

        #计算每个词的概率得分
        score_list = []
        for i in range(id + 1,question.__len__()):
            output, state = self.rnnlm(input, state)
            try:
                word_id = self.dict[question[i]]
            except:
                score_list.append(0.0)
                continue
            score_list.append(float(output[0][word_id]))
            input.fill_(word_id)
        return sum(score_list)/(score_list.__len__()+1)
