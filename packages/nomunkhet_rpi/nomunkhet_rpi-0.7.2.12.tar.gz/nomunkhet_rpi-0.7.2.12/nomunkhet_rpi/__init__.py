...
class Nud_garalt():
	def __init__(self, a, virtualC,  name, file_on = "/usr/local/lib/python3.5/dist-packages/nomunkhet_rpi/lamp.py", file_off = "/usr/local/lib/python3.5/dist-packages/nomunkhet_rpi/lamp1.py", r = 0, c = 0, port = 0, keyboard_short = None):
		global All, app
		All.append(self)
...
def cayenne_interrupt(channel):
	for each_orolt in ALL_orolt:
		if each_orolt.port == channel:
			each_orolt.digital = GPIO.input(each_orolt.port)
			client.virtualWrite(each_orolt.virtualChannel, each_orolt.digital)
			each_orolt.isValueChanged = True

class Nud_orolt():
	def __init__(self, a, virtualC,  name, file_on = "/usr/local/lib/python3.5/dist-packages/nomunkhet_rpi/arrowU.py", file_off = "/usr/local/lib/python3.5/dist-packages/nomunkhet_rpi/arrowD.py", r = 0, c = 0, port = 0, pullup = "no", interrupt = None):
		global All, app, ALL_orolt
		ALL_orolt.append(self)
		self.virtualChannel = virtualC
		self.port = port
		if is_RPI:
			if pullup == "no":
			...
			if interrupt == None:
				pass
			elif interrupt == "RISING":
				GPIO.add_event_detect(self.port, GPIO.RISING, callback = cayenne_interrupt)
			elif interrupt == "FALLING":
				GPIO.add_event_detect(self.port, GPIO.FALLING, callback = cayenne_interrupt)
		print("hello:", name)
		self.name = name
		self.photo1  = tk.PhotoImage(file=file_on)
		self.photo0  = tk.PhotoImage(file=file_off)
		if is_RPI:
			self.digital = GPIO.input(self.port)
		else:
			self.digital = 0
		self.root = a
		self.button = tk.Button(a, image=self.photo0, text=name,
...
app.iconphoto(True, tk.PhotoImage(file=os.path.join(program_directory, "/usr/local/lib/python3.5/dist-packages/nomunkhet_rpi/eslogo.py"))) #end uurchlult hiih
...
def on_message2(message):
	...
	for each_Nud in All:
		if 	int(each_Nud.virtualChannel) == int(message.channel):
...
def do_it():
	...
	for each_orolt in ALL_orolt:
		inputGPIO = each_orolt.digital
		if is_RPI:
			inputGPIO = GPIO.input(each_orolt.port) 
		if inputGPIO != each_orolt.digital or each_orolt.isValueChanged == True:
			each_orolt.digital = inputGPIO
			each_orolt.change_port_value()
			client.virtualWrite(each_orolt.virtualChannel, each_orolt.digital)
			each_orolt.isValueChanged = False
