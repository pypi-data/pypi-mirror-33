...
def set_app_key(app_key):
    ...
    Notes
    -----
    The application Key identifies your application on Thomson Reuters Platform.
    You can get an application key using the Application Key generator. This App can
    be launched from Eikon Desktop or Eikon Proxy welcome page.
    """
...
class Profile(object):
    ...
    def set_app_key(self, app_key):
        ...
        if port_number is not None:
            ...
            self.logger.info('Application ID: {}'.format(self.app_key))
        else:
            # port number wasn't identified => do nothing ?
            self.logger.error('Port number was not identified, cannot send any request')
...
def read_firstline_in_file(filename):
    ...
    try:
        ...
        logger.error('I/O error({}): {1}'.format(e.errno, e.strerror))
        return ''
...
def check_port(session, application_key, port, timeout=(1.0,2.0)):
    logger = get_profile().logger
...
def handshake(session, application_key, port, timeout=(1.0,2.0)):
    logger = get_profile().logger
