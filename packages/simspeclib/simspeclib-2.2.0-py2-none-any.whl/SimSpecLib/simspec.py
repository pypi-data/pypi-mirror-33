...
class SimSpec:
    ...
    iraf.ccdred.instrument=ProDir+'extern/blank.dat'
    #iraf.task(ss_lacos=ProDir+'extern/lacos_spec.cl')
    ...
    def __init__(self, inifile='setup.conf', Object=False, DataDir='', WorkDir=''):
        ...
        if not self.os.path.isfile(inifile):
            raise ValueError("Unable to find configuration file: "+inifile)
    ...
    def parseObject(self, Object):
        ...
        if self.StdFiles:
            try:
                ...
                raise RuntimeError('No name defined for "'+self.StdGrp+'" group standard.')
        ...
        if not self.ObjectFiles:
            raise RuntimeError('No files listed for the "'+Object+'" group object.')
    ...
    def parseSetup(self):
        ...
        if self.inifile!=self.ConfDir+'default/setup.conf':
            try:
                ...
                raise RuntimeError('Error loading '+self.Instrument+' configuration.')
    ...
    def PreCleanExec(self,files):
        ...
        execfile(self.param['precleanscript'])
    ...
    def Clean(self,DelOlder=False):
        ...
        if not self.os.path.isfile(self.Files['Object2D']):
            ...
            if self.param['deleteobjsingles']:
                self.iraf.imdelete(self.Files['ObjectSQ']+'???.fits')
        execfile(self.param['postcleanscript'])
    ...
    def ApExtract(self, spectype='object',DelOlder=False):
        ...
        self.iraf.apall.width         = 5.             #Profile centering width
        self.iraf.apall.radius        = 8.             #Profile centering radius
        ...
        if spectype.lower()=='object':
            ...
            raise ValueError('Invalid SpecType '+spectype)
...
class SSutil:
    ...
    def ObsLog(inst='default',lsinp='*.fits', datadir=''):
        ...
        if (len(split)>1):
            ...
            index=''
            indexn=0
        for ls in sorted(glob.glob(os.path.join(datadir,lsinp))):
            row=[ls+index];
            try:
    ...
    def RunSNID(sfile,SnidExe):
        ...
        SimSpec.iraf.wspectext(sfile+'[*,1]' ,SimSpec.os.path.join(SimSpec.os.path.dirname(sfile),fname+'.txt'), header='no')
