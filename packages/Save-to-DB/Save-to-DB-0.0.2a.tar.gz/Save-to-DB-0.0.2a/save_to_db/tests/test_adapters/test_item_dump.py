...
class TestItemDump(TestBase):
    ...
    def test_multiple_items_dump(self):
        ...
        item_as_dict = self.persister.load(storage).to_dict()
        self.assertEqual(item_as_dict, {'f_integer': 1})
        
        item_as_dict = self.persister.load(storage).to_dict()
        self.assertEqual(item_as_dict, {'f_integer': 2})
        
        item_as_dict = self.persister.load(storage).to_dict()
        self.assertEqual(item_as_dict, {'f_integer': 3})
        
        # no more items
        item_as_dict = self.persister.load(storage)
        self.assertIsNone(item_as_dict)
