...
def get_reviews(isbn13, bookstores=Naverbook, count=10):
    """
    ...
    if isinstance(bookstores, type):
        yield from bookstores.get_reviews(isbn13=isbn13, count=count)
    elif isinstance(bookstores, Iterable):
        for bookstore in bookstores:
            yield from bookstore.get_reviews(isbn13=isbn13, count=count)
