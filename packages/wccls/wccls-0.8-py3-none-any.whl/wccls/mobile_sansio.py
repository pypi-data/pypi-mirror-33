...
# return login request to transport
# get login response from transport - just need page text
# return new page requests that we want fulfilled
# get login response from transport - just need page text
# we implicitly ask that the transport keep the session

class SansioError(Exception):
	pass
...
class MobileSansio:
	...
	def Requests(self):
		return requestedUrls.values()

	def Response(requestedUrl, responseText):
		"""Called by transport to pass http response"""
		if requestedUrl not in self.requestedUrls:
			raise SansioError(f"{requestedUrl} wasn't requested")
		handler = self.requestedUrls[requestedUrl]
