...
def _denominator(equation, variables=None):
    """find denominators containing the given variables in rhs of equation"""
...
def _simplify(eqn, rand=random.random, target=None, **kwds):
    ...
    #FIXME: following is insufficient w/multiple 'zs' (i.e. A != 0, A != -B)
    ...
    def _testvals(testcode):
        ...
        _locals = testvars.copy()
        code = """from numpy import *; from math import *;""" # prefer math
        code += """from numpy import mean as average;"""
        code += """from numpy import var as variance;"""
        code += """from numpy import ptp as spread;"""
        code += """_sqrt = lambda x:x**.5;"""
        ...
        return dict((i,_locals[i]) for i in allvars)
    ...
    for sign in signs:
        if _equals(before, after, testvals.next(), list(variants), **kwds):
            new = [after]
        else:
   ...
   #results = condense(*results, **kwds) #XXX: may remove based on testvals
...
"""
    ...
    _locals = kwds['_locals'] if '_locals' in kwds else {} #XXX: HACK _locals
...
def simplify(constraints, variables='x', target=None, **kwds):
    ...
    import random
    import itertools as it
    _locals = kwds['_locals'] = {} #XXX: HACK _locals
...
if __name__ == '__main__':
    ...
    var = list('ABCD')
    eqns = simplify(equations, variables=var, all=True)
