...
def test_getstate():
    ...
    dumps(f)
    b = bar[0]
    dumps(lambda: f, recurse=False, byref=True) # doesn't call __getstate__
    assert bar[0] == b
    dumps(lambda: f, recurse=True, byref=True) # calls __getstate__
    assert bar[0] == b + 1
   #dumps(lambda: f, recurse=False, byref=False) # calls __getstate__
   #assert bar[0] == b + 2
   #dumps(lambda: f, recurse=True, byref=False) # calls __getstate__
   #assert bar[0] == b + 3
