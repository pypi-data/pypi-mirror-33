...
def pickles(obj,exact=False,safe=False,**kwds):
    ...
    try:
        ...
        if not exact:
           ...
           ## ..alternate for the above line...       #XXX: more strict
           #try: from dill.source import getsource
           #except: from .source import getsource
           #try:
           #    sp,so = getsource(pik),getsource(obj)
           #except: # if fails to getsource, return
           #    return result
           #if sp.startswith('import dill') and so.startswith('import dill'):
           #    # deal with oddities of python3.4
           #    if sys.hexversion >= 0x03040000:
           #        result = repr(type(pik)) == repr(type(obj))
           #    else:
           #        result = sp == so
           #return result
        return False
