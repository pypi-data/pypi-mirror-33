...
from pickle import HIGHEST_PROTOCOL, PicklingError, UnpicklingError, POP
try:
...
def save_cell(pickler, obj):
    log.info("Ce: %s" % obj)
    f = obj.cell_contents
   #_memo = not IS_PYPY and id(f) in stack
   #if not IS_PYPY: stack.add(id(f))
   #if _memo: pickler.memoize(f)
    if id(obj) in pickler.memo:
        pickler.write(POP + picker.get(pickler.memo[id(obj)][0]))
    else:
        pickler.memoize(obj)
    #closure = '__closure__' if PY3 else 'func_closure' 
    # if getattr(f, closure)[0].cell_contents is f: _byref = True
