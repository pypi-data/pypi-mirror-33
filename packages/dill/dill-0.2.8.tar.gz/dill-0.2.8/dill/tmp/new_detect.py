...
def globalvars(func, recurse=True, builtin=False):
    ...
    if isfunction(func):
        ...
        # get 'nested' references (e.g. within closures)
        from .source import importable
        try: code = importable(func) or ''
        except: code = ''
        print('CODE: %s' % code)
        nested_func = set()
        code = nestedcode(compile(code, '<string>', 'exec'))
        for obj in code:
            funcs = globalvars(obj, recurse, builtin)
#           nested_func.update(funcs.keys())
#           globs.update(funcs)
        print('GLOBS: %s' % globs.keys())
        # get remaining function globals
        globs.update(getattr(func, func_globals) or {})
        print('GLOBS: %s' % globs.keys())
        # get referenced names
        if not recurse:
            nested_func.update(getattr(func, func_code).co_names)
            func = nested_func
        else:
            orig_func = func #XXX: better way to stop infinite recursion?
            nested_func.update(nestedglobals(getattr(func, func_code)))
            func = nested_func
