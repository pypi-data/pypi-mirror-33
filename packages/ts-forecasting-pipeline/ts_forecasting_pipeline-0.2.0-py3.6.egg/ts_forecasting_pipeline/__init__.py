"""
Import top-level classes/methods
"""
from typing import Tuple, Union
from statsmodels.regression.linear_model import RegressionResultsWrapper as StatsModel
...
class ModelState(object):
    ...
    Container to hold all model-relevant state information:
    The trained model and the ModelsSpecs it is based on."""

    model: MODEL_TYPES  # A statsmodels trained model
    specs: ModelSpecs

    def __init__(self, model, specs):
        if not isinstance(model, MODEL_CLASSES):
            print(model)
            raise Exception("ModelState(): model parameter needs to be of type %s" % str(MODEL_CLASSES))
        self.model = model
        if not isinstance(specs, ModelSpecs):
            raise Exception("ModelState(): specs parameter needs to be of type <ModelSpecs>")
        self.specs = specs
