...
def layer_wise_reduce_mode(callback=None):
    return configuration.using_config('layer_wise_reduce_params', callback)
...
class VariableNode(object):
    ...
    @property
    def order(self):
        return self._lwr_order

    @order.setter
    def order(self, order):
        self._lwr_order = order
...
- grad: {grad}"""
    ...
    def backward(self, retain_grad=False, enable_double_backprop=False, loss_scale=None):
        ...
        callback = getattr(configuration.config, 'layer_wise_reduce_params',
                           None)

        if ooc_enabled and (callback is not None):
            print('Disable layer-wised reduction. Cannot enabled with OoC')
            callback = None
        ...
        if ooc_enabled:
            ...
            break_points = self.node.get_break_points(callback.fine_granularity)
            if callback.debug:
                print('# break_points: {}'.format([x[2].rank for x in break_points]))
            callback.start_backward()
        else:
        ...
        while bp is not None or bp_next is not None:
            ...
            if bp is not None:
                ...
                if callback is not None and callback.debug:
                    print('# variable.py, backward, {} {}'
                          .format(bp.rank, bp._creator_node_g.rank))

                bp.resume_backward()
                bp_var = bp.get_variable()
                if bp_var._grad_var is None:
                    bp_var._grad_var = bp._grad_var
                bp_var._backward(root_node, retain_grad, enable_double_backprop,  loss_scale)
                ...
                if callback is not None:
                    ...
                    ancestor_vnodes = bp.ancestors()
                    callback.run_vnode(
                        [x for x in ancestor_vnodes
                         if (x.grad is not None) and not x._break_point])
        ...
        if callback is not None:
            callback.end_backward()

    def _backward(self, root_node, retain_grad=False, enable_double_backprop=False,
                 loss_scale=None):
        ...
        with chainer.using_config('enable_backprop', enable_double_backprop):
            self._backward_main(retain_grad, loss_scale, root_node)

    def _backward_main(self, retain_grad, loss_scale, root_node):
