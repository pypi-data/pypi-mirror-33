...
#        f_r16estim = nodes_infinum.min()
        abs_nodes_infinum = abs(nodes_infinum)
#        abs_f_r16estim = abs(f_r16estim)
#        abs_r40 = abs(r40)
        abs_r41 = abs(r41)
        
        rTolThreshold = p.rTol * where(abs_nodes_infinum < abs_r41, abs_nodes_infinum, abs_r41)
#        print('rTolThreshold[:2]:', rTolThreshold[:2], 'abs_r41:', abs_r41, 'abs_tmp[:2]:', abs_tmp[:2])
#        print('f_r16estim:', f_r16estim, 'tmp[:2] - f_r16estim:', tmp[:2] - f_r16estim)
        
        #prev
        cond_exclude = r41 - nodes_infinum < rTolThreshold
        l = len(where(cond_exclude)[0])
        if p.debug and l != 0:
            print('num excluded by rTol: %d from %d' % (l, len(an)))
