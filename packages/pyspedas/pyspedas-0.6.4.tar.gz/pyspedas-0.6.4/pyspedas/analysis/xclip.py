...
Parameters:
    ...
    flag:
        Values outside (ymin, ymax) are replaced with flag. Default is numpy.nan.
...
Notes:
    ...
    Similar to xclip in IDL SPEDAS.
    This function clips data. To clip times, use time_clip.
...
def xclip(names, ymin, ymax, flag=None, new_names=None, suffix=None):
    
    old_names = pyspedas.tnames(names)
    
    if len(old_names) < 1:
        print('xclip error: No pytplot names were provided.')
        return 
    ...
    for i in range(len(old_names)):
        ...
        print('xclip was applied to: '+ n_names[i])