...
class ExcelHelper:
    ...
    def load_data(self):
        ...
        self.cloumns_names = lines[0].strip().split(self.column_split)
        ...
        for item in lines:
            try:
                ...
                tmp_row = {}
                for item_index in range(0, len(columns_val)):
                    tmp_row[self.cloumns_names[item_index]] = columns_val[item_index]
                data.append(tmp_row)
            except Exception, ex:
    ...
    def get_dict_data(self, key_column_name, val_columns_name):
        ...
        for item in self.data:
            ...
            key_name = item[key_column_name]
            if dict(data_dict).has_key(key_name):
                data_dict[item[key_column_name]].append(tmp_val)
            else:
                tmp_list = [tmp_val]
                data_dict[item[key_column_name]] = tmp_list
