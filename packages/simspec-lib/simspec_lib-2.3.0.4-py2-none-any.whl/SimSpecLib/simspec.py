    ...
    def parseSetup(self):
        ...
        if self.inifile!=self.ConfDir+'default/setup.conf':
            try:
                ...
                raise self.SimSpecError('Error loading '+self.Instrument+' configuration.')
...
class SSutil:
    ...
    def InstallModule(modpackage):
        ...
        if not SimSpec.os.access(mdir, SimSpec.os.W_OK):
            raise SimSpec.SimSpecError("Insufficient file permission!! You can not install/upgrade Modules, try 'sudo'")
