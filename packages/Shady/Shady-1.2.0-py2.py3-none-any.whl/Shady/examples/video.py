...
if __name__ == '__main__':
	...
	* left-arrow/right-arrow to go back/forward one frame
	* shift + left-/right-arrow to rewind to first frame or skip ahead to last
	"""#:
	@w.EventHandler( -1 )
	def VideoKeyControl( self, event ):
		if event.type == 'key_press':
			...
			if event.key == 'left':
				...
				if 'shift' in event.modifiers: s.video.frame = 0
				else: s.video.frame -= 1
			if event.key == 'right':
				s.video.Pause()  # another syntax for setting s.video.playing = False
				if 'shift' in event.modifiers: s.video.frame = -1
				else: s.video.frame += 1	
