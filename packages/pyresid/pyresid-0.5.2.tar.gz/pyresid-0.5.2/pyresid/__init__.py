...
def query_epmc_for_IDs(query, pageSize=999, pageNumberStart=1, verbose=False):
    """"""
    ...
    if r.status_code == 200:
        ...
        if nPages > 1:
            ...
            for pageNumber in range(2, nPages + 1):
                pageNumberString = "&pageNumber=" + str(pageNumber)
                request_url = url + query + pageSizeString + pageNumberString
                if verbose:
...
def generate_EBI_output(context_matches, source, provider="West-Life", verbose=False):
    """
