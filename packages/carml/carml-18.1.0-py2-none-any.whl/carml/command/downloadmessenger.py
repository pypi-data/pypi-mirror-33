...
# XXX TODO this is just a copy-pasta of downloadbundle. refactor, "or
# something".


class DownloadMessengerOptions(usage.Options):
    ...
    def __init__(self):
        super(DownloadMessengerOptions, self).__init__()
...
class VerifyCertChainContextFactory(ssl.ClientContextFactory):
    def __init__(self, cert_chain):
        '''
        :param cert_chain:
            A dict mapping depth to
            ``twisted.internet.ssl.Certificate`` instances. Create
...
def get_download_urls(plat, arch, target_version):
    if '64' in arch:
        ...
        arch = 32
    dist_fname = dict(linux='tor-messenger-linux%d-%s_en-US.tar.xz' % (arch, target_version),
                     darwin='TorMessenger-%s-osx32_en-US.zip' % (target_version, ),
                     windows='tormessenger-install-%s_en-US.exe' % (target_version, ),
                     )[plat]
    return ('sha256sums.txt', 'sha256sums.txt.asc', dist_fname)
...
class DownloadMessengerCommand(object):
    name = 'downloadmessenger'
    ...
    build_state = False
    options_class = DownloadMessengerOptions
    ...
    def run(self, options, mainoptions, connection):
        ...
        uri = 'https://www.torproject.org/projects/tormesenger/RecommendedTBBVersions'
        data = StringIO()
        ...
        for to_download in [sig_fname, dist_fname]:
            uri = bytes('https://www.torproject.org/dist/tormessenger/%s/%s' % (target_version, to_download))
