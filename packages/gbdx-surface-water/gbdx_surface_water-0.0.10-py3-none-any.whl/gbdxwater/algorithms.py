...
def get_image(id, bbox):
    image = gb.CatalogImage(id)
    
    return image.aoi(bbox=bbox)

def ee_get_path(image, region, width=1024):
    return image.getThumbURL({'region' : region.getInfo(), 'format': 'png', 'dimensions': width})

def ee_download(image, region, path_out, width):
    path = ee_get_path(image, region, width)
