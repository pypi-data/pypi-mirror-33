...
require(['mapboxgl', 'bootstrap', 'jquery'], function(mapboxgl, bootstrap, jquery){
    ...
    map.on('load', function() {
        ...
        map.addLayer({
            "id": "waterbodies-highlighted-osm",
            "type": "line",
            'source': {
                'type': 'geojson',
                'data': dataHoverOSM
        ...
        // TODO: does not work very nice because waterbody may consist of multiple ways
        
        /*
        // select OSM feature
         map.on("click", layerOSM, function(e)  {
            const zoom = map.getZoom()
            const radius = zoom * 10
            let around = radius + ',' + e.lngLat.lat + ',' + e.lngLat.lng
            const bounds = map.getBounds()
            let bbox = bounds.getSouth() + ',' + bounds.getWest() + ',' + bounds.getNorth() + ',' + bounds.getEast()
            
            let query = 'https://www.overpass-api.de/api/interpreter?data=' +
                    '[timeout:10][out:json];(way(around:' + around + '););out tags geom(' + bbox + ');(relation(around:' + around + '););out tags geom(' + bbox + ');';
            ...
            $.ajax({
                ...
                if(elements.length) {
                    ...
                    dataHoverOSM.features[0].geometry.coordinates = []
                    
                    geometry.map(function(latLon) {
                        if(latLon) {
                            dataHoverOSM.features[0].geometry.coordinates.push([latLon.lon, latLon.lat]);
                        }
                    })
                    
                    map.getSource('waterbodies-highlighted-osm').setData(dataHoverOSM);
                }
