...
def parseConfig(args):
	...
	try:
		opts, arg = getopt.getopt(args, "hm:ql:s:xy", ["help", "metadata=", "quiet", "fft-length=", "samples=", "xx", "yy"])
	...
	for s in xrange(ChunkSize):
		...
		for pol in pols:
			...
			for k in xrange(nSec):
				blList, freq, vis = fxc.FXMaster(data[:,k*secSize:(k+1)*secSize], mapper2, LFFT=LFFT, Overlap=Overlap, IncludeAuto=True, verbose=False, SampleRate=SampleRate, CentralFreq=CentralFreq, ReturnBaselines=True, GainCorrect=True)
