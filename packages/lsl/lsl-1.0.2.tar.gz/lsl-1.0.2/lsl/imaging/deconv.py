...
def deconvolve(aa, dataDict, aipyImg, MapSize=80, MapRes=0.50, MapWRes=0.10, chan=None, gain=0.3, maxIter=150, scales=[0,], verbose=True):
	...
	beamClean = estimateBeam(aa, 0.0, aa.lat*180/numpy.pi, MapSize=MapSize, MapRes=MapRes, MapWRes=MapWRes, chan=chan, baselines=dataDict['bls']['xx'])
...
def buildSky(aa, jd, MapSize=80, MapRes=0.50, MapWRes=0.10):
	...
	pmap = skymap.ProjectedSkyMap(smap, aa.lat*180.0/numpy.pi, aa.long*180.0/numpy.pi, aa.get_jultime())
	
	az  = pmap.visibleAz*numpy.pi/180.0
	alt = pmap.visibleAlt*numpy.pi/180.0
	pwr = pmap.visiblePower
	
	img = ImgW(MapSize, MapRes, MapWRes)
	iTop = img.get_top(center=(MapSize,MapSize))
	iAz,iAlt = top2azalt(iTop)
	...
	for i in xrange(iAz.shape[0]):
		for j in xrange(iAlt.shape[1]):
			...
			output[i,j] += pwr[diff.argmin()] * numpy.sin(iAlt[i,j])**1.5
...
def mem(aa, dataDict, aipyImg, MapSize=80, MapRes=0.50, MapWRes=0.10, chan=None, maxIter=150, verbose=True):
	...
	psfSrcs = {'z': RadioFixedBody(aa.sidereal_time(), aa.lat, jys=1.0, index=0)}
		...
		for i in xrange(b_i.shape[0]):
			for j in xrange(b_i.shape[1]):
				if dec.mask[i,j]:
					continue
				nm = '%i-%i' % (i,j)
				bSrcs[nm] = RadioFixedBody(ra[i,j], dec[i,j], name=nm, jys=b_i[i,j], index=0)
