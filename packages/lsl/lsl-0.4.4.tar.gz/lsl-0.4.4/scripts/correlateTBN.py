...
def parseConfig(args):
	...
	try:
		opts, arg = getopt.getopt(args, "hm:qc:l:t:s:o:xy", ["help", "metadata=", "quiet", "central-freq=", "fft-length=", "avg-time=", "samples=", "offset=", "xx", "yy"])
...
def processChunk(fh, site, good, filename, intTime=6.0, LFFT=64, Overlap=1, CentralFreq=49.0e6, SampleRate=dp_common.fS, pols=['xx',], ChunkSize=300):
	...
	for s in xrange(ChunkSize):
		...
		for pol in pols:
			print "->  %s" % pol
			blList, freq, vis = fxc.FXMaster(data, mapper2, LFFT=LFFT, Overlap=Overlap, IncludeAuto=True, verbose=False, SampleRate=SampleRate, CentralFreq=CentralFreq, ReturnBaselines=True, GainCorrect=True)
...
def main(args):
	...
	# Select those with the right polarization
