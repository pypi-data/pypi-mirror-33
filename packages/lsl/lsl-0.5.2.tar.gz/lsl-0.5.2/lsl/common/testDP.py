...
from lsl.common.dp import fS, SoftwareDP, _delayFIRs


def getDelaysCourseFine(antennas, frequency, az, el):
	# Get delays
	delays = beamformer.calcDelay(antennas, freq=frequency, azimuth=float(az), elevation=float(el))
	delays *= 1e9
	...
	# FIR delays (~0.3 ns)
	samples = numpy.round(delays * fS * 16 / 1e9)
...
def main(args):
	...
	time = numpy.arange(500000, dtype=numpy.int64)
	...
	gains = []
	for i in xrange(len(antennas)/2):
		ant1 = antennas[2*i+0]
		ant2 = antennas[2*i+1]
		#if ant1.stand.id == stand1:
			#gains.append([1, 0, 0, 0])
		#elif ant1.stand.id == stand2:
			#gains.append([0, 0, 1, 0])
		#else:
			#gains.append([0, 0, 0, 0])
		if ant1.getStatus() >= 23 and ant2.getStatus() >= 23:
			gains.append([1, 0, 0, 1])
		else:
			gains.append([0, 0, 0, 0])
	...
	for i in xrange(outputX.shape[0]):
		...
		for j in xrange(outputX.shape[1]):
			...
			courseDelays, fineDelays = getDelaysCourseFine(antennas, 74.0e6, az, el)
			
			dp = SoftwareDP(mode='DRX', filter=7, centralFreq=74.0e6)
			beamX, beamY = dp.formBeam(antennas, time, data, courseDelays=courseDelays, fineDelays=fineDelays, gains=gains)
			drxX = dp.applyFilter(time[:len(beamX)], beamX)
			drxY = dp.applyFilter(time[:len(beamY)], beamY)
