...
def cli(config, verbose):
    changed = False
    if config.settings.token == "" or config.settings.token == None:
...
def findGist(config):
    ...
    if len(configurations) == 0:
        click.echo("You have no configurations in your Gist repo. First perform 'rc up'")
        exit()
    ...
    if len(configurations) > 1:
        ...
        if index < len(configurations) or index > (len(configurations)):
