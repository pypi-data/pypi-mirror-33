...
class CommandV(Extensible):
    ...
    def build_image(self):
        ...
        res = build_helper.build_image(self.__config)
        ...
        tag_img = '{img}:{tag}'.format(
            img=res.get('image_name'), tag=res.get('image_sha'))
    ...
    def restore(self):
        ...
        if self.__config.instance_config.get('install_module'):
            install_res = self._install_module(database_name)
            res['result'].update({
    ...
    def _check_update_production_requirements(self):
        ...
        if not db_conn:
            self.sender.send_message('Failed to connect to the database', log_type='ERROR')
            return False
        logger.debug('Postgres config check successful')
        # Check free space
        free_space = self._check_free_space()
        if free_space.get('error'):
            self.sender.send_message(free_space['error'], log_type='ERROR')
            return False
        ...
        if not os.path.isdir(os.path.expanduser(working_folder)):
            self.sender.send_message(
                'The {folder} working folder does not exist.'.format(folder=working_folder),
                log_type='ERROR')
            return False
        ...
        try:
            ...
            self.sender.send_message('Image "{img}" doesn\'t exist'.format(img=image),
                                     log_type='ERROR')
            return False
        return True
