...
from pykemen.utilities import create_api, verboseprint
import pandas as pd
...
class Analytics(object):
    ...
    def get_report(self, filename, data_frame=False, **kwargs):
        ...
        verboseprint(Analytics.VERBOSE,
                     "Saved file " + filename.replace(".csv", "_{frm}_{to}.csv".format(frm=kwargs.get('start_date'),
                                                                                       to=kwargs.get('end_date'))))
    ...
    def get_unsampled_report(self, **kwargs):
        ...
        columns = ",".join([kwargs.get('dimensions', ''), kwargs.get('metrics', ''), kwargs.get('filters', ''),
                            kwargs.get('sort', '')])
        id = hashlib.md5(columns.encode('utf-8')).hexdigest()
        ...
        for day in range(diff_days):
            actual_date = (start_date + timedelta(days=day)).strftime("%Y-%m-%d")
            filename = Analytics.CACHE_REPORT.format(profile=kwargs.get('ids').replace('ga:', ''), id=id,
                                                     date=actual_date)
            if not self._in_cache(kwargs.get('ids').replace('ga:', ''), id, actual_date):
                kwargs["start_date"] = actual_date
                kwargs["end_date"] = actual_date
                ...
                if report.get("containsSampledData"):
                    warnings.warn("There are sampled results on the report: {dimensions}{metrics} - date{date}".format(
                        dimensions=kwargs.get("dimensions"), metrics=kwargs.get("metrics"), date=actual_date))
                rows.extend(report.get("rows", []))
                iteration = 1
                while report.get("nextLink"):
                    try:
                    ...
                    verboseprint(Analytics.VERBOSE, "Saved file " + filename)
                    rows = []
        ...
        for reports in os.listdir('./cache/{id}/'.format(id=id_)):
            for report in os.listdir('./cache/{id}/{reports}/'.format(id=id_, reports=reports)):
                ...
                if day < last_day:
                    ...
                    verboseprint(Analytics.VERBOSE,
                                 'Removed cache/{id}/{reports}/{report}'.format(id=id_, reports=reports, report=report))
