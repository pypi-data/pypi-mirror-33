...
class DNN:
    ...
    def to_tfl (self, path, saved_model_dir, quantized_input_stats = None, default_ranges_stats = (-1, 1)):
        from tensorflow.contrib.lite.python.convert import tensor_name
        
        resolved_quantized_input_stats = {}
        for k, v in quantized_input_stats.items ():
            resolved_quantized_input_stats [tensor_name (k)] = v # [mean, std]
                        
        converter = tf.contrib.lite.TocoConverter.from_saved_model (saved_model_dir)
        if quantized_input_stats:
            converter.inference_type = tf.contrib.lite.constants.QUANTIZED_UINT8
            converter.quantized_input_stats = resolved_quantized_input_stats    
