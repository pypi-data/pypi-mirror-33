...
class DNN:
    ...
    def train (self, x, y, **kargs):
        self.is_validating = False
        self.is_improved = False
        r = self.evaluate (x, y, True, **kargs)
    ...
    def gaussian_encode (self, input, n_output):
        ...
        self.__mu = gaussian_params [:, :n_output]
        self.__sigma = 1e-6 + tf.nn.softplus (gaussian_params[:, n_output:])
        return self.__mu + self.__sigma * tf.random_normal(tf.shape(self.__mu), 0, 1, dtype=tf.float32)        
    ...
    def vae_loss (self, output, target):
        # https://github.com/hwalsuklee/tensorflow-mnist-VAE/blob/master/vae.py
        marginal_likelihood = tf.reduce_sum(output * tf.log(target) + (1 - output) * tf.log(1 - target), 1)
        KL_divergence = 0.5 * tf.reduce_sum(tf.square(self.__mu) + tf.square(self.__sigma) - tf.log(1e-8 + tf.square(self.__sigma)) - 1, 1)
