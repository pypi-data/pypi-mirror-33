...
.. code-block:: r
    ...
    # Note that reticulate doesn't cope with dotted package names:
    cardinal_pythonlib <- reticulate::import("cardinal_pythonlib")  # works
    cpl_drugnames <- reticulate::import("cardinal_pythonlib.drugnames")  # FAILS
    cpl_fileops <- reticulate::import("cardinal_pythonlib.fileops")  # FAILS
    ...
    # Or, for development:
    cpl_drugs <- reticulate::import_from_path("drugs", "~/Documents/code/cardinal_pythonlib/cardinal_pythonlib/psychiatry")
...
class Drug(object):
    def __init__(
        ...
        wildcard = ".*"  # see re.DOTALL below, which makes this match newlines
        for p in list(set(self.all_generics + self.alternatives)):
            # We add front/back wildcards by default; this handles all
            # situations like "depot X", etc.
