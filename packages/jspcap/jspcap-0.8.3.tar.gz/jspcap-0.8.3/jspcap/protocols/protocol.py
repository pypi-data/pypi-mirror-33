...
class Protocol:
    ...
    def _check_termination(self):
        ...
        iterable = self._exproto if isinstance(self._exproto, tuple) else (self._exproto or '',)
        match = filter(lambda string: re.fullmatch(pattern, string, re.IGNORECASE), iterable)
        return (len(list(match)) or re.fullmatch(self.__layer__ or '', self._exlayer or '', re.IGNORECASE))
