...
from jspcap.utilities.warnings import FormatWarning
...
class Extractor:
    ...
    def __init__(self, *,
        ...
        self._exlyr = layer             # extract til layer
        self._exptl = protocol          # extract til protocol
        ...
        if not self._flag_q:
            if fmt == 'plist':
                ...
                from jspcap.dumpkit import NotImplementedIO as output
                                                            # no output file
                warnings.warn(f'Unsupported output format: {fmt}', FormatWarning, stacklevel=stacklevel())
