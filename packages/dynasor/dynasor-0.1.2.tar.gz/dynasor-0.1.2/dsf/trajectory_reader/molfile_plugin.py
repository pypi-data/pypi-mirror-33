...
from ctypes import (
    ...
    Structure, cast, pointer, c_int, c_uint, c_float, c_double,
...
class molfile_plugin_t(Structure):
    ...
    _fields_ = [
        ...
        # int (* write_structure)(void *, int optflags,
        ...
        ('read_timestep', CFUNCTYPE(c_int, c_void_p, c_int,
                                    POINTER(molfile_timestep_t),
        ...
        ('read_timestep_metadata',
            CFUNCTYPE(c_int, c_void_p, POINTER(molfile_timestep_metadata_t))),
        # int (* read_qm_timestep_metadata)(void *,
...
class _MolfilePlugin(object):
    ...
    def _load_plugin_library(self, plugin_path):
        ...
        library.vmdplugin_register.argtypes \
            = (c_void_p, vmdplugin_register_cb_t)
        ...
        def py_register_callback(v, p):
            ...
            if contents.type == MOLFILE_PLUGIN_TYPE:
                if contents.abiversion >= MIN_ABI_VERSION:
                    plugin_p.contents \
                        = cast(p, POINTER(molfile_plugin_t)).contents
...
def _molfile_machine_specific_plugin_dir_name():
    ...
    raise RuntimeError('No known {}-bit plugin-path on {} system'
                       .format(bits, _system))
...
def _vmddir():
    ...
    for tentive_vmddir in ['C:\\Program Files (x86)\\University of Illinois\\VMD']:  # noqa
...
def _get_vmddir_from_vmd_script(vmd_script_path):
    with open(vmd_script_path) as fh:
        for L in islice(fh, 10):
            match = re.match(r'^(?:set )?defaultvmddir=(?:"(/.*)"|(/[^# ]*)).*$', L)  # noqa
            if match:
                a, b = match.groups()
                return a or b
