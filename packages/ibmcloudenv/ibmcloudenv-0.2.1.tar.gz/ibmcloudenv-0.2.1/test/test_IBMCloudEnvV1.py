...
class TestParser(unittest.TestCase):
    ...
    @mock.patch.dict(os.environ, {'VCAP_SERVICES': '{"service1": [{"name": "service1-name1","credentials": {"username": "service1-username1"}},{"name": "service1-name2","credentials": {"username": "service1-username2"}}],"user-provided": [{"name": "serviceUserProvided1", "credentials": {"username": "service-username1"}}, {"name": "serviceUserProvided2", "credentials": {"cred": {"password": "service-password1"}}}]}',
        'VCAP_APPLICATION': '{"application_name": "test-application"}',
        'ENV_VAR_STRING': 'test-12345',
        'ENV_VAR_JSON': '{"credentials": {"username": "env-var-json-username"}}' })
    ...
    def test_file_user_provided_with_service_name(self):
        self.assertEqual(IBMCloudEnv.getString("user_provided_var1"), "service-username1")
        self.assertTrue(type(IBMCloudEnv.getDictionary("user_provided_var1")), dict)
        self.assertTrue("value" in IBMCloudEnv.getDictionary("user_provided_var1"))
        self.assertEqual(IBMCloudEnv.getDictionary("user_provided_var1")["value"], "service-username1")

    def test_file_user_provided_with_nested_service(self):
        self.assertEqual(IBMCloudEnv.getString("user_provided_var2"), "service-password1")
        self.assertTrue(type(IBMCloudEnv.getDictionary("user_provided_var2")), dict)
        self.assertTrue("value" in IBMCloudEnv.getDictionary("user_provided_var2"))
        self.assertTrue(type(IBMCloudEnv.getDictionary("user_provided_var2")["value"]), dict)
        self.assertEqual(IBMCloudEnv.getDictionary("user_provided_var2")["value"], "service-password1")
