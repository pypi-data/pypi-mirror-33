    ...
    def renew(self, book):
        ...
        for checkout in checkouts.find_all('div', lambda class_: class_ and class_.startswith('listItem')):
            if checkout.find(class_='title title_extended').text == book.title:
                self.session.get('https://sfpl.bibliocommons.com/{}'.format(
                ...
                r = self.session.post('https://sfpl.bibliocommons.com/checkedout/renew', data={
                    'authenticity_token': checkouts.find('input', {'name': 'authenticity_token'})['value'],
                    'items[]': checkout.find(class_='btn btn-link single_circ_action')['href'].split('/')[3]
                }, headers={
                    'X-Requested-With': 'XMLHttpRequest'
                })
                return r
