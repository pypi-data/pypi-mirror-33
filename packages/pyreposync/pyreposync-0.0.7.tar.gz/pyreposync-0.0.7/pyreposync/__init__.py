...
class PyRepoSync(object):
    ...
    def work(self):
        ...
        workers = set()
        print("starting, check {0} for details".format(self.config.get('main', 'log')))
        if self.method == 'sync':
    ...
    def do_snap_list(self, jobs):
        for job in jobs:
            try:
                ...
                print(job.reponame)
                print("The following timestamp snapshots exist:")
                for timestamp in job.snap_list_timestamp_snapshots():
                    print("{0} -> {1}".format(timestamp, referenced_timestamps.get(timestamp, [])))
                print("The following named snapshots exist:")
                base = "{0}/snap/{1}/".format(job.destination, job.reponame)
                for named in job.snap_list_named_snapshots():
                    named = "named/{0}".format(named)
                    print("{0} -> {1}".format(named, job.snap_list_named_snapshot_target("{0}/{1}".format(base, named))))
                latest = "{0}/latest".format(base)
                print("latest -> {0}".format(job.snap_list_named_snapshot_target(latest)))
