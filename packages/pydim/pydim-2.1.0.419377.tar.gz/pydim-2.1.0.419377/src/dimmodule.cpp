...
void _dic_info_service_dummy (void* tag, void* buffer, int* size) {
  ...
  */
  if(svc->format == NULL) {
    format = dic_get_format(0); /*returns NULL if current service doesn't exists*/
    if(format != NULL){
      /*format is found, service exists*/
      svc->format = (char *)malloc(sizeof(char) * strlen(format));
      strcpy(svc->format,format);
    }
  }
