...
DETAILS_ENDPOINT = urljoin(BASE_API_URL, "maps/api/place/details/json")
NEARBY_ENDPOINT = urljoin(BASE_API_URL, 'maps/api/place/nearbysearch/json?location={latitude},{longitude}&radius={radius}&type=bar&key={api_key}&pagetoken={pagetoken}')
...
def get_place_details(api_key, place_id, **kwargs):
    ...
    resp = requests.get(url=DETAILS_ENDPOINT, params=params)

    if resp.status_code >= 300:
        raise Exception('Bad status code rerieved from google api')

    data = json.loads(resp.text)
    check_response_code(data)   # check api response status codess
...
def get_places_by_geo_radius(api_key, latitude, longitude, radius=500, types=[], **kwargs):
    ...
    while pagetoken:
        ...
        resp = requests.get(url=NEARBY_ENDPOINT, params=params)
        if resp.status_code >= 300:
            raise Exception('Bad status code rerieved from google api')
