...
class AChart(AElementMQTT):
    ...
    def __init__(self, config, update_available, mqtt_client, logger, logger_name):
        ...
        if self._group_by < 0:
            self._logger.error("SequentialChart.__init__ - 'group-by' must be none-negative. ({})".format(self._group_by))
            raise ValueError("SequentialChart.__init__ - 'group-by' must be none-negative. ({})".format(self._group_by))
        try:
            if self._config["aggregator"] == "avg":
                ...
                self._logger.error("SequentialChart.__init__ - unknown aggregator '{}'.".format(self._aggregator))
                raise ValueError("SequentialChart.__init__ - unknown aggregator '{}'.".format(self._aggregator))
        except ValueError:
            if self._group_by>0:
                self._logger.error("SequentialChart.__init__ - no aggregator provided. must be set if group-by>0.")
                raise ValueError("SequentialChart.__init__ - no aggregator provided. must be set if group-by>0.")
        ...
        if self._chart_pixel_per_value <= 0:
            self._logger.error("SequentialChart.__init__ - 'pixel-per-value' must be > 0 ('{}').".
                               format(self._chart_pixel_per_value))
            raise ValueError("SequentialChart.__init__ - 'pixel-per-value' must be > 0 ('{}').".
                             format(self._chart_pixel_per_value))
        elif self._chart_pixel_per_value >= self._width:
            self._logger.error("SequentialChart.__init__ - 'pixel-per-value' ({}) must be smaller than the width ({}).".
                               format(self._chart_pixel_per_value, self._width))
            raise ValueError("SequentialChart.__init__ - 'pixel-per-value' ({}) must be smaller than the width ({}).".
                             format(self._chart_pixel_per_value, self._width))
    ...
    def _topic_sub_handler(self, value):
        ...
        """
        value = float(value)
        self._logger.info("SequentialChart._topic_sub_handler - received value '{}'.".format(value))
        t = time.time()
    ...
    def _aggregation_to_history(self):
        ...
        self._logger.info("SequentialChart._aggregation_to_history - add value: {}, len history: {}".
    ...
    def update_image(self):
        ...
        self._update_image(draw, min_history, max_history)
        self._logger.debug("AChart.updateImage - done.")

    def _update_image(self, draw, min_history, max_history):
        """
        Update image method for silblings of AChart

        :param draw: PIL Image draw instance
        :param min_history: minimum value for the chart
        :param max_history: maximum value for the chart
        """
