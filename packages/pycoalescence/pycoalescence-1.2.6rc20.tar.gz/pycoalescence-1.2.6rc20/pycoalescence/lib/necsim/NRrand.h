...
//		return pow(pow(distx, 2) + pow(disty, 2), 0.5);
	...
	double rayleighMinDist(const double &dist)
	{
		...
		double rand_num = d01();
		double rand_prob = (min_prob * (rand_num - 1) + 1) / rand_num;
		// TODO move to DEBUG and alter
		double out = sigma * pow(-2 * log(rand_prob), 0.5);
		if(out < dist)
		{
			throw runtime_error("Out distance is less than input distance during rayleigh min dist - please report "
								 "this bug");
		}
		return out;
	}
	...
	{
		return 1 - exp(pow(-dist, 2) / (2 * pow(sigma, 2)));
	}
