...
class Reconstructor:
    ...
    def full_reconstruction_nested(self,division_number,alpha,Resolution_full, Resolution_hom,beta,starts_all = None,rank=0,box_sizes_return = False,box_size_total=100.0, N_fake_particle=0):
        ...
        box_sizes=np.array([])
        redshift_mins = np.array([])
        redshift_maxs = np.array([])
        for i, start in enumerate(starts_all[:,0:3]):
            ...
            redshift_mins = np.append(redshift_mins, np.asarray([z_min] * len(ID)))  # to keep track of the redshift slice in which a particle is
            redshift_maxs = np.append(redshift_maxs, np.asarray([z_max] * len(ID)))  # to keep track of the redshift slice in which a particle is

        if box_sizes_return == True:
            return X_r_tot,ID_tot, box_sizes, redshift_mins, redshift_maxs
        else:
            return X_r_tot, ID_tot, redshift_mins, redshift_maxs
