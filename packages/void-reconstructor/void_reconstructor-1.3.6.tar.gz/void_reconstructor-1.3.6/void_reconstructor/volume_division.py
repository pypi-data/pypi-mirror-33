...
def volume_division(Reconstruction,Resolution, division_number,beta,starts_all,rank,N_lim=128**3, flag = False):
    ...
    for start in starts_all[:,0:3]:
        ...
        if N_part > N_lim:
            ...
            starts_sub = np.append(starts_sub, box_length, axis=1) #box_length_x
            starts_sub = np.append(starts_sub, box_length, axis=1) #box_length_y
            starts_sub = np.append(starts_sub, box_length, axis=1) #box_length_z
            starts_all_new = np.append(starts_all_new, starts_sub, axis=0)
            flag = True
        else:
            starts_sub = np.append(start, box_size)
            starts_sub = np.append(starts_sub, box_size) #box_length_y
            starts_sub = np.append(starts_sub, box_size) #box_length_z
            starts_sub = np.reshape(starts_sub, (1, 6))
            starts_all_new = np.append(starts_all_new, starts_sub, axis=0)
