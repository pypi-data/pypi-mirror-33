...
if prefix is not None and len(prefix) > 0 and not prefix.endswith('/'):
  prefix = prefix + '/'
if cprefix is not None and len(cprefix) > 0 and not cprefix.endswith('/'):
  cprefix = cprefix + '/'
...
if sys.platform == 'darwin' and packaging:
  ...
  for plugin in plugin_dirs:
    if prefix is None:
      plugin_data_files.append((cprefix + "share/diamond/plugins/" + plugin,
        glob.glob('plugins/' + plugin + '/*.py')))
    else:
...
if sys.platform == 'darwin' and packaging:
  ...
  if prefix is None:
    gui_data_files.append((cprefix + "share/diamond/gui",
      ["gui/gui.glade", "gui/diamond.svg"]))
  else:
    gui_data_files.append((prefix + "share/diamond/gui",
      ["gui/gui.glade", "gui/diamond.svg"]))

# Now parse the schema directories
schema_data_files = []
if sys.platform == 'darwin' and packaging:
  schema_data_files.append(("./schema",
    ["schema/spud_base.rnc", "schema/spud_base.rng"]))
else:
  if prefix is None:
    schema_data_files.append((cprefix + "share/diamond/schema",
      ["schema/spud_base.rnc", "schema/spud_base.rng"]))
  else:
    schema_data_files.append((prefix + "share/diamond/schema",
      ["schema/spud_base.rnc", "schema/spud_base.rng"]))
...
setup(
      ...
      scripts=["bin/diamond"],
      data_files = schema_data_files + gui_data_files + plugin_data_files
          + [('', ['README.txt', 'COPYING', 'GPL-3'])],
      long_description="""
Diamond  (spud-diamond)
=======================

A model option preprocessor.
