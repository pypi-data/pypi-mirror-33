...
class ObjDetect(TensorFlowKernel):
...
@autobatch(uniforms=[0, 'nms_threshold', 'custom_output'])
def detect_objects(db, videos, frames=None, nms_threshold=None, custom_output=None):
    """
    ...
    if custom_output is None:
        ...
        output_args = [v.scanner_name() + '_objdet' for v in videos]
    else:
        (output, output_args) = custom_output
        output = output(bboxes)
    ...
    jobs = [
        Job(
            op_args={
                ...
                output: oa
            }) for v, f, oa in zip(videos, frames or [None for _ in range(len(videos))], output_args)
    ]
