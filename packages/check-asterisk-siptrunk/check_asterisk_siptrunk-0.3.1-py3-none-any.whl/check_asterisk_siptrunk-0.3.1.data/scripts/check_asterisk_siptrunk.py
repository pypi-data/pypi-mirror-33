...
class AsteriskSiptrunkContext(nagiosplugin.Context):
    ...
    def evaluate(self, metric, resource):
        if 'OK' in metric.value:
            ...
            return self.result_cls(nagiosplugin.state.Critical, metric=metric, hint=metric.value)


class AsteriskSiptrunkSummary(nagiosplugin.Summary):

    def ok(self, results):
        return results[0].resource.peer + ' is ' + str(results[0].metric)
        ...
        # connect to the manager
            try:
                ...
                if response.headers.get('Response') != 'Success':
                    ...
                    yield nagiosplugin.Metric('AsteriskSiptrunk', status, context='asterisksiptrunk')
