...
#         print("Historical Data loaded in: %s" %(datetime.now() - self.start_time))
        ...
        for asset in response['payload']:
            ...
            self.symbols[asset] = self._process_df_columns(self.symbols[asset])
...
#                     self.symbols[symbol] = self._process_df_columns(self.symbols[symbol])



    def _append(self, message):
        tick_df = pd.DataFrame(message, index=[message["index"]])
        tick_df = self._process_df_columns(tick_df)
