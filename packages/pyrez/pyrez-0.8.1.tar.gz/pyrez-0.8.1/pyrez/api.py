...
from pyrez.models import APIResponse, HiRezServerStatus, Champion, ChampionSkin, God, DataUsed, PlayerPaladins, PlayerStatus, PlayerSmite, Friend, GodRank, Session, PatchInfo, PaladinsItem, SmiteItem, PlayerLoadouts, MatchHistory, PlayerRealmRoyale
...
class BaseAPI: #BaseAPI (HttpRequest):
    ...
    def __init__ (self, devKey: int, authKey: str, endpoint: str, responseFormat = ResponseFormat.JSON, language = LanguageCode.ENGLISH):
        if not devKey or not authKey:
            ...
            self.__responseFormat__ = int (responseFormat) if (responseFormat == ResponseFormat.JSON or responseFormat == ResponseFormat.XML) else int (ResponseFormat.JSON)
...
class HiRezAPI (BaseAPI):
    ...
    def getMatchIdsByQueue (self, queueID: int, date, hour = -1: str):
        return self.makeRequest ("getmatchidsbyqueue", [queueID, date.strftime ("%Y%m%d") if isinstance (date, datetime) is else date, hour])
    ...
    def getPlayer (self, playerID):
        if (len (playerID) <= 3):
            ...
            if isinstance (self, RealmRoyaleAPI):
                ...
                return PlayerRealmRoyale (** self.makeRequest ("getplayer", [playerID, plat]) [0])
            else:
