...
def Predict (was, request, timeout = 10):
	...
	for k, v in request.inputs.items ():
		tensor_name, tensor, dtype, shape = sess.input_map [signature_name][k]
	...
	for i, result in enumerate (sess.outputs [signature_name]):
		predict_result = predict_results [i]		
		response.outputs [sess.outputs [signature_name][i][0]].CopyFrom (
...
def predict (was, **inputs):
	...
	for i, result in enumerate (sess.outputs [signature_name]):
		predict_result = predict_results [i]
		response [sess.outputs [signature_name][i][0]] = predict_result.astype ("float32").tolist ()		
