...
class TokenRepository:
    ...
    def logout(token):
        try:
            unsigned_token = signing.loads(token, max_age=settings.TOKEN_EXPIRY, salt='access_token')
    ...
    def refresh_token(user, token, refresh_token):
        try:
            unsigned_token = signing.loads(token, max_age=settings.REFRESH_TOKEN_EXPIRY, salt='access_token')
            token_id = unsigned_token['tokenId']
            auth_token = AuthToken.objects.filter(id=token_id).filter(refresh_key=refresh_token).get()
        except signing.SignatureExpired:
            raise ValidationError('Expired token.', field='token')
        except signing.BadSignature:
            raise ValidationError('Invalid token.', field='token')
...
class ResetRepository:
    ...
    def reset_password(email):
        ...
        reset_token = signing.dumps({'userId': str(user.id),
                                     'rand': get_rand_str(16)}, salt=ResetRepository.namespace, compress=True)
...
class MailVerificationRepository:
    ...
    def send_email_validation(user):
        ...
        val_token = signing.dumps({'userId': str(user.id),
                                   'rand': get_rand_str(16)}, salt=MailVerificationRepository.namespace)
