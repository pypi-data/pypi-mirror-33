...
db = peewee.SqliteDatabase("caduceus.db")
...
def load_config() -> dict:
    try:
        configstr = open("caduceus.toml").read()
    except:  # noqa - We don't care *why* this failed.
        app.logger.error("Could not read configuration file.")
        sys.exit(1)
...
def notify(subject: str, body: str, config: dict):
    msg = MIMEText(body)
    msg["Subject"] = subject
    ...
    s = smtplib.SMTP()
    s.connect(host=smtp_conf["hostname"], port=smtp_conf["port"])
...
in every %s seconds, but we haven't seen it for longer than that.
...
def check_and_notify_fixed(alert_id: str, config: dict):
