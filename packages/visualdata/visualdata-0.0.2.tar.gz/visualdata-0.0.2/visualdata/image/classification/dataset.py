...
class ImgClfDatasetInfoKey(DatasetInfoKeyBase):
  """Data info key name.
  """
...
class ImgClfDataSetInfo(DataSetInfoBase):
  ...
  def __init__(self, save_dir, multi_label=False):
    ...
    super(ImgClfDataSetInfo, self).__init__(info_fn)
    if os.path.exists(info_fn):
      self.load_info()
    # add new init code.
    self._ds_info[ImgClfDatasetInfoKey.NAME] = "dataset"
    self._ds_info[ImgClfDatasetInfoKey.MULTI_LABEL] = multi_label
    self._ds_info[ImgClfDatasetInfoKey.SAVE_DIR] = save_dir

  def get_value(self, key_name):
    if key_name in [
        ImgClfDatasetInfoKey.LABEL_ID_TO_NAME,
        ImgClfDatasetInfoKey.LABEL_NAME_TO_ID
    ]:
      return self._ds_info[ImgClfDatasetInfoKey.LABELS][key_name]
    else:
      return super(ImgClfDataSetInfo, self).get_value(key_name)
  ...
  def set_value(self, key_name, val):
    if key_name in [
      ...
      self._ds_info[ImgClfDatasetInfoKey.LABELS][key_name] = val
    else:
      super(ImgClfDataSetInfo, self).set_value(key_name, val)
...
class ImgClfDataset(DatasetBase):
  ...
  def __init__(self, save_dir, ds_info_=None):
    ...
    """
    if ds_info_ is None:
      self.ds_info = ImgClfDataSetInfo(save_dir)
    else:
  ...
  def is_multi_label(self):
    ...
    """
    return self.ds_info.get_value(ImgClfDatasetInfoKey.MULTI_LABEL)
  ...
  def split_metadata(self, meta_fn, train_ratio=0.7, target_label_col=1):
    ...
    # save mappers.
    self.ds_info.set_value(ImgClfDatasetInfoKey.LABEL_ID_TO_NAME, label_names)
    ...
    self.ds_info.set_value(ImgClfDatasetInfoKey.TRAIN_META_FN, "train.csv")
    ...
    self.ds_info.set_value(ImgClfDatasetInfoKey.TEST_META_FN, "test.csv")
