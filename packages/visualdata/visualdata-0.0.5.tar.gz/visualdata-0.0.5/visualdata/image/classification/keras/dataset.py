...
class Dataset(ImgClfDataset):
  ...
  def build_from_folder(self,
    ...
    # convert to hdf5.
    self.build_from_metadata(commons.DataType.TRAIN, target_img_height,
                             target_img_width)
    self.build_from_metadata(commons.DataType.TEST, target_img_height,
                             target_img_width)
  ...
  def gen_batch_data(self,
    ...
    while True:
      ...
      with h5py.File(data_fn, "r", libver="latest") as f:
        img_dset = f["images"]
        label_dset = f["labels"]
