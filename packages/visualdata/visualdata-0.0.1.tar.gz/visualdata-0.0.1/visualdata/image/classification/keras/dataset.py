...
from visualdata.image.classification.dataset import ImgClfDatasetInfoKey, ImgClfDataset
...
class Dataset(ImgClfDataset):
  ...
  def __init__(self, save_dir, ds_info=None):
    ...
    self.ds_info.set_value(ImgClfDatasetInfoKey.ENGINE, "keras")
  ...
  def build_from_metadata_single_label(self,
    ...
    save_dir = self.ds_info.get_value(ImgClfDatasetInfoKey.SAVE_DIR)
    data_fn = ""
    if data_type == commons.DataType.TRAIN:
      data_fn = os.path.join(save_dir, "train.h5")
      self.ds_info.set_value(ImgClfDatasetInfoKey.TRAIN_DATA_FN, data_fn)
    else:
      data_fn = os.path.join(save_dir, "test.h5")
      self.ds_info.set_value(ImgClfDatasetInfoKey.TEST_DATA_FN, data_fn)
    if not os.path.exists(data_fn):
      ...
      self.ds_info.set_value(ImgClfDatasetInfoKey.LABEL_ID_TO_NAME,
                             label_names)
      self.ds_info.set_value(ImgClfDatasetInfoKey.LABEL_NAME_TO_ID, label_ids)
      ...
      with h5py.File(data_fn, "w", libver="latest") as f:
        ...
        print("metadata {} has been converted to data file: {}".format(
            meta_fn, data_fn))
        if data_type == commons.DataType.TRAIN:
          self.ds_info.set_value(ImgClfDatasetInfoKey.TRAIN_SAMPLE_COUNT,
                                 img_dset.len())
        else:
          self.ds_info.set_value(ImgClfDatasetInfoKey.TEST_SAMPLE_COUNT,
                                 img_dset.len())
  ...
  def build_from_folder(self, data_folder, train_ratio=0.8):
    ...
    # create metadata.
    vals = self.gen_metadata_from_folder(data_folder, train_ratio=train_ratio)
    self.ds_info.set_value(ImgClfDatasetInfoKey.TRAIN_META_FN, vals[0])
    self.ds_info.set_value(ImgClfDatasetInfoKey.TEST_META_FN, vals[1])
    self.ds_info.set_value(ImgClfDatasetInfoKey.LABEL_ID_TO_NAME, vals[2])
    self.ds_info.set_value(ImgClfDatasetInfoKey.LABEL_NAME_TO_ID, vals[3])
    # convert to hdf5.
    self.build_from_metadata(
        self.ds_info.get_value(ImgClfDatasetInfoKey.TRAIN_META_FN),
        commons.DataType.TRAIN, 224, 224)
    self.build_from_metadata(
        self.ds_info.get_value(ImgClfDatasetInfoKey.TEST_META_FN),
        commons.DataType.TEST, 224, 224)
  ...
  def gen_batch_data(self,
    ...
    samp_count = 0
    if data_type == commons.DataType.TRAIN:
      data_fn = self.ds_info.get_value(ImgClfDatasetInfoKey.TRAIN_DATA_FN)
      samp_count = self.ds_info.get_value(
          ImgClfDatasetInfoKey.TRAIN_SAMPLE_COUNT)
    else:
      data_fn = self.ds_info.get_value(ImgClfDatasetInfoKey.TEST_DATA_FN)
      samp_count = self.ds_info.get_value(
          ImgClfDatasetInfoKey.TEST_SAMPLE_COUNT)
    data_fn = os.path.join(save_dir, data_fn)
        ...
        for idx in range(num_batches):
          ...
          if not self.is_multi_label():
            label_batch = to_categorical(
                label_batch,
                len(
                    self.ds_info.get_value(
                        ImgClfDatasetInfoKey.LABEL_NAME_TO_ID)))
