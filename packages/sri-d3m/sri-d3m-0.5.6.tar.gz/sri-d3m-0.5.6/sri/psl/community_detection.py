...
class CommunityDetection(pi_unsupervised_learning.UnsupervisedLearnerPrimitiveBase[Inputs, Outputs, CommunityDetectionParams, CommunityDetectionHyperparams]):
    ...
    def set_training_data(self, *, inputs: Inputs) -> None:
        self._training_graph = self._validate_inputs(inputs)
    ...
    def produce(self, *, inputs: Inputs, timeout: float = None, iterations: int = None) -> pi_base.CallResult[Outputs]:
        ...
        graph = self._validate_inputs(inputs)
        labels_target = self._write_data(graph)
        ...
        output = self._build_output(labels_target, graph, psl_output)

        return pi_base.CallResult(output)

    # Predict the strongest label for each target.
    def _build_output(self, labels_target, graph, psl_output):
        ...
        for (node, (best_label, best_score)) in best_labels.items():
            ...
            if (constants.D3M_INDEX in graph.node[node]):
                ...
                results.append([d3m_index, best_label])

        return container.DataFrame(results, columns = [constants.D3M_INDEX, community_detection_parser.COLUMN_LABEL])

    def _write_data(self, graph):
        out_dir = self.hyperparams['psl_temp_dir']
    ...
    def _validate_inputs(self, frame):
        if (len(frame) != 1):
            raise ValueError("Expected exactly one row (graph), got %d." % (len(frame)))

        graph = frame[frame.columns[0]][0]
