...
class GeneralRelational(pi_supervised_learning.SupervisedLearnerPrimitiveBase[Inputs, Outputs, GeneralRelationalParams, GeneralRelationalHyperparams]):
    ...
    def produce(self, *, inputs: Inputs, timeout: float = None, iterations: int = None) -> pi_base.CallResult[Outputs]:
        ...
        self._write_data(self.hyperparams['psl_temp_dir'], targets, inputs)
    ...
    def _build_output(self, targets, psl_output):
        # Build up the result.
        output = []

        for d3mIndex in targets:
            prediction = None
            ...
            if (self._categorical):
                ...
                for category in self._uniform_prior:
                    ...
                    if ((d3mIndex, cat_id) in psl_output):
                        category = self._id_to_cat[cat_id]
                        score = psl_output[(d3mIndex, cat_id)]
                ...
                prediction = self._mean_prediction
                if ((d3mIndex, DEFAULT_CATEGORY) in psl_output):
                    prediction = psl_output[(d3mIndex, DEFAULT_CATEGORY)]

                prediction = self._denormalize_value(prediction)

            output.append([d3mIndex, prediction])

        return container.DataFrame(output, columns = [constants.D3M_INDEX, self._prediction_column])
