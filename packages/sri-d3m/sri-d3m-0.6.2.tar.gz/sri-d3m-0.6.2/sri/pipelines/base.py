...
class BasePipeline(object):
    ...
    def _eval_predictions(self, tester, predictions, score_dir):
        columns = sorted(list(predictions.columns), key = lambda column: '' if column == 'd3mIndex' else column)
        predictions.to_csv(PREDICTIONS_PATH, index = False, columns = columns)
