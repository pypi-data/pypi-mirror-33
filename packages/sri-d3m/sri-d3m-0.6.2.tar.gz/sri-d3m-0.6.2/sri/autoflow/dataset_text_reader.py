...
class DatasetTextReader(TransformerPrimitiveBase[Inputs, Outputs, Hyperparams]):
    ...
    def produce(self, *, inputs: Inputs, timeout: float = None, iterations: int = None) -> CallResult[Outputs]:
        ...
        for ci in use_cols:
            ...
            metadata = metadata.update((rid, metadata_base.ALL_ELEMENTS, ci), mdata, source=self)
    ...
    def _produce_column(self, inputs, rid, column_index):
        ...
        column = container.DataFrame(newdata, generate_metadata=False)
        metadata = self._produce_column_metadata(
